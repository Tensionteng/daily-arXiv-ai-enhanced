...
Motivation: 高频金融系统对延迟有严格要求，需要一种能够在不确定工作负载下满足低延迟需求的方法。

Method: TIP-Search通过离线分析延迟和泛化性能，在线动态选择模型，无需显式输入域标签。

Result: 在三个真实数据集上测试，TIP-Search比静态基线准确率提升8.5%，且100%满足截止时间。
...
<details>
  <summary>Details</summary>
Motivation: 现有记忆系统在结构灵活性、时间感知和从原始数据中提取高层次洞察方面存在局限，需要更先进的记忆架构来支持大型语言模型代理的持续学习和动态适应。

Method: 采用多层时空共振图（STRG）管理语义丰富的洞察粒子（IPs），并通过语义预言接口（SOI）动态丰富IPs，形成动态知识网络。
...
<details>
  <summary>Details</summary>
Motivation: LLM在复杂工业SOP执行中表现不足，缺乏公开基准测试其能力。

Method: 开发合成数据生成框架，创建SOP-Bench基准，评估两种LLM代理架构。

Result: Function-Calling代理成功率27%，ReAct代理48%，工具调用错误率高。

Conclusion: LLM代理能力与工业SOP需求差距大，需领域特定基准和架构优化。
...
<details>
  <summary>Details</summary>
Motivation: ML领域论文提交量激增导致评审质量下降，需AI辅助提升评审效率和一致性。

Method: 提出构建AI增强的评审生态系统，利用LLM辅助作者、评审人和AC，并需更多结构化评审数据支持。

Result: 论文未提供具体实验结果，但提出了研究议程和技术挑战。

Conclusion: 呼吁ML社区开发AI辅助评审系统，以维护科学验证的完整性和可扩展性。
...
Method: 利用差异约束扩展ASP，将时间相关部分外部处理。

Result: 实现了不受时间精度影响的解决方案。

Conclusion: 该方法有效解决了ASP的瓶颈问题，提升了处理时间约束的能力。
...
<details>
  <summary>Details</summary>
Motivation: 天文台数据因远程传输限制而受限，改进无损压缩技术可显著提升科学数据获取量。

Method: 引入AstroCompress挑战，提供新数据集并比较七种无损压缩方法（三种神经和四种非神经）。

Result: 神经压缩技术可提升天文台数据收集效率，为科学应用提供指导。
...
Result: 自动形式化模块在正确证明中准确形式化57%的策略，在错误证明中准确识别30%的错误步骤。反馈生成器在准确性和相关性上优于基线。

Conclusion: LeanTutor展示了LLM在数学辅导中的潜力，尤其在形式化验证和反馈生成方面表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 集成电路设计涉及大量参数配置，微小变化可能对性能、功耗和面积产生重大影响，LLM为高维优化任务提供了新机会。

Method: ORFS-agent通过自适应探索参数配置，结合自然语言目标进行多目标优化，无需额外微调即可与前沿LLM集成。

Result: 在两个技术节点和多种电路基准测试中，ORFS-agent将布线长度和有效时钟周期改善了13%以上，同时减少40%的优化迭代次数。

Conclusion: ORFS-agent展示了LLM在高维优化任务中的潜力，提供了一种灵活、可解释且高效的参数调优框架。
...
<details>
  <summary>Details</summary>
Motivation: 在建筑设计过程中，动态迭代的平面图设计需要快速生成完整设计以减少重复修改的工作量。

Method: 提出基于Masked Autoencoders (MAE)的方法，通过掩码部分平面图并训练轻量级Vision Transformer (ViT)来重建缺失部分。

Result: 实验表明FloorplanMAE能从不完整平面图生成高质量完整设计，优于现有基准。
...
Method: 通过线性探针分析模型激活，发现推理强度由预分配方向向量控制，并通过实验验证其影响。

Result: 方向向量的幅度调节推理强度，调整向量可改变推理标记数量和性能。
...
Method: 利用规则基础的链式思考监督，帮助模型推理安全风险并做出上下文感知的拒绝。

Result: 实验表明，SafeCoT显著减少过度拒绝并提升泛化能力，即使在有限训练数据下。
...
<details>
  <summary>Details</summary>
Motivation: 现有图推荐系统在面对攻击时存在漏洞，传统攻击方法隐蔽性低且破坏性强，因此需要一种更隐蔽且影响小的攻击策略。

Method: 设计了一个单节点触发器生成器，通过插入一个虚假用户节点实现目标物品的多重曝光，并引入目标节点与无关节点之间的约束条件以减少对系统性能的影响。

Result: 实验结果显示，目标物品对99%目标用户的曝光率不低于50%，同时对推荐系统性能的影响控制在约5%以内。

Conclusion: 该方法在隐蔽性和性能影响方面优于传统攻击策略，为图推荐系统的安全性研究提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 传统手动分类法对齐方法成本高且主观性强，现有自动化方法在处理语义关系和跨领域一致性上存在不足。

Method: 结合LLMs、专家标注示例、多阶段提示工程和人工验证，生成分类法链接及支持理由。

Result: 在概念重要性映射任务中F1-score达0.97，远超人工基准0.68。
...
Method: SHIELD结合了Mixture-of-Depths（MoD）技术和基于上下文的聚类层，动态选择节点并利用层次结构优化表示。

Result: 在9个真实地图和16种VRP变体上，SHIELD显著优于现有方法，展现了更好的泛化能力。

Conclusion: SHIELD通过稀疏性和层次性设计，有效提升了多任务多分布VRP问题的解决能力。
...
<details>
  <summary>Details</summary>
Motivation: 数学推理是AI研究的重要前沿，LLMs在此领域取得显著进展，但仍有挑战。

Method: 通过预训练策略提升理解能力，从直接预测到逐步推理（CoT），并探讨了训练无关提示到微调方法。
...
<details>
  <summary>Details</summary>
Motivation: 工业过程需要鲁棒性和适应性，但传统AI依赖监督学习和标注数据，难以泛化到多变且数据稀缺的环境。

Method: CIPHER整合了过程专家、回归模型和检索增强生成技术，支持物理知识驱动的链式推理。

Result: 该框架在分布外任务中表现优异，能解释输入、生成精确指令，且无需标注数据。

Conclusion: CIPHER为工业环境中的自主系统提供了精确、透明和可信的解决方案。
...
Method: 提出RHealthTwin框架，核心是Responsible Prompt Engine（RPE），通过动态提取预定义槽位结构化输入，结合反馈循环优化提示结构。

Result: 在四个健康领域（心理支持、症状分诊、营养规划和活动指导）中，RPE在BLEU、ROUGE-L和BERTScore指标上表现优异，伦理合规和指令遵循超过90%。
...
Method: 结合梯度一致性正则化、类感知锐度最小化和曲率感知动态加权方案，增强条件分布对齐。
...
<details>
  <summary>Details</summary>
Motivation: 低空经济的快速发展推动了无人机的广泛应用，但现有研究常忽略城市空域约束和经济效率等关键因素，且DRL的学习效率限制了其实际应用。

Method: 提出了一种结合DRL与LLM推理的无人机轨迹规划框架，旨在实现安全、合规且经济可行的路径规划。

Result: 实验表明，该方法在数据收集率、避障、成功降落、法规合规性和能源效率等多项指标上显著优于现有基线。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法难以在网络拓扑约束和阶段依赖的两阶段Colonel Blotto游戏中实现全局最优策略。

Method: 提出HGformer框架，结合增强图Transformer编码器和层次化决策模型，并设计反馈强化学习算法优化策略。

Result: 实验表明HGformer在资源分配效率和对抗收益上优于现有方法。

Conclusion: HGformer在复杂动态游戏场景中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 传统对齐方法在处理高选择性和并发性模型时易导致状态空间爆炸，且无法充分表示并发行为。

Method: 使用定向Petri网展开实时计算部分序对齐。

Result: 在485个合成模型-日志对和19个真实/基准模型-日志对上验证，FoldA减少了排队状态数并更准确表示并发，但计算时间较长。

Conclusion: FoldA在减少状态空间和提高并发表示准确性方面优于传统方法，尽管计算时间增加。
...
<details>
  <summary>Details</summary>
Motivation: 开发适用于任何机器人形态的通用控制器，以提高计算和数据效率。

Method: 采用模块化循环架构，利用上下文信息推断未观测到的机器人特性。

Result: 在MuJoCo机器人上测试，对未见过的动力学、运动学和拓扑结构表现出显著改进的性能。

Conclusion: 模块化循环架构能有效提升对未训练上下文的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 直接对离散化数据应用条件独立性检验可能导致错误结论，而现有方法通过二值化处理会损失信息，影响检验性能。

Method: 利用广义矩方法（GMM）解决过识别限制问题，并通过节点回归推导检验统计量及其渐近分布。

Result: 理论和实证结果表明，该方法在多种数据集上表现出优越性和有效性。

Conclusion: 提出的方法避免了二值化过程，显著提升了条件独立性检验的准确性和效率。
...
Method: 通过识别知识图谱中的元路径子图，并利用学习排序模型优化子图选择，将排名靠前的子图融入零样本提示中，提升LLM的因果推理能力。

Result: 在生物医学和开放领域数据集上，该方法在F1分数上比基线方法最高提升44.4分。

Conclusion: 结合KGs和LLMs的方法显著提升了知识因果发现的性能和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM在数据科学中的应用日益广泛，研究其作为助手和代理的评估现状，以揭示当前研究的局限性和未来发展方向。

Method: 通过文献综述，分析LLM助手和代理在数据科学中的评估情况，总结其关注点和不足。

Result: 发现当前研究主要关注目标导向活动，忽视数据管理和探索性活动；集中在纯助手或完全自主代理，缺乏中间层次协作；强调人类替代，未考虑任务转换带来的更高自动化。

Conclusion: 未来研究应更全面地评估LLM在数据科学中的应用，包括探索性活动、中间协作层次和任务转换带来的自动化潜力。
...
<details>
  <summary>Details</summary>
Motivation: 探究LLM辅助写作对认知和行为的影响，评估其教育价值。

Method: 将参与者分为LLM、搜索引擎和无工具组，通过EEG和NLP分析认知负荷和写作质量。

Result: LLM使用者认知连接最弱，写作自主性最低，长期表现较差；无工具组认知连接最强。

Conclusion: LLM的便捷性可能带来认知代价，需深入研究AI在教育中的作用。
...
Method: POCCO设计了条件计算块将子问题路由到专用神经网络架构，并提出偏好驱动优化算法学习解决方案间的偏好。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于规则的交通模拟器难以真实模拟驾驶行为，而交叉口是交通安全和效率的关键，需数据驱动的模拟器。

Method: 提出多头部自注意力轨迹预测模型，结合信号信息，并通过模拟闭环管道评估交通工程指标。

Result: 新模型在评估指标上优于先前模型。

Conclusion: 数据驱动的模拟器结合交通工程指标，能更真实地模拟交叉口驾驶行为。
...
<details>
  <summary>Details</summary>
Motivation: 交通路口是事故多发区，现有模型仅关注轨迹重建误差，缺乏对交通工程实际问题的考量。

Method: 训练多车辆轨迹预测模型，使用真实路口数据，通过微仿真评估模型在未知交通条件下的表现。

Result: 模型输入理想轨迹时仍生成违反交通规则的行为，新指标能有效捕捉这些行为。

Conclusion: 新工具和指标为交通工程提供了更全面的模型评估视角。
...
Result: 总结了NKGs链接预测的现状，并提出了未来研究方向。

Conclusion: 本文为NKGs链接预测领域提供了重要参考，并指明了未来研究的潜力方向。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs在现实场景中面对不确定或无法回答的问题时拒绝回答的能力，以提升其可靠性。

Method: 引入AbstentionBench基准，评估20个前沿LLMs在多种问题类型（如未知答案、模糊前提等）下的表现。

Result: 研究发现LLMs在拒绝回答方面表现不佳，推理微调反而降低能力（平均24%），系统提示虽能改善但无法根本解决问题。

Conclusion: AbstentionBench的发布旨在推动LLM可靠性的研究，当前模型在拒绝回答问题上仍有待改进。
...
<details>
  <summary>Details</summary>
Motivation: 解决多智能体在动态环境中的协作问题，尤其是基于视觉输入的推理和多样化具身类型的支持不足。

Method: 提出VIKI-Bench基准测试和VIKI-R框架，后者通过预训练视觉语言模型和强化学习实现多级奖励信号下的协作。

Result: VIKI-R在所有任务层次上显著优于基线方法，并展示了异构智能体之间的组合协作模式。
...
<details>
  <summary>Details</summary>
Motivation: 研究AI系统在解决复杂优化问题（如物流、排班和生产规划）中的表现，填补现有基准测试的不足。

Method: 基于AtCoder Heuristic Contests的真实任务，设计支持迭代优化的软件框架，评估前沿LLM的性能。

Result: 前沿LLM在特定问题上表现优异，但在问题一致性和长期规划能力上与人类仍有差距。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM推理中KV Cache的高内存需求问题，避免现有方法在精度分配和动态优先级处理上的不足。

Method: 利用梯度重要性分析，动态分配层特定比特宽度，优化长上下文任务中的KV对精度，并提供高效的低比特量化和CUDA内核。
...
<details>
  <summary>Details</summary>
Motivation: 传统的难民统计数据通常局限于行政边界，难以揭示局部化的流离失所模式。本研究旨在通过高分辨率的数据分解，提供更细致的难民分布信息。

Method: 整合了UNHCR的ProGres注册数据、Google Open Buildings的卫星建筑足迹数据以及OpenStreetMap的人口地点坐标，采用标签传播算法生成高精度的网格单元难民统计数据。

Result: 该方法在将1000多万难民观察数据分配到网格单元时，平均准确率达到92.9%，能够识别出传统统计数据中隐藏的局部流离失所模式。

Conclusion: 生成的高分辨率难民数据集为深入理解流离失所的驱动因素提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 部分领域自适应问题需要对齐跨领域样本并区分异常类，现有加权框架仅能表征样本级关系，对异常类的识别不够准确。
...
Method: 集成POD降维与预训练LLM微调，设计流体动力学文本模板以增强预测性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法未能有效抑制LLM内部偏差，且依赖离线数据，无法适应动态分布变化。

Method: MBPO通过对抗性扰动生成硬负样本，结合GRPO使用离线-在线混合数据训练模型。

Result: 实验表明MBPO能显著提升多模态模型性能并减少幻觉。

Conclusion: MBPO为解决模态不平衡问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何在减少比特表示的同时保持模型精度，尤其是在NVIDIA Blackwell GPU的MX格式下实现高效预训练。

Method: 提出了一种改进的舍入模式（round-to-infinity），用于计算MXFP8格式的缩放因子，解决了原OCP规范中舍入模式导致的问题。

Result: 成功在MXFP8格式下预训练了一个8B参数的模型，使用了15T tokens的数据集。

Conclusion: 改进的舍入模式能够有效支持MXFP8格式下的LLM预训练，避免了原方法的发散问题。
...
Method: 使用细粒度和H3空间单元聚合的图表示，结合多模态数据，评估多种GNN架构，最终采用DSTGCN在粗粒度图上实现预测。

Result: ST-GraphNet在测试集上达到97.74%的准确率，显著优于细粒度模型（64.7%）。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在块状缺失数据场景中提取特征效果不佳，静态图结构限制了模型对非平稳数据的灵活性。

Method: 提出混合专家框架（MoE）捕获时空特征及其权重，设计低秩采样图注意力机制（LrSGAT）动态平衡局部和全局相关性。
...
<details>
  <summary>Details</summary>
Motivation: 减少监督微调的计算开销，探索大型语言模型的高效部署方法。

Method: 基于理想化假设（无限计算资源和完整数据集），证明上下文学习可近似微调能力，并扩展到有限上下文和部分数据集场景。

Result: 在文本生成和线性分类任务中，给出了所需数据集大小的理论边界。

Conclusion: 为大型语言模型的高效部署提供了理论基础，实际技术（如检索增强生成）可连接理论与应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有线性标量化方法无法捕捉公平导向目标（如Nash社会福利或最大最小公平），且离线环境下非线性福利标准的统一优化方法尚未探索。

Method: FairDICE利用分布校正估计，联合考虑福利最大化和分布正则化，实现稳定且样本高效的学习。

Result: 在多个离线基准测试中，FairDICE表现出优于现有基线的公平感知性能。

Conclusion: FairDICE为离线多目标强化学习中的非线性福利优化提供了首个统一解决方案。
...
Method: Lite-RVFL通过引入新的目标函数，为新增样本分配指数级增加的权重，强调近期数据并实现及时适应。

Result: 理论分析验证了目标函数的可行性，实验结果表明Lite-RVFL在适应漂移和捕捉时间模式方面高效有效。

Conclusion: Lite-RVFL是一种适用于实时应用的高效解决方案，能够有效应对概念漂移。
...
<details>
  <summary>Details</summary>
Motivation: 现实数据不断增长，传统方法保留所有数据导致效率低下，主动学习虽减少冗余但引入偏差。

Method: 利用任务特定模型和开源模型，选择性标注并整合在线和网络数据。

Result: 在ImageNet-1K等数据集上，减少32%的标注和训练成本，性能无损失；半监督学习下可进一步降至50%。

Conclusion: Info-Coevolution高效提升数据集质量，无需调参，支持检索式数据集增强。
...
<details>
  <summary>Details</summary>
Motivation: 电力价格预测对电力现货市场交易决策至关重要，但生成式人工智能和时间序列基础模型在EPF中的有效性尚不明确。
...
Conclusion: 双季节性MSTL模型在电力价格预测中表现最优，TSFMs尚未超越传统方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统算法（如k-NN）在结合神经网络特征提取时表现优异，但由于其不可微分性难以直接整合到监督微调中。

Method: 引入NONA回归层，利用注意力机制和学习的注意力掩码方案，近似k-NN回归算法。

Result: 在多个非结构化数据集上，NONA在回归任务中优于密集层预测和基于SFT嵌入的k-NN。

Conclusion: NONA为传统算法与神经网络的结合提供了可行方案，显著提升了回归性能。
...
### [51] [Accelerating Spectral Clustering under Fairness Constraints](https://arxiv.org/abs/2506.08143)
*Francesco Tonin,Alex Lambert,Johan A. K. Suykens,Volkan Cevher*

Main category: cs.LG

TL;DR: 提出了一种高效的公平谱聚类方法（Fair SC），通过将问题转化为凸差函数框架，显著提高了计算效率。


<details>
  <summary>Details</summary>
Motivation: 解决决策算法中的公平性问题，确保每个聚类中的人口群体比例与总体一致。

Method: 采用凸差函数（DC）框架和变量增强策略，结合ADMM算法优化子问题。

Result: 在合成和真实数据集上验证了方法的有效性，计算速度显著优于现有方法。
...
### [52] [Fully data-driven inverse hyperelasticity with hyper-network neural ODE fields](https://arxiv.org/abs/2506.08146)
*Vahidullah Taç,Amirhossein Amiri-Hezaveh,Manuel K. Rausch,Grace N. Bechtel,Francisco Sahli Costabal,Adrian Buganza Tepole*

Main category: cs.LG

TL;DR: 提出了一种新框架，用于识别异质材料的力学特性，无需封闭形式的本构方程。结合神经网络和物理驱动方法，通过优化超网络参数实现材料识别。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要封闭形式的本构方程，限制了异质材料力学特性的识别。新框架旨在克服这一限制，提供更灵活和通用的解决方案。

Method: 使用神经网络结合傅里叶特征近似应变场，基于NODE发现本构方程，并通过超网络处理异质性。优化多目标损失函数以满足平衡方程和边界条件。

Result: 数值实验表明，该方法在识别异质材料力学特性时具有鲁棒性和通用性，适用于噪声环境和实验数据。

Conclusion: 该框架为异质材料力学特性识别提供了有效的替代方案，减少了对传统方法的依赖。
...
### [53] [BLUR: A Bi-Level Optimization Approach for LLM Unlearning](https://arxiv.org/abs/2506.08164)
...
TL;DR: 论文提出了一种新的遗忘问题分层建模方法，并基于此设计了Bi-Level UnleaRning（BLUR）算法，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 确保大语言模型（LLMs）在训练后能遗忘特定知识或能力，以符合数据法规和伦理要求，同时避免性能下降。

Method: 采用双层优化框架，下层目标最小化遗忘损失，上层目标保持模型效用，提出BLUR算法。

Result: BLUR在多种任务、模型和指标上均优于现有方法。

Conclusion: BLUR算法通过分层建模解决了遗忘问题的优先级，具有理论保证和实际性能优势。
...
### [54] [UniVarFL: Uniformity and Variance Regularized Federated Learning for Heterogeneous Data](https://arxiv.org/abs/2506.08167)
*Sunny Gupta,Nikita Jangid,Amit Sethi*

Main category: cs.LG

TL;DR: UniVarFL是一种新的联邦学习框架，通过两种正则化策略解决非独立同分布数据下的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法在处理非独立同分布数据时存在高计算成本或适应性问题，UniVarFL旨在直接模拟IID训练动态。

Method: 采用分类器方差正则化和超球面均匀性正则化，分别优化概率分布和特征表示。

Result: 在多个基准数据集上表现优于现有方法，准确性更高。
...
### [55] [Federated Learning on Stochastic Neural Networks](https://arxiv.org/abs/2506.08169)
*Jingqiao Tang,Ryan Bausback,Feng Bao,Richard Archibald*

Main category: cs.LG

TL;DR: 联邦学习通过边缘计算优化模型并保护用户隐私，但本地数据噪声可能影响性能。本文提出使用随机神经网络作为本地模型，以估计真实数据状态并量化噪声，称为联邦随机神经网络。实验验证了其处理非独立同分布数据的有效性。


<details>
  <summary>Details</summary>
Motivation: 联邦学习依赖客户端数据，但数据噪声可能影响模型性能，需解决这一问题。

Method: 在联邦学习框架中引入随机神经网络作为本地模型，估计数据真实状态并量化噪声。

Result: 实验证明联邦随机神经网络能有效处理非独立同分布数据。

Conclusion: 随机神经网络在联邦学习中能有效应对数据噪声问题，提升模型性能。
...
### [56] [FedGA-Tree: Federated Decision Tree using Genetic Algorithm](https://arxiv.org/abs/2506.08176)
*Anh V Nguyen,Diego Klabjan*

Main category: cs.LG

TL;DR: 本文提出了一种基于遗传算法的个性化决策树构建方法，适用于联邦学习中的分类和回归任务，优于本地数据和基准算法。


<details>
  <summary>Details</summary>
Motivation: 当前联邦学习主要关注基于梯度的参数模型，而对非参数模型（如决策树）的研究较少，且现有方法受限于分类树和分类数据。

Method: 利用遗传算法构建个性化决策树，支持分类和数值数据，适用于分类和回归任务。

Result: 实验表明，该方法优于仅基于本地数据训练的决策树和基准算法。

Conclusion: 遗传算法为联邦学习中的决策树提供了一种灵活且高效的方法，适用于更广泛的数据类型和任务。
...
### [57] [Correlated Noise Mechanisms for Differentially Private Learning](https://arxiv.org/abs/2506.08201)
...
TL;DR: 该专著研究了差分隐私（DP）中相关噪声机制的设计与分析，重点应用于通过加权前缀和估计的AI和机器学习模型私有训练。相比传统独立噪声注入，相关噪声机制通过噪声的（反）相关性显著提升隐私-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 传统DP机制在随机梯度下降（SGD）中独立注入噪声保护训练数据隐私，但研究表明相关噪声能通过噪声抵消优化隐私-效用平衡。

Method: 采用矩阵机制、因子分解机制及DP-FTRL等方法，在训练算法中引入相关噪声。

Result: 相关噪声机制在实践中广泛应用，已在全球范围内工业部署。

Conclusion: 相关噪声机制显著提升差分隐私的隐私-效用权衡，具有实际应用价值。
...
### [58] [A Machine Learning Approach to Generate Residual Stress Distributions using Sparse Characterization Data in Friction-Stir Processed Parts](https://arxiv.org/abs/2506.08205)
*Shadab Anwar Shaikh,Kranthi Balusu,Ayoub Soulami*

Main category: cs.LG

TL;DR: 本文提出了一种基于机器学习的残余应力生成器（RSG），通过有限测量推断全场应力分布，显著减少实验工作量。


<details>
  <summary>Details</summary>
Motivation: 残余应力会影响组件性能，但全场应力分布的实验表征工作量过大，因此需要一种高效方法。

Method: 通过大量工艺模拟构建数据集，基于U-Net架构训练机器学习模型，并通过超参数调优优化性能。

Result: 模型在模拟应力预测中表现出高精度和泛化能力，实验数据验证了其有效性。

Conclusion: RSG方法能从有限测量中全面理解残余应力分布，显著降低实验需求。
...
### [59] [What makes an Ensemble (Un) Interpretable?](https://arxiv.org/abs/2506.08216)
*Shahaf Bassan,Guy Amir,Meirav Zehavi,Guy Katz*

Main category: cs.LG

TL;DR: 论文探讨了集成模型的可解释性问题，通过计算复杂性理论分析了影响其可解释性的因素。


<details>
  <summary>Details</summary>
Motivation: 集成模型（如提升树）通常被视为黑箱，缺乏对其可解释性的数学理解。本文旨在填补这一空白。

Method: 应用计算复杂性理论，研究不同集成配置下生成解释的挑战。

Result: 发现集成模型的解释复杂性受多种因素影响，例如小规模决策树集成可高效解释，而线性模型集成则难以解释。
...
### [60] [Mondrian: Transformer Operators via Domain Decomposition](https://arxiv.org/abs/2506.08226)
*Arthur Feeney,Kuei-Hsiang Huang,Aparna Chandramowlishwaran*

Main category: cs.LG

TL;DR: Mondrian是一种基于Transformer的算子学习方法，通过将域分解为不重叠的子域并应用注意力机制，解决了高分辨率多尺度域中注意力计算成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer算子模型在高分辨率多尺度域中因注意力计算成本高和与离散化耦合而难以扩展的问题。

Method: 将域分解为不重叠的子域，在每个子域内使用神经算子替换标准层，并通过基于softmax的内积计算子域间的注意力。支持分层窗口和邻域注意力。

Result: 在Allen-Cahn和Navier-Stokes PDEs上表现出色，支持分辨率扩展而无需重新训练。
...
### [61] [Scaling Laws of Motion Forecasting and Planning -- A Technical Report](https://arxiv.org/abs/2506.08228)
...
TL;DR: 研究了编码器-解码器自回归Transformer模型在自动驾驶领域中的联合运动预测和规划任务上的经验性扩展规律。


<details>
  <summary>Details</summary>
Motivation: 探索模型性能如何随计算预算增加而提升，并验证开环指标是否适用于模型开发。

Method: 使用50万小时驾驶数据集，分析模型训练损失与评估指标的关系，研究模型参数与数据规模的优化比例。

Result: 性能随计算预算呈幂律提升，模型参数规模需比数据规模增长快1.5倍。小模型通过采样和聚类在推理时表现接近大模型。

Conclusion: 优化训练和推理扩展性是提升自动驾驶模型性能的关键，同时利用其他代理的驾驶数据可缓解机器人数据稀缺问题。
...
### [62] [Ensuring Reliability of Curated EHR-Derived Data: The Validation of Accuracy for LLM/ML-Extracted Information and Data (VALID) Framework](https://arxiv.org/abs/2506.08231)
...
TL;DR: 本文提出一个评估LLM提取临床数据质量的综合框架，包括性能基准测试、自动验证和复制分析，以提升数据可靠性和公平性。


<details>
  <summary>Details</summary>
Motivation: LLM在临床数据提取中虽高效但存在可靠性、准确性和公平性问题，现有框架未能完全解决。

Method: 提出多维框架，包括性能基准测试、自动验证和复制分析，支持偏差评估。

Result: 框架能识别需改进变量、系统性错误，并确认数据适用性，支持AI证据生成的可靠使用。

Conclusion: 该框架为LLM提取的RWD提供严格评估方法，推动行业标准，支持肿瘤学研究和实践中的可信AI应用。
...
### [63] [Dealing with the Evil Twins: Improving Random Augmentation by Addressing Catastrophic Forgetting of Diverse Augmentations](https://arxiv.org/abs/2506.08240)
*Dongkyu Cho,Rumi Chunara*

Main category: cs.LG

TL;DR: 本文探讨了随机数据增强的局限性，并提出了一种简单方法来解决其导致的特征失真问题，从而提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: 随机数据增强虽然成本低，但因其局限性被认为效果不佳。本文旨在解决其导致的特征失真问题，提升其在单源域泛化（sDG）中的表现。

Method: 通过分析随机增强的随机性导致的特征失真（类似灾难性遗忘），提出了一种简单解决方案来缓解这一问题。

Result: 该方法在多个单源域泛化基准测试中表现出强大的泛化性能。

Conclusion: 研究表明，通过解决随机增强的局限性，可以显著提升其泛化效果，为低成本数据增强提供了新思路。
...
### [64] [Temporalizing Confidence: Evaluation of Chain-of-Thought Reasoning with Signal Temporal Logic](https://arxiv.org/abs/2506.08243)
*Zhenjiang Mao,Artem Bisliouk,Rohith Reddy Nama,Ivan Ruchkin*

Main category: cs.LG

TL;DR: 该论文提出了一种基于信号时序逻辑（STL）的结构化框架，用于评估大语言模型（LLMs）在数学推理任务中的逐步置信度，并通过不确定性重塑策略提升校准效果。
...
Method: 提出STL框架，定义时序约束并计算鲁棒性分数作为置信度估计，同时引入不确定性重塑策略。

Result: 实验表明，该方法显著改善了校准指标，提供了比传统方法更可靠的置信度估计。

Conclusion: 该框架为LLMs的推理过程提供了结构化、可解释的置信度评估，提升了可靠性。
...
### [65] [Parameter-free approximate equivariance for tasks with finite group symmetry](https://arxiv.org/abs/2506.08244)
*Riccardo Ali,Pietro Liò,Jamie Vicary*

Main category: cs.LG

TL;DR: 提出一种零参数的近似等变方法，通过在损失函数中添加额外项，在潜在空间中实现对称性，实验表明其性能优于现有方法且参数更少。


<details>
  <summary>Details</summary>
Motivation: 现有等变方法计算复杂、参数多且架构固定，需要一种更简单高效的方法。

Method: 在潜在空间中通过损失函数的额外项实现近似等变性，让网络学习群表示。

Result: 实验显示网络倾向于学习正则表示，性能与现有方法相当或更好，且参数更少。

Conclusion: 该方法简单高效，适用于多种任务，显著降低了计算和参数成本。
...
### [66] [SHIELD: Secure Hypernetworks for Incremental Expansion Learning Defense](https://arxiv.org/abs/2506.08255)
*Patryk Krukowski,Łukasz Gorczyca,Piotr Helm,Kamil Książek,Przemysław Spurek*

Main category: cs.LG

TL;DR: SHIELD结合超网络和区间算术，解决持续学习中的灾难性遗忘和对抗攻击问题。


<details>
  <summary>Details</summary>
Motivation: 传统深度神经网络存在灾难性遗忘和对抗攻击脆弱性问题，目前缺乏同时解决两者的模型。

Method: SHIELD通过超网络动态生成针对特定数据的子网络，并结合区间算术提供严格的安全保证。

Result: SHIELD在增强安全性的同时不牺牲网络适应性，解决了持续学习中的安全问题。

Conclusion: SHIELD为持续学习中的灾难性遗忘和对抗攻击提供了创新解决方案。
...
### [67] [Reinforcement Learning from Human Feedback with High-Confidence Safety Constraints](https://arxiv.org/abs/2506.08266)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在安全性和帮助性之间存在权衡，可能导致敏感领域的不当回应，因此需要一种更可靠的方法。

Method: HC-RLHF将人类偏好分解为帮助性和安全性，分别训练奖励模型和成本模型，并通过两步优化和安全性测试确保安全。
...
### [68] [Sparse Interpretable Deep Learning with LIES Networks for Symbolic Regression](https://arxiv.org/abs/2506.08267)
*Mansooreh Montazerin,Majd Al Aawar,Antonio Ortega,Ajitesh Srivastava*

Main category: cs.LG

TL;DR: LIES框架通过固定神经网络架构和优化策略，高效生成稀疏且准确的符号表达式，优于现有方法。
...
Method: LIES采用固定神经网络架构（包含对数、恒等、指数、正弦激活函数），结合过采样策略和定制损失函数优化，训练后通过剪枝简化表达式。
...
### [69] [SWAT-NN: Simultaneous Weights and Architecture Training for Neural Networks in a Latent Space](https://arxiv.org/abs/2506.08270)
...
<details>
  <summary>Details</summary>
Motivation: 传统神经网络设计依赖手动试错或离散化的架构搜索与权重优化，效率低且耗时。

Method: 训练多尺度自编码器嵌入架构和参数信息至连续潜在空间，通过梯度下降联合优化结构和权重，并引入稀疏性和紧凑性惩罚。

Result: 在合成回归任务中，该方法成功发现性能优异的稀疏紧凑神经网络。

Conclusion: 该方法为神经网络设计提供了一种高效、自动化的新途径。
...
### [70] [Universal Differential Equations for Scientific Machine Learning of Node-Wise Battery Dynamics in Smart Grids](https://arxiv.org/abs/2506.08272)
*Tarushri N. S.*
...
<details>
  <summary>Details</summary>
Motivation: 智能电网中电池动态建模因太阳能输入随机性和家庭负载变化而复杂，传统方法难以捕捉未建模的动态。

Method: 提出UDE方法，将神经残差嵌入物理启发的电池ODE中，学习节点特定的电池动态。

Result: 实验显示UDE与真实电池轨迹高度吻合，收敛平滑且长期预测稳定。
...
### [71] [The Impact of Feature Scaling In Machine Learning: Effects on Regression and Classification Tasks](https://arxiv.org/abs/2506.08274)
...
TL;DR: 系统评估12种特征缩放技术对14种机器学习算法和16个数据集的影响，发现集成方法对缩放不敏感，而其他模型性能显著依赖缩放选择。


<details>
  <summary>Details</summary>
Motivation: 解决特征缩放领域缺乏全面研究的问题，为实践者提供具体指导。
...
### [72] [From Debate to Equilibrium: Belief-Driven Multi-Agent LLM Reasoning via Bayesian Nash Equilibrium](https://arxiv.org/abs/2506.08292)
...
Method: 将多LLM协调建模为不完全信息博弈，利用贝叶斯纳什均衡优化响应策略，结合分布式推理与集中输出。

Result: ECON在六个复杂推理和规划任务上平均优于现有方法11.2%，并具有扩展性。

Conclusion: ECON为高效、可扩展的多LLM协作提供了新思路，代码已开源。
...
### [73] [From Passive to Active Reasoning: Can Large Language Models Ask the Right Questions under Incomplete Information?](https://arxiv.org/abs/2506.08295)
*Zhanke Zhou,Xiao Feng,Zhaocheng Zhu,Jiangchao Yao,Sanmi Koyejo,Bo Han*

Main category: cs.LG

TL;DR: AR-Bench是一个新基准，用于评估大语言模型（LLM）的主动推理能力，发现当前模型在主动推理中存在显著困难。
...
### [74] [H$^2$GFM: Towards unifying Homogeneity and Heterogeneity on Text-Attributed Graphs](https://arxiv.org/abs/2506.08298)
*Trung-Kien Nguyen,Heng Ping,Shixuan Li,Peiyu Zhang,Nikos Kanakaris,Nicholas Kotov,Paul Bogdan*

Main category: cs.LG

TL;DR: 论文提出了一种名为H$^2$GFM的框架，旨在统一处理同质和异质文本属性图（TAGs），通过上下文编码和新型图变换器提升模型泛化能力。
...
Method: 提出H$^2$GFM框架，利用统一文本空间投影多样元关系，采用上下文编码捕捉语义关系，并设计上下文自适应图变换器（CGT）增强节点表示。

Result: 在多种HoTAGs和HeTAGs上的实验验证了模型的有效性。

Conclusion: H$^2$GFM显著提升了图基础模型在异质图上的泛化能力。
...
### [75] [Learnable Spatial-Temporal Positional Encoding for Link Prediction](https://arxiv.org/abs/2506.08309)
*Katherine Tieu,Dongqi Fu,Zihao Li,Ross Maciejewski,Jingrui He*

Main category: cs.LG

TL;DR: 论文提出了一种可学习的时空位置编码方法L-STEP，解决了现有位置编码方法的局限性，并在多个数据集和算法中验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 现有位置编码方法在适应性、动态性和计算效率上存在不足，无法满足复杂属性图和动态拓扑的需求。

Method: 提出L-STEP方法，通过可学习的时空位置编码，结合MLP实现高效表达，并验证其理论复杂度和实际性能。

Result: L-STEP在13个经典数据集和10种算法中表现优异，并在TGB基准测试中取得领先性能。

Conclusion: L-STEP是一种高效且有效的位置编码方法，适用于动态图数据，并在多个任务中优于现有方法。
...
### [76] [Private Evolution Converges](https://arxiv.org/abs/2506.08312)
*Tomás González,Giulia Fanti,Aaditya Ramdas*

Main category: cs.LG

TL;DR: PE是一种无需训练的差分隐私合成数据生成方法，在某些领域表现优异，但在表格数据中表现不一致。本文提出新的理论框架分析PE的收敛性，并证明其生成的数据与原始数据的1-Wasserstein距离的理论界限。


<details>
  <summary>Details</summary>
Motivation: 解决PE在理论和实践中表现不一致的问题，提供更普适的理论分析框架。

Method: 提出新的理论框架，分析PE在d维数据集上的收敛性，并扩展到一般Banach空间。

Result: 证明PE生成的数据与原始数据的1-Wasserstein距离的理论界限为$\tilde{O}(d(n\epsilon)^{-1/d})$。

Conclusion: PE在理论上和实践中的表现得到解释，为差分隐私合成数据生成提供了新的理论支持。
...
### [77] [Why Masking Diffusion Works: Condition on the Jump Schedule for Improved Discrete Diffusion](https://arxiv.org/abs/2506.08316)
...
<details>
  <summary>Details</summary>
Motivation: 研究离散扩散模型中掩码扩散表现优异的原因，并探索如何将这一优势推广到其他离散扩散模型。

Method: 提出了一种新方法SCUD（schedule-conditioned discrete diffusion），将跳跃时间分布的知识融入离散扩散模型，并应用于图像、文本和蛋白质数据。

Result: SCUD方法在多种数据上优于掩码扩散模型。

Conclusion: 通过利用离散马尔可夫过程的特性，SCUD方法能够改进传统离散扩散模型，并在实际应用中表现出色。
...
### [78] [Graph Prompting for Graph Learning Models: Recent Advances and Future Directions](https://arxiv.org/abs/2506.08326)
*Xingbo Fu,Zehong Wang,Zihan Chen,Jiazheng Li,Yaochen Zhu,Zhenyu Lei,Cong Shen,Yanfang Ye,Chuxu Zhang,Jundong Li*

Main category: cs.LG

TL;DR: 本文系统综述了图提示（graph prompting）的最新进展，包括图预训练方法、主流图提示技术及其应用，并探讨了未来研究方向。
...
Method: 1. 介绍图预训练方法；2. 综述主流图提示技术及其设计；3. 总结图提示在不同领域的应用。

Result: 图提示通过可学习提示调整预训练模型，适应下游任务，展现了广泛的应用前景。

Conclusion: 图提示是图学习领域的重要方向，但仍面临挑战，需进一步研究以优化其性能和应用范围。
...
### [79] [A Simple Analysis of Discretization Error in Diffusion Models](https://arxiv.org/abs/2506.08337)
*Juhyeok Choi,Chenglin Fan*

Main category: cs.LG

TL;DR: 本文提出了一种简化的理论框架，用于分析DDPM中VP-SDE的Euler--Maruyama离散化误差，证明了收敛速度为$\mathcal{O}(1/T^{1/2})$，并展示了离散噪声可替代高斯噪声而不影响收敛。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型的离散化误差分析依赖复杂概率工具，本文旨在简化分析并提供更实用的噪声替代方案。

Method: 利用Grönwall不等式推导收敛速度，并实验验证离散噪声（如Rademacher或均匀噪声）的效果。

Result: 实验验证了误差按预测比例缩放，离散噪声与高斯噪声质量相当，噪声缩放错误会降低性能。

Conclusion: 本文通过简化分析和离散噪声替代，将理论严谨性与实际效率结合，推动了扩散生成模型的进展。
...
### [80] [Dynamical System Optimization](https://arxiv.org/abs/2506.08340)
*Emo Todorov*

Main category: cs.LG

TL;DR: 论文提出了一种优化框架，通过将控制权转移给策略，从而在自主动态系统层面优化策略参数，无需直接使用动态规划或强化学习。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖动态规划和强化学习，复杂度高。本文旨在简化优化过程，直接在策略层面操作。

Method: 在自主系统层面设计算法，计算策略梯度、Hessian矩阵等，并与传统方法结果一致。

Result: 证明了算法与传统方法（如策略梯度、自然梯度）等价，适用于行为克隆、系统辨识等任务。

Conclusion: 该框架简化了优化过程，适用于更广泛的任务，如生成式AI模型的调优。
...
### [81] [Differentially Private Relational Learning with Entity-level Privacy Guarantees](https://arxiv.org/abs/2506.08347)
*Yinan Huang,Haoteng Ying,Eli Chien,Rongzhe Wei,Pan Li*

Main category: cs.LG

TL;DR: 本文提出了一种针对关系型数据的差分隐私学习框架，解决了直接应用DP-SGD的挑战，包括实体参与多关系导致的高敏感性和多阶段采样问题。


<details>
  <summary>Details</summary>
Motivation: 在敏感领域中保护个体隐私的需求日益增长，但现有方法难以直接应用于关系型数据。

Method: 通过敏感性分析和自适应梯度裁剪方案，扩展了隐私放大结果，提出了一种专为关系型数据设计的DP-SGD变体。

Result: 实验表明，该方法在文本属性网络数据上实现了良好的隐私-效用平衡。

Conclusion: 该框架为关系型学习提供了严格的实体级差分隐私保证，具有实际应用价值。
...
### [82] [An Adaptive Method Stabilizing Activations for Enhanced Generalization](https://arxiv.org/abs/2506.08353)
...
Method: AdaAct通过动态调整学习率（基于激活方差）实现神经元输出的稳定性优化，并与Adam和SGD的性能进行对比。

Result: 在CIFAR和ImageNet基准测试中，AdaAct表现优异，平衡了Adam的收敛速度和SGD的泛化能力。

Conclusion: AdaAct是一种有效的优化算法，兼具收敛速度和泛化能力，代码已开源。
...
### [83] [NysAct: A Scalable Preconditioned Gradient Descent using Nystrom Approximation](https://arxiv.org/abs/2506.08360)
*Hyunseok Seung,Jaewoo Lee,Hyunsuk Ko*
...
<details>
  <summary>Details</summary>
Motivation: 自适应梯度方法计算高效但泛化能力差，二阶方法泛化能力强但计算和内存成本高，需要一种平衡两者的方法。
...
### [84] [AlphaFold Database Debiasing for Robust Inverse Folding](https://arxiv.org/abs/2506.08365)
*Cheng Tan,Zhenxiao Cao,Zhangyang Gao,Siyuan Li,Yufei Huang,Stan Z. Li*

Main category: cs.LG

TL;DR: AlphaFold数据库（AFDB）的结构数据虽然高精度，但在训练对原子几何敏感的深度学习模型时存在系统性偏差。通过引入Debiasing Structure AutoEncoder（DeSAE），可以纠正AFDB的结构偏差，显著提升逆折叠等任务的性能。


<details>
  <summary>Details</summary>
Motivation: AFDB的结构数据虽然覆盖广且精度高，但其几何特征与实验数据（PDB）存在系统性偏差，影响模型在逆折叠等任务中的泛化能力。

Method: 提出DeSAE模型，通过从故意损坏的骨架几何中重建天然构象，学习更稳健的结构流形。

Result: DeSAE纠正了AFDB的结构偏差，显著提升了逆折叠任务在多个基准测试中的性能。

Conclusion: 研究揭示了预测结构中的系统性偏差对下游任务的影响，并提供了去偏框架，显著提升了结构学习任务的性能。
...
### [85] [Reinforce LLM Reasoning through Multi-Agent Reflection](https://arxiv.org/abs/2506.08379)
...
Method: 将多轮优化建模为马尔可夫决策过程，提出DPSDP算法，基于自生成数据直接偏好学习。
...
### [86] [Network Threat Detection: Addressing Class Imbalanced Data with Deep Forest](https://arxiv.org/abs/2506.08383)
*Jiaqi Chen,Rongbin Ye*

Main category: cs.LG

TL;DR: 论文提出了一种基于机器学习的恶意流量检测方法，通过处理数据集不平衡问题，结合集成方法（如gcForest）提升了检测性能。
...
### [87] [Reinforcement Learning Teachers of Test Time Scaling](https://arxiv.org/abs/2506.08388)
*Edoardo Cetin,Tianyu Zhao,Yujin Tang*

Main category: cs.LG

TL;DR: 提出了一种新的强化学习教师（RLT）框架，专注于高效蒸馏，避免了传统RL的探索问题。


<details>
  <summary>Details</summary>
Motivation: 传统RL训练推理语言模型依赖初始探索能力，而RLT旨在通过详细解释直接提升学生模型性能。

Method: RLT通过问题和解决方案生成详细解释，利用密集奖励训练，直接优化学生理解。

Result: 7B规模的RLT在竞赛和研究生任务中表现优于现有蒸馏方法，且适用于更大模型和零样本任务。

Conclusion: RLT框架显著提升了推理模型的效率和复用性。
...
### [88] [Spatiotemporal deep learning models for detection of rapid intensification in cyclones](https://arxiv.org/abs/2506.08397)
*Vamshika Sutar,Amandeep Singh,Rohitash Chandra*

Main category: cs.LG

TL;DR: 论文探讨了利用深度学习和数据增强技术解决气旋快速增强检测中的类别不平衡问题，并验证了空间坐标作为关键输入特征的重要性。
...
Method: 结合深度学习、集成学习和数据增强框架，生成模拟气旋的空间坐标和风强度数据，并利用深度学习分类模块区分快速与非快速增强事件。

Result: 数据增强显著提升了快速增强检测效果，空间坐标是关键输入特征。

Conclusion: 该研究为极端时空事件合成数据生成提供了新思路。
...
### [89] [FUSE: Measure-Theoretic Compact Fuzzy Set Representation for Taxonomy Expansion](https://arxiv.org/abs/2506.08409)
*Fred Xu,Song Jiang,Zijie Huang,Xiao Luo,Shichang Zhang,Adrian Chen,Yizhou Sun*

Main category: cs.LG

TL;DR: 论文提出了一种基于模糊集的集合表示学习方法FUSE，用于分类学扩展任务，显著优于现有基线。
...
Method: 提出FUSE框架，通过体积近似模糊集来学习集合表示，满足所有集合操作且高效。

Result: 在分类学扩展任务中，FUSE比现有基线提升高达23%。

Conclusion: FUSE是首次尝试理解和高效计算模糊集嵌入的工作，为集合表示学习提供了新思路。
...
### [90] [Learning to Hear Broken Motors: Signature-Guided Data Augmentation for Induction-Motor Diagnostics](https://arxiv.org/abs/2506.08412)
*Saraa Ali,Aleksandr Khizhik,Stepan Svirin,Artem Ryzhikov,Denis Derkach*

Main category: cs.LG

TL;DR: 提出了一种结合机器学习和无监督异常生成方法的新型框架SGDA，用于三相电机的智能诊断，显著提升了诊断性能和准确性。
...
Method: 结合ML算法与无监督异常生成方法，提出SGDA框架，在健康电流信号的频域中合成物理合理的故障。

Result: SGDA框架实现了更高的诊断准确性和可靠性，适用于广泛工业应用。

Conclusion: 该方法为电机诊断领域提供了高效、鲁棒的解决方案，具有实际应用潜力。
...
### [91] [Improved Scaling Laws in Linear Regression via Data Reuse](https://arxiv.org/abs/2506.08415)
...
Result: 多轮SGD实现了更优的测试误差（Θ(M^{1-b} + L^{(1-b)/a})），优于单轮SGD（Θ(M^{1-b} + N^{(1-b)/a})）。

Conclusion: 数据重用（L>N）在数据受限情况下能改进缩放定律，数值模拟验证了理论结果。
...
### [92] [Offline RL with Smooth OOD Generalization in Convex Hull and its Neighborhood](https://arxiv.org/abs/2506.08417)
...
<details>
  <summary>Details</summary>
Motivation: 离线强化学习中，分布偏移导致对分布外（OOD）动作的Q值高估，现有方法因过度约束限制了Q函数泛化，影响策略改进。

Method: 提出平滑贝尔曼算子（SBO），在CHN内通过平滑OOD Q值与邻近样本内Q值来更新Q值，理论证明其能近似真实Q值。
...
### [93] [Online Learning-guided Learning Rate Adaptation via Gradient Alignment](https://arxiv.org/abs/2506.08419)
*Ruichen Jiang,Ali Kavis,Aryan Mokhtari*

Main category: cs.LG

TL;DR: GALA框架通过动态调整学习率，基于梯度对齐和局部曲率估计，无需繁琐调参即可提升优化器性能。


<details>
  <summary>Details</summary>
Motivation: 大规模深度学习模型的学习率调优通常需要大量网格搜索，效率低下且耗时。

Method: 提出GALA框架，通过梯度对齐和局部曲率估计动态调整学习率，将其转化为一维在线学习问题。

Result: GALA在SGD和Adam等优化器中表现稳健，适应性强，无需调参即可竞争性能。

Conclusion: GALA为学习率动态调整提供了理论支持和实用方法，显著减少调参需求。
...
### [94] [HASFL: Heterogeneity-aware Split Federated Learning over Edge Computing Systems](https://arxiv.org/abs/2506.08426)
*Zheng Lin,Zhe Chen,Xianhao Chen,Wei Ni,Yue Gao*

Main category: cs.LG

TL;DR: HASFL框架通过自适应控制批次大小和模型分割，解决了分片联邦学习中因设备异构性导致的拖尾效应，显著提升了学习性能。


<details>
  <summary>Details</summary>
Motivation: 现有分片联邦学习方法因边缘设备能力异构性导致拖尾效应严重，亟需解决方案。

Method: 提出HASFL框架，通过收敛边界分析自适应控制批次大小和模型分割，平衡通信计算延迟与训练收敛。

Result: 实验验证HASFL在多种数据集上优于现有基准方法。

Conclusion: HASFL有效解决了设备异构性问题，提升了分片联邦学习的性能。
...
### [95] [Boosting Gradient Leakage Attacks: Data Reconstruction in Realistic FL Settings](https://arxiv.org/abs/2506.08435)
...
<details>
  <summary>Details</summary>
Motivation: 探讨联邦学习（FL）中梯度泄漏攻击的实际隐私风险，填补现有研究中对现实环境中攻击有效性的认知空白。
...
Result: TAWM在多种控制任务中表现优于传统模型，尤其在观测率变化时。
...
TL;DR: MOBODY是一种基于模型的离线强化学习算法，通过生成目标域的新合成数据来增强学习，解决了现有方法在目标域探索受限的问题。
...
Conclusion: MOBODY通过模型生成和潜在表示学习，有效解决了跨域动态不匹配问题，提升了离线强化学习的性能。
...
TL;DR: Reinforced RCSL通过引入“分布内最优未来回报”概念，解决了传统RCSL缺乏拼接能力的问题，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 传统RCSL因依赖离线数据集质量而性能受限，缺乏拼接能力。

Method: 提出Reinforced RCSL框架，利用策略识别当前状态下可实现的分布内最优未来回报。

Result: 理论分析和实验验证表明，Reinforced RCSL性能优于标准RCSL。

Conclusion: Reinforced RCSL是一种简单有效的方法，解决了RCSL的局限性。
...
Method: 分析Kronecker因子的特征谱，提出高效近似方法，并应用于注意力层的FIM。

Result: MAC在准确性、训练时间和内存使用上优于KFAC及其他先进方法。

Conclusion: MAC是一种高效且收敛性强的优化方法，适用于多种网络架构。
...
TL;DR: 论文提出AsFT方法，通过在微调中引入对齐方向作为锚点，抑制有害更新，提升模型安全性和性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在微调时易受恶意或无害数据影响，导致安全风险。研究旨在通过控制参数空间中的更新方向，保持模型安全性。

Method: 提出AsFT方法，在训练目标中加入正则化项，利用对齐方向作为锚点，抑制有害方向的更新。

Result: 实验表明，AsFT优于Safe LoRA，减少有害行为7.60%，提升性能3.44%，且在不同设置下表现稳健。

Conclusion: AsFT通过约束微调在安全区域内，有效提升模型安全性和性能，为安全微调提供了新思路。
...
TL;DR: 提出了一种基于热力学的潜在空间动力学识别框架（tLaSDI），用于参数化非线性动力系统的降阶建模，结合自编码器和参数化GENERIC形式神经网络，显著提升效率和精度。


<details>
  <summary>Details</summary>
Motivation: 解决参数化非线性动力系统建模中效率和热力学原理保持的问题。

Method: 结合自编码器降维和参数化GENERIC形式神经网络（pGFINNs），并采用物理信息主动学习策略。

Result: 在Burgers方程和1D/1V Vlasov-Poisson方程上实现最高3,528倍加速，误差1-3%，训练和推理成本显著降低。

Conclusion: tLaSDI框架高效且精确，同时揭示了系统的热力学行为。
...
TL;DR: 提出了一种基于抽象-精炼技术的高效方法，用于计算神经网络预测的可证明充分解释，显著提升了验证效率。


<details>
  <summary>Details</summary>
Motivation: 当前的后验解释方法缺乏形式化保证且计算效率低，需要一种可扩展的技术来提供可证明的解释。

Method: 通过构建一个大幅简化的网络抽象原始网络，逐步精炼网络规模直至收敛，以高效计算充分解释。
...
TL;DR: 论文提出SHAMs和DiffGradCAM，分别用于测试CAM方法的鲁棒性和改进其对抗性脆弱性。


<details>
  <summary>Details</summary>
Motivation: 现有CAM方法仅关注单个logits，而softmax输出的概率依赖于logits间的差异，导致CAM易受对抗性攻击（如被动欺骗）。

Method: 提出SHAMs作为对抗性测试基准，并设计DiffGradCAM，一种轻量级且对比性的CAM方法，以抵御被动欺骗。
...
TL;DR: ML4CFD竞赛通过评估机器学习模型在空气动力学模拟中的表现，展示了ML替代传统求解器的潜力。


<details>
  <summary>Details</summary>
Motivation: 机器学习在物理科学中的应用面临准确性、泛化性和物理一致性的挑战，ML4CFD竞赛旨在系统评估和推动进展。

Method: 竞赛组织了240多支团队，使用OpenFOAM生成的数据集，通过多标准框架评估模型。

Result: 最佳模型在综合指标上超越了传统OpenFOAM求解器，证明了ML替代方案的潜力。

Conclusion: 竞赛结果为未来科学ML挑战提供了设计原则和评估框架的指导。
...
TL;DR: 论文探讨了在非常大学习率下梯度下降（GD）的动态特性，发现学习率在某一区域时，GD会从纯利用型转变为探索-利用平衡型，且训练时间最短。


<details>
  <summary>Details</summary>
Motivation: 研究传统神经网络优化算法（如GD）在异常大学习率下的动态行为，揭示其探索-利用平衡特性。

Method: 通过分析GD轨迹的最大Lyapunov指数，表征其对初始条件的敏感依赖性，并在MNIST分类任务中验证。

Result: 发现学习率在特定区域时，GD表现出混沌特性，且训练时间最短，适用于多种学习任务和架构。

Conclusion: 研究表明，瞬态混沌动态在神经网络训练中具有建设性作用，可通过定位混沌起点加速训练。
...
TL;DR: EMNAS利用遗传算法自动优化自动驾驶强化学习的神经网络架构，提升奖励并减小模型尺寸，同时通过并行化和师生方法加速搜索和优化。实验表明其优于手动设计模型。


<details>
  <summary>Details</summary>
Motivation: 为大规模自动驾驶强化学习优化神经网络架构，提升性能并减小模型规模。

Method: 使用遗传算法自动化网络设计，结合并行化和师生方法加速搜索和优化。

Result: 实验显示EMNAS在奖励和参数效率上优于手动设计模型。
...
TL;DR: DeepForm是首个专为通信系统设计优化的LLM，通过两阶段训练和专用数据集CSFRC，显著优于通用LLM。
...
Conclusion: DeepForm为通信系统设计提供了高效解决方案，相关资源将开源以促进研究。
...
TL;DR: 研究发现，尽管LLM的激活模式可用于区分答案正确性，但这种“真理几何”具有任务依赖性，无法跨任务迁移。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM激活模式在跨任务中的可靠性问题，揭示现有方法的局限性。

Method: 通过线性分类器分析不同任务中的激活模式，并测试稀疏正则化及混合探针方法。

Result: 发现激活模式在跨任务中差异显著，分类器支持集几乎不重叠，复杂方法也无法解决。

Conclusion: LLM的“真理几何”具有任务依赖性，跨任务可靠性评估仍需新方法。
...
TL;DR: SLEEPYLAND是一个开源睡眠分期评估框架，旨在解决模型评估、泛化、偏见和标注变异性问题。SOMNUS作为集成模型，在24个数据集中表现优于单个模型和现有方法，甚至超越人类评分者。


<details>
  <summary>Details</summary>
Motivation: 临床采用深度学习进行睡眠分期的障碍包括模型评估不公、泛化能力差、偏见和标注变异性。

Method: SLEEPYLAND提供大量ID和OOD睡眠数据，并发布预训练模型。SOMNUS通过软投票集成不同架构和通道设置的模型。

Result: SOMNUS在24个数据集中表现稳健，宏F1分数68.7%-87.2%，超越人类评分者，并量化了模型偏见。

Conclusion: 集成模型提升鲁棒性，但无法完全消除偏见；SOMNUS在不确定区域预测人类分歧，提供数据驱动的代理。
...
TL;DR: 提出一种基于生成式AI的深度学习方法，用于提升污水系统上下文预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 通过捕捉多元时间序列数据中的复杂相关性，提高极端天气条件下的预测可靠性。

Method: 开发扩散模型处理时间序列数据，并结合保形推理技术校准预测区间。

Result: 实证测试显示模型在极端天气下仍能保持高准确性和可靠性。

Conclusion: 该方法显著提升了污水系统预测的鲁棒性和统计可靠性。
...
TL;DR: Transformer模型通过深度学习学习符号计算，CALT库为非专家提供训练支持。
...
Result: 成功实现符号计算的端到端学习，提供用户友好工具。
...
TL;DR: 提出了一种名为PBFM的新生成框架，显式嵌入物理约束到流匹配目标中，显著提高了物理残差精度。
...
Method: PBFM结合流匹配损失和物理残差损失，无需超参数调优，并引入时间展开训练策略。

Result: 在三个PDE问题上，PBFM的物理残差精度比FM高8倍，且分布准确性优于现有算法。
...
TL;DR: 论文提出了一种名为CASE的高效示例选择方法，通过将示例选择建模为多臂老虎机问题，显著减少了LLM的调用次数和运行时间。


<details>
  <summary>Details</summary>
Motivation: 在上下文学习范式中，示例选择对构建高效提示至关重要，但现有方法计算成本高。

Method: 将示例选择建模为多臂老虎机问题，提出CASE策略，通过维护挑战者臂列表减少样本复杂度。
...
<details>
  <summary>Details</summary>
Motivation: 传统图基准假设边是非空间且简单的，忽略了物理路径的多样性。HSG-12M填补了这一空白。

Method: 通过Poly2Graph管道，将1-D晶体的哈密顿量映射为谱图，生成多样化的物理拓扑。

Result: HSG-12M包含1160万静态和510万动态谱图，展示了多边几何学习的新挑战。

Conclusion: HSG-12M为几何感知图学习奠定了基础，并在凝聚态物理等领域开辟了新机会。
...
TL;DR: TiViT将时间序列转换为图像，利用预训练的视觉Transformer（ViT）提升时间序列分类性能，达到SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 解决时间序列分类中公开数据集稀缺的问题，探索视觉表示在非视觉领域的重用。

Method: 通过将时间序列转换为图像，利用预训练的ViT和OpenCLIP模型的隐藏表示进行分类。

Result: TiViT在标准时间序列分类基准上表现优异，中间层高维表示效果最佳。

Conclusion: TiViT展示了视觉表示在非视觉领域的潜力，与TSFM表示空间互补，结合后性能进一步提升。
...
TL;DR: DICE框架用于解决策略诱导的稳态分布与目标分布不匹配问题，但现有方法在提升离线RL性能时削弱了其OPE能力。本文通过半梯度优化解决了这一问题。


<details>
  <summary>Details</summary>
Motivation: 解决DICE框架在离线强化学习中因半梯度优化导致的成本估计失败问题。

Method: 提出一种新方法，通过半梯度DICE实现OPE和约束RL，确保成本估计准确性。

Result: 在离线约束RL基准DSRL上达到最优性能。

Conclusion: 新方法成功解决了DICE框架的局限性，实现了准确的成本估计和高效约束RL。
...
TL;DR: RP-KrossFuse方法通过随机投影和Kronecker乘积融合跨模态和单模态嵌入，在保持跨模态对齐的同时提升单模态任务性能。


<details>
  <summary>Details</summary>
Motivation: 跨模态嵌入（如CLIP）在跨模态对齐上表现良好，但在单模态任务上不如专用嵌入；而单模态嵌入缺乏跨模态对齐能力。本文旨在统一两者优势。

Method: 提出RP-KrossFuse，利用随机投影的Kronecker乘积融合跨模态和单模态嵌入，通过核空间高效操作实现可扩展性。

Result: 实验表明，RP-KrossFuse在单模态任务上表现优异，同时保持跨模态对齐能力。

Conclusion: RP-KrossFuse成功弥合了跨模态与单模态嵌入之间的差距，兼具两者优势。
...
Result: 在Tiny Shakespeare任务中，JoFormer的困惑度更低，收敛更快。

Conclusion: JoFormer为Transformer中位置信息的融入提供了原则性方法，展示了其潜力。
...
TL;DR: 论文发现简单k-NN方法在网络流量分类中表现优异，原因是数据集冗余和常见划分实践导致性能高估，并提出新方向以改进任务设计和评估。


<details>
  <summary>Details</summary>
Motivation: 研究复杂神经网络在网络流量分类中表现优异的现象，并揭示简单k-NN方法表现良好的原因。

Method: 评估k-NN基线在12个数据集和15个分类任务中的表现，分析数据集冗余和划分实践的影响。

Result: 发现大多数数据集包含超过50%冗余样本，导致性能高估和理论精度下降。
...
TL;DR: 论文提出了一种名为ChannelTokenFormer的Transformer模型，用于解决多变量时间序列数据中的通道依赖性、异步采样和缺失值问题，实验证明其在真实场景中具有优越性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的多变量时间序列数据通常存在复杂的通道依赖关系、异步采样和缺失值问题，而现有模型基于过于简化的假设，无法有效处理这些挑战。

Method: 提出ChannelTokenFormer，一种基于Transformer的模型，能够显式捕捉跨通道交互、适应通道异步采样并有效处理缺失值。

Result: 在三个基准数据集和一个真实工业数据集上的实验表明，ChannelTokenFormer在复杂现实条件下具有更高的鲁棒性和准确性。

Conclusion: ChannelTokenFormer为解决多变量时间序列数据中的实际挑战提供了一种有效的解决方案，显著提升了预测性能。
...
TL;DR: 论文提出了一种针对变分自编码器图像压缩的额外微调训练步骤，以解决量化噪声建模不准确的问题，显著提升了编码效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在训练过程中难以准确建模量化噪声，导致网络性能次优，尤其是在熵约束量化器中。

Method: 在传统端到端训练后，对网络部分进行基于量化潜在表示的微调训练。

Result: 实验表明，该方法在均匀标量量化和熵约束量化中均能提升编码效率，Kodak和TecNick测试集的比特率节省分别达到1%-2%和2.2%。
...
TL;DR: 提出了一种通过LLM生成蓝图增强SLM推理能力的框架，并减少对提示变化的敏感性，无需增加模型规模或额外训练。
...
Conclusion: 该框架为资源受限环境提供了轻量级解决方案，无需额外训练或模型扩展。
...
TL;DR: 本文研究了共识聚类中的公平性问题，首次提出了一种常数因子近似算法，并探讨了如何最小化修改现有聚类以实现公平性。


<details>
  <summary>Details</summary>
Motivation: 共识聚类通常忽略公平性，本文旨在结合公平聚类理论，确保每个受保护组在聚类中得到比例代表。

Method: 提出了一种最优算法处理等比例组，以及近线性时间常数因子近似算法处理不等比例组，并证明了问题的NP难度。

Result: 提供了首个常数因子近似算法，并展示了如何高效实现公平聚类。

Conclusion: 本文的成果不仅解决了共识聚类的公平性问题，还可能对其他聚类问题的公平变体产生广泛影响。
...
TL;DR: 本文提出了一种基于重要性采样的新方法（IS-DAAs），用于缓解离线直接对齐算法（DAAs）中的过优化问题，并通过实验验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 直接对齐算法（如DPO）在将大语言模型与人类价值观对齐时容易出现过优化问题，导致性能下降。

Method: 提出IS-DAAs方法，通过重要性采样比率调整目标函数，并限制比率最大值以避免高方差问题。

Result: 实验表明，IS-DAAs能有效缓解过优化，尤其在低正则化强度下表现优于其他方法。

Conclusion: IS-DAAs是一种有效的解决方案，可提升DAAs的性能和稳定性。
...
TL;DR: 提出基于变分自编码器（VAE）的VAE-LF模型，用于高效表示和补全高维不完整（HDI）电力负荷监测（PLM）数据，提升电力负荷预测（PLF）性能。


<details>
  <summary>Details</summary>
Motivation: 智能电网中高维不完整（HDI）电力负荷监测数据对负荷预测模型性能提出挑战，需高效数据补全方法。

Method: 采用编码器-解码器结构的VAE-LF模型，将HDI PLM数据分向量输入，学习低维潜在表示并生成补全数据。

Result: 在UK-DALE数据集上，VAE-LF在5%和10%稀疏度测试中表现优于基准模型，RMSE和MAE显著更低，低稀疏度数据表现尤佳。

Conclusion: VAE-LF为智能电网电力负荷管理提供了高效数据补全解决方案。
...
TL;DR: 论文探讨了生成式AI（如大型语言模型）对能源和环境的影响，提出了一种基于现有基准的框架R-ICE，用于估算推理碳排放。
...
Method: 利用现有SOTA基准数据，开发框架R-ICE，估算提示级推理碳排放。
...
TL;DR: RRP是一种新的强化学习探索策略，通过向环境奖励添加零均值噪声增强策略多样性，兼容现有方法且计算开销低。


<details>
  <summary>Details</summary>
Motivation: 探索策略在强化学习中至关重要，现有方法如动作扰动存在局限性，RRP旨在通过奖励扰动提升探索效果。

Method: RRP通过向奖励信号添加零均值噪声，结合动作扰动方法（如ε-greedy），无需额外计算负担。

Result: 实验表明，RRP显著提升了PPO和SAC的性能，提高了样本效率并避免局部最优。

Conclusion: RRP为奖励塑形与噪声驱动探索建立了理论联系，展示了其互补潜力，是一种通用且高效的探索策略。
...
TL;DR: 提出一种多视图、多输出的GNN模型，结合政府检查数据和众包报告数据，预测城市事件的真实潜在状态，并在纽约市案例中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 解决政府检查数据稀疏和众包报告数据偏差的问题，提高城市事件预测的准确性。

Method: 采用多视图、多输出的GNN模型，结合政府检查数据和众包报告数据，预测潜在事件状态。

Result: 模型在真实和半合成数据上表现优于仅使用单一数据源的模型，尤其是在检查数据稀疏时。同时揭示了众包报告中的社会经济偏差。

Conclusion: 该模型为利用异构、稀疏和偏差数据进行潜在状态预测提供了广泛适用的方法。
...
TL;DR: 论文提出了一种适用于稀疏和非独立同分布权重的深度神经网络稳定性理论，扩展了初始化方案的理论基础。


<details>
  <summary>Details</summary>
Motivation: 解决深度神经网络中因梯度爆炸或消失导致的谱稳定性问题，尤其是针对稀疏性和非独立同分布权重的网络。

Method: 利用随机矩阵理论的最新进展，建立适用于稀疏和非独立同分布权重的网络稳定性定理。

Result: 为更广泛的网络模型提供了谱稳定性的严格保证，扩展了初始化方案的理论适用范围。

Conclusion: 该研究为现代神经网络中具有结构化和依赖性随机性的初始化方案提供了更广泛的理论支持。
...
TL;DR: 提出了一种设计模式，用于构建具有可证明抗提示注入攻击能力的AI代理。
...
Method: 提出了一套设计模式，并系统分析了其效用与安全性之间的权衡。

Result: 通过案例研究展示了这些设计模式的实用性。

Conclusion: 设计模式能有效提升AI代理对提示注入攻击的抵抗力。
...
TL;DR: 论文提出IMAGIC-500数据集，用于评估缺失数据填补方法，填补了社会经济数据领域公开基准的空白。


<details>
  <summary>Details</summary>
Motivation: 社会经济数据因隐私限制难以公开共享，导致缺失数据填补方法缺乏系统性评估基准。

Method: 利用世界银行的公开合成数据，构建IMAGIC-500数据集，评估多种填补方法在不同缺失机制和比例下的表现。

Result: 结果展示了统计、传统机器学习和深度学习方法在填补连续与分类变量时的优劣。

Conclusion: IMAGIC-500数据集和基准旨在促进稳健填补算法的开发，推动可重复的社会科学研究。
...
TL;DR: 论文提出Agile Reinforcement Learning (aRL)方法，通过智能探索和动作屏蔽优化边缘计算中软实时任务调度，提升收敛速度和命中率。


<details>
  <summary>Details</summary>
Motivation: 边缘计算环境中软实时应用任务调度复杂度高，传统启发式和元启发式算法难以适应动态环境，强化学习算法学习时间长，需改进。

Method: 提出aRL方法，结合智能探索和动作屏蔽，减少无关动作探索，提升RL-agent的适应性和收敛速度。

Result: 实验表明aRL比基线方法具有更高的命中率和更快的收敛速度。

Conclusion: aRL适合边缘计算中软实时任务调度，能有效应对动态环境和复杂任务需求。
...
TL;DR: 论文提出了一种基于结构引导的GNN（SG-GNN），通过构建具有更高标签同质性的新图结构，解决GNN在异质性数据上的性能问题。


<details>
  <summary>Details</summary>
Motivation: GNN通常假设同质性，但在异质性数据中表现不佳，需要改进。

Method: 通过链接具有相似结构属性的节点创建新图结构，并引入SG-GNN，自适应学习多图结构的贡献。

Result: 在多个异质性数据集上，SG-GNN实现了最优或极具竞争力的性能。

Conclusion: 利用结构信息引导GNN能显著提升性能，尤其在异质性数据中。
...
TL;DR: 论文探讨了数据填补技术在水分配网络监测中的应用，通过比较多种填补方法，发现其能显著提升水质数据的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 智能水表数据因技术问题存在缺失，影响操作决策和效率，需填补技术提升数据质量。

Method: 比较了k-最近邻、MissForest、Transformer和循环神经网络等多种填补方法。

Result: 有效的数据填补显著提升了水质数据的准确性和可靠性，支持漏损检测和预测性维护。

Conclusion: 填补技术能显著改善智能水表数据的实用性，为水网络管理提供更可靠支持。
...
TL;DR: InfoDPCCA是一种动态概率CCA框架，用于提取两个相关序列的共享潜在表示，同时平衡压缩与预测能力。


<details>
  <summary>Details</summary>
Motivation: 解决高维序列数据中提取有意义潜在表示的挑战，应用于自然科学与工程。

Method: 采用信息论目标提取共享潜在表示，学习序列特定信息，并引入两步训练方案和残差连接机制。

Result: 在合成和医学fMRI数据上表现优异。

Conclusion: InfoDPCCA是有效的表示学习工具，提升了可解释性和鲁棒性。
...
TL;DR: SeerAttention-R是一个稀疏注意力框架，专为推理模型的长解码设计，保留了自蒸馏门控机制，支持自回归解码，且无需修改预训练模型参数。


<details>
  <summary>Details</summary>
Motivation: 解决长解码推理模型中稀疏注意力的高效实现问题。

Method: 通过自蒸馏门控机制学习注意力稀疏性，移除查询池化以适应自回归解码。

Result: 在AIME基准测试中，仅用0.4B tokens训练，4K token预算下保持接近无损的推理精度，稀疏注意力块大小为64/128时，解码速度比FlashAttention-3快9倍。
...
### [138] [SwS: Self-aware Weakness-driven Problem Synthesis in Reinforcement Learning for LLM Reasoning](https://arxiv.org/abs/2506.08989)
*Xiao Liang,Zhong-Zhi Li,Yeyun Gong,Yang Wang,Hengyuan Zhang,Yelong Shen,Ying Nian Wu,Weizhu Chen*

Main category: cs.LG

TL;DR: 论文提出了一种自感知弱点驱动的问题合成框架（SwS），通过识别模型弱点并针对性生成问题，提升强化学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有问题集质量不足且合成策略效率低，限制了强化学习在复杂推理任务中的扩展性。

Method: 定义模型弱点为训练中持续失败的问题，提取核心概念并合成新问题以增强模型弱项。

Result: 在8个主流推理基准上，7B和32B模型平均性能分别提升10.0%和7.7%。

Conclusion: SwS框架通过自识别和解决弱点，无需外部知识蒸馏即可实现鲁棒泛化。
...
### [139] [Intention-Conditioned Flow Occupancy Models](https://arxiv.org/abs/2506.08902)
*Chongyi Zheng,Seohong Park,Sergey Levine,Benjamin Eysenbach*

Main category: cs.LG

TL;DR: 论文提出了一种基于意图条件流占用模型（InFOM）的强化学习预训练方法，通过预测未来状态分布提升样本效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习中预训练模型的长期依赖问题，并利用生成AI工具建模复杂分布。

Method: 使用流匹配构建概率模型预测未来状态分布，并引入潜在变量捕捉用户意图。

Result: 在36个状态基准和4个图像基准任务中，性能提升1.8倍中位数回报和36%成功率。

Conclusion: InFOM方法显著提升了强化学习的预训练效果，适用于多任务场景。
...
### [140] [e3: Learning to Explore Enables Extrapolation of Test-Time Compute for LLMs](https://arxiv.org/abs/2506.09026)
*Amrith Setlur,Matthew Y. R. Yang,Charlie Snell,Jeremy Greer,Ian Wu,Virginia Smith,Max Simchowitz,Aviral Kumar*

Main category: cs.LG

TL;DR: 论文提出了一种名为e3的配方，通过训练LLM进行上下文探索，以提升推理能力并实现计算预算外的性能扩展。


<details>
  <summary>Details</summary>
Motivation: 现有推理模型在计算预算外的性能扩展表现不佳，需要一种方法来实现更好的推理能力。

Method: 通过训练LLM进行上下文探索，结合三种关键要素：链式技能、利用错误轨迹的负梯度、以及任务难度与训练预算的课程设计。

Result: e3-1.7B模型在AIME'25和HMMT'25上表现最佳，并能扩展到2倍训练预算，显著提升推理性能。

Conclusion: e3配方通过上下文探索显著提升了LLM的推理能力和性能扩展性。
...
### [141] [Enhancing generalizability of model discovery across parameter space with multi-experiment equation learning (ME-EQL)](https://arxiv.org/abs/2506.08916)
...
Method: 提出OAT ME-EQL（逐个参数学习并插值）和ES ME-EQL（统一模型库）两种方法。

Result: 两种方法显著降低参数恢复误差，OAT ME-EQL在参数空间泛化性更优。

Conclusion: ME-EQL能提升复杂生物系统模型的泛化性和可解释性。
...
### [142] [Local MDI+: Local Feature Importances for Tree-Based Models](https://arxiv.org/abs/2506.08928)
*Zhongyuan Liang,Zachary T. Rewolinski,Abhineet Agarwal,Tiffany M. Tang,Bin Yu*

Main category: cs.LG

TL;DR: 论文提出了一种新的局部特征重要性方法LMDI+，扩展了全局MDI+框架，在解释树模型预测时优于现有方法LIME和TreeSHAP。


<details>
  <summary>Details</summary>
Motivation: 树模型在表格数据中表现优异，但现有局部特征重要性方法（如LIME和TreeSHAP）依赖近似和扰动，忽略了模型内部结构，导致不稳定。

Method: 提出LMDI+方法，将MDI+框架扩展到样本特定场景，利用决策树与线性模型在节点基础上的等价性。

Result: LMDI+在12个真实数据集上平均提升下游任务性能10%，且特征重要性排名更稳定。

Conclusion: LMDI+为局部可解释性提供了新工具，支持反事实识别和同质子群发现。
...
### [143] [BioLangFusion: Multimodal Fusion of DNA, mRNA, and Protein Language Models](https://arxiv.org/abs/2506.08936)
*Amina Mollaysa,Artem Moskale,Pushpak Pati,Tommaso Mansi,Mangal Prakash,Rui Liao*

Main category: cs.LG

TL;DR: BioLangFusion提出了一种简单方法，将预训练的DNA、mRNA和蛋白质语言模型整合为统一的分子表示，通过生物学意义的密码子对齐实现跨模态对应。
...
Method: 研究了三种标准融合技术：密码子级嵌入拼接、熵正则化注意力池化和跨模态多头注意力，无需额外预训练或修改基础模型。

Result: 在五个分子属性预测任务中，BioLangFusion优于单模态基线，表明简单融合预训练模型即可捕获互补的多组学信息。

Conclusion: BioLangFusion展示了预训练模型简单融合的有效性，为多组学信息整合提供了低开销解决方案。
...
### [144] [KARMA: A Multilevel Decomposition Hybrid Mamba Framework for Multivariate Long-Term Time Series Forecasting](https://arxiv.org/abs/2506.08939)
*Hang Ye,Gaoxiang Duan,Haoran Zeng,Yangxin Zhu,Lingxue Meng,Xiaoying Zheng,Yongxin Zhu*

Main category: cs.LG

TL;DR: 论文提出KARMA模型，通过自适应时间通道分解和混合频率-时间分解模块，高效处理多变量时间序列预测问题，显著优于现有方法。
...
Method: KARMA结合自适应时间通道分解（ATCD）和混合频率-时间分解（HFTD），并集成多尺度Mamba模块协同处理全局与局部信息。

Result: 在八个真实数据集上，KARMA在预测精度和计算效率上显著优于主流基线方法。

Conclusion: KARMA为复杂时间序列预测提供了一种高效且准确的解决方案。
...
### [145] [Towards Robust Deep Reinforcement Learning against Environmental State Perturbation](https://arxiv.org/abs/2506.08961)
*Chenxu Wang,Huaping Liu*

Main category: cs.LG

TL;DR: 论文研究了深度强化学习中环境状态扰动的对抗攻击与防御，提出了一种名为BAT的防御框架，显著提升了智能体的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有研究较少关注环境状态扰动，而这类扰动在实体场景中很常见，因此需要提升智能体在此类扰动下的鲁棒性。

Method: 首先提出一种非目标攻击方法作为校准对手，随后提出BAT框架，结合监督学习和对抗训练增强智能体鲁棒性。

Result: 实验证明主流智能体在环境状态扰动下易受攻击，而BAT框架能显著提升鲁棒性。

Conclusion: BAT框架能有效应对环境状态扰动，优于现有鲁棒强化学习算法。
...
### [146] [GFRIEND: Generative Few-shot Reward Inference through EfficieNt DPO](https://arxiv.org/abs/2506.08965)
*Yiyang Zhao,Huiyu Bai,Xuejiao Zhao*

Main category: cs.LG

TL;DR: 提出了一种数据增强与扩展框架，通过偏好细化和多级直接偏好优化（M-DPO），在少样本数据下训练高性能奖励模型，效果媲美大规模数据集训练。
...
Method: 采用偏好细化（Chain-of-Thought采样）和困惑度评分机制，结合M-DPO捕捉细粒度偏好差异。

Result: 显著提升数据效率和模型性能，少样本训练效果媲美大规模数据集。
...
### [147] [Tailored Architectures for Time Series Forecasting: Evaluating Deep Learning Models on Gaussian Process-Generated Data](https://arxiv.org/abs/2506.08977)
*Victoria Hankemeier,Malte Schilling*

Main category: cs.LG

TL;DR: 论文提出了一种新数据集和模型TimeFlex，旨在揭示时间序列特性与模型性能之间的关联。
...
Result: TimeFlex在多种时间序列条件下表现优于现有模型，揭示了模型性能与数据特性的关联。

Conclusion: 研究为时间序列预测提供了更系统的评估框架，并展示了模型设计对数据特性的适应性。
...
### [148] [Propositional Logic for Probing Generalization in Neural Networks](https://arxiv.org/abs/2506.08978)
*Anna Langedijk,Jaap Jumelet,Willem Zuidema*

Main category: cs.LG

TL;DR: 论文研究了三种神经网络架构（Transformers、GCNs和LSTMs）在命题逻辑任务中的泛化能力，发现其对未见过模式（尤其是涉及否定的组合）的泛化能力有限。
...
Method: 通过扩展数据集消除表面模式，测试三种架构在生成逻辑公式满足分配任务中的泛化能力。

Result: 所有模型在训练分布内表现良好，但对未见过模式（尤其是否定组合）的泛化能力较差，Transformers需引入结构偏置才能正确应用否定。

Conclusion: 标准架构在系统学习逻辑运算符表示方面存在局限性，需更强的归纳偏置以支持基于规则的稳健推理。
...
### [149] [On Finetuning Tabular Foundation Models](https://arxiv.org/abs/2506.08982)
*Ivan Rubachev,Akim Kotelnikov,Nikolay Kartashev*

Main category: cs.LG

TL;DR: TabPFNv2的微调策略研究表明，完全微调是最优方法，能提升性能并揭示其内部机制类似检索增强模型。


<details>
  <summary>Details</summary>
Motivation: 探索TabPFNv2模型的最佳微调方法及其内部机制变化，填补现有研究的空白。

Method: 系统评估多种微调策略，分析微调对模型内部机制的影响，类比检索增强模型。

Result: 完全微调在时间和效果上最优，性能提升显著，尤其在I.I.D.数据集上达到SOTA。

Conclusion: 完全微调是TabPFNv2的最佳实践，但其在复杂数据集上的稳定性仍需改进。
...
### [150] [Branched Schrödinger Bridge Matching](https://arxiv.org/abs/2506.09007)
*Sophia Tang,Yinuo Zhang,Alexander Tong,Pranam Chatterjee*

Main category: cs.LG

TL;DR: 论文提出了一种新框架BranchSBM，用于学习分支的薛定谔桥，解决了现有方法无法捕捉多路径演化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如流匹配和薛定谔桥匹配）只能建模单一路径，无法捕捉从共同起源到多个不同结果的分支演化。

Method: 引入BranchSBM框架，参数化多个时间依赖的速度场和增长过程，以表示种群水平的分支演化。

Result: BranchSBM不仅更具表达能力，还能应用于多路径导航、细胞命运分叉建模和细胞响应模拟等任务。

Conclusion: BranchSBM为多路径演化问题提供了有效的解决方案，扩展了生成建模的应用范围。
...
### [151] [Effective Data Pruning through Score Extrapolation](https://arxiv.org/abs/2506.09010)
*Sebastian Schmidt,Prasanga Dhungel,Christoffer Löffler,Björn Nieth,Stephan Günnemann,Leo Schwinn*

Main category: cs.LG

TL;DR: 提出了一种新的重要性分数外推框架，仅需少量数据训练即可预测样本重要性，解决了传统数据剪枝技术需完整训练的局限性。


<details>
  <summary>Details</summary>
Motivation: 大规模数据集训练机器学习模型计算成本高，现有剪枝技术需完整训练才能识别冗余样本，效率低。

Method: 引入重要性分数外推框架，基于少量数据训练，使用k近邻和图神经网络预测样本重要性。

Result: 在2种剪枝方法、4种数据集和3种训练范式下验证了有效性，表明外推方法可扩展昂贵分数计算任务。
...
### [152] [SPEED-RL: Faster Training of Reasoning Models via Online Curriculum Learning](https://arxiv.org/abs/2506.09016)
*Ruiqi Zhang,Daman Arora,Song Mei,Andrea Zanette*

Main category: cs.LG

TL;DR: SPEED是一种自适应在线RL课程，通过选择中等难度的训练示例提高学习效率，实现2x到6x的训练加速。


<details>
  <summary>Details</summary>
Motivation: 传统RL训练中均匀采样提示效率低下，SPEED旨在通过选择性采样优化训练过程。

Method: SPEED选择性选择中等难度提示，理论上提升梯度估计器的信噪比，加速收敛。

Result: 实验表明SPEED实现2x到6x训练加速且不影响准确性，无需手动调参。

Conclusion: SPEED高效且易集成，显著提升RL训练效率。
...
### [153] [Edit Flows: Flow Matching with Edit Operations](https://arxiv.org/abs/2506.09018)
...
<details>
  <summary>Details</summary>
Motivation: 解决非自回归模型在生成变长序列时的局限性，使其更贴近序列数据的结构。

Method: 基于连续时间马尔可夫链定义编辑操作，利用扩展状态空间和辅助变量进行高效训练。

Result: 在图像描述生成任务中优于自回归和掩码模型，在文本和代码生成中显著优于掩码构造。

Conclusion: Edit Flows通过灵活的编辑操作实现了高效的序列生成，为非自回归模型提供了新的可能性。
...
TL;DR: FZOO是一种快速零阶优化器，通过减少前向传递次数和利用并行处理，显著提升了优化速度和内存效率，性能优于现有方法MeZO。
...
Result: FZOO在11个任务中平均准确率提升3%，前向传递次数减少3倍，RoBERTa-large任务中准确率提升5.6%，前向传递减少18倍。
...
TL;DR: 论文提出了一种可视化损失景观的方法，以补充理论分析，并扩展了Performative Prediction框架。


<details>
  <summary>Details</summary>
Motivation: 通过可视化损失景观，为理论分析提供实践洞察，并探索更现实的场景。

Method: 引入解耦风险可视化方法，分析模型参数和数据参数的风险景观；扩展Performative Prediction框架。

Result: 提出了新的兴趣点属性，验证了现有算法在更现实条件下的表现。

Conclusion: 可视化方法为Performative Prediction提供了实用工具，扩展框架更贴近现实场景。
...
<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统依赖静态配置，限制了灵活性和性能。ANN旨在通过神经符号方法实现动态协作优化。

Method: 采用两阶段优化策略：前向阶段动态分解任务并构建协作团队；后向阶段通过反馈优化全局和局部协作。

Result: 在四个基准数据集上，ANN优于现有多智能体基线，表现出更高的准确性和适应性。

Conclusion: ANN为多智能体系统提供了可扩展、数据驱动的框架，结合了LLMs的协作能力和神经网络的高效灵活性。
...
Method: 通过损失景观分析和实验验证任务向量的线性组合特性，并预测其在高秩映射中的失败。

Result: 任务向量在低秩任务中表现良好，但在高秩任务中失效，通过显著性分析和参数可视化验证了猜想。

Conclusion: 研究深化了对任务向量的理解，为基于Transformer的上下文学习机制提供了新见解。
...
### [158] [Conservative Bias in Large Language Models: Measuring Relation Predictions](https://arxiv.org/abs/2506.08120)
...
Method: 通过多提示、数据集和关系类型的系统评估，引入Hobson's choice概念，并使用SBERT和LLM提示量化保守偏差。

Result: 保守偏差比幻觉更常见（两倍频率），且保守行为在约束提示与半约束/开放式提示生成的标签间语义相似。

Conclusion: LLMs的保守偏差虽避免错误关系分配，但导致信息丢失，需在任务设计中权衡。
...
### [159] [QA-LIGN: Aligning LLMs through Constitutionally Decomposed QA](https://arxiv.org/abs/2506.08123)
*Jacob Dineen,Aswin RRV,Qin Liu,Zhikun Xu,Xiao Ye,Ming Shen,Zhaonan Li,Shijie Lu,Chitta Baral,Muhao Chen,Ben Zhou*

Main category: cs.CL

TL;DR: QA-LIGN是一种自动符号奖励分解方法，通过为每个原则制定评估问题并分离奖励组件，提高了语言模型对齐的透明度和适应性。


<details>
  <summary>Details</summary>
Motivation: 传统的基于奖励的对齐方法将多样反馈压缩为单一标量奖励，导致目标模糊且难以解释。

Method: QA-LIGN通过制定原则特定的评估问题，分解奖励为多个组件，替代传统黑盒奖励模型。

Result: 实验表明，QA-LIGN在透明度和适应性上优于传统方法，且性能与DPO基线相当或更好。

Conclusion: QA-LIGN为语言模型对齐提供了更可解释和可控的方法，同时不牺牲任务性能。
...
### [160] [EconWebArena: Benchmarking Autonomous Agents on Economic Tasks in Realistic Web Environments](https://arxiv.org/abs/2506.08136)
*Zefang Liu,Yinzhu Quan*

Main category: cs.CL

TL;DR: EconWebArena是一个用于评估自主代理在复杂、多模态经济任务中表现的基准测试，包含360个来自权威网站的任务，强调真实性和多模态推理。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试缺乏对真实经济任务和多模态推理的评估，EconWebArena填补了这一空白。

Method: 通过LLM生成候选任务，人工筛选确保清晰性和可行性，评估多模态LLM在导航、视觉理解和交互中的表现。

Result: 结果显示性能差距显著，尤其在多模态理解和导航方面存在挑战。

Conclusion: EconWebArena为经济智能代理提供了一个严格的测试平台。
...
### [161] [Multilingual Hate Speech Detection in Social Media Using Translation-Based Approaches with Large Language Models](https://arxiv.org/abs/2506.08147)
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体上的仇恨言论威胁在线安全与包容性，但乌尔都语等语言的仇恨言论检测研究较少，尤其是基于翻译的方法。

Method: 使用注意力层增强特征提取，结合TF-IDF和多种模型（如GPT-3.5 Turbo、Qwen 2.5 72B、SVM、BERT等）进行多语言仇恨言论检测。

Result: 模型在英语、西班牙语、乌尔都语及多语言联合任务中表现优异，F1分数显著提升（8.75%-8.97%）。
...
### [162] [ETT-CKGE: Efficient Task-driven Tokens for Continual Knowledge Graph Embedding](https://arxiv.org/abs/2506.08158)
*Lijing Zhu,Qizhen Lan,Qing Tian,Wenbo Sun,Li Yang,Lu Xia,Yixin Xie,Xi Xiao,Tiehang Duan,Cui Tao,Shuteng Niu*

Main category: cs.CL

TL;DR: ETT-CKGE提出了一种高效、任务驱动的持续知识图谱嵌入方法，通过可学习令牌简化知识转移，显著提升效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在效率和可扩展性上存在不足，主要由于手动设计的节点/关系重要性评分和计算成本高的图遍历。

Method: 引入任务驱动的可学习令牌，避免显式节点评分或遍历，通过矩阵操作实现高效知识转移。

Result: 在六个基准数据集上表现优异，显著提升了训练效率和可扩展性。

Conclusion: ETT-CKGE在性能和效率上均优于现有方法，为持续知识图谱嵌入提供了高效解决方案。
...
### [163] [Can Artificial Intelligence Write Like Borges? An Evaluation Protocol for Spanish Microfiction](https://arxiv.org/abs/2506.08172)
*Gerardo Aleman Manzanarez,Nora de la Cruz Arana,Jorge Garcia Flores,Yobany Garcia Medina,Raul Monroy,Nathalie Pernelle*

Main category: cs.CL

TL;DR: 论文提出GrAImes评估协议，基于文学理论评估AI生成微小说的文学价值，并通过专家和爱好者验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成故事缺乏对文学美学的严格评估，需建立客观评价框架。

Method: 提出GrAImes协议，结合文学理论，评估微小说的主题一致性、文本清晰度、阐释深度和美学质量。

Result: 验证了评估协议的有效性，得到文学专家和爱好者的认可。

Conclusion: GrAImes为AI生成微小说的文学价值评估提供了可靠基础。
...
### [164] [LLM-BT: Back-Translation as a Framework for Terminology Standardization and Dynamic Semantic Embedding](https://arxiv.org/abs/2506.08174)
*Li Weigang,Pedro Carvalho Brom*

Main category: cs.CL

TL;DR: LLM-BT利用大语言模型实现多语言术语标准化，通过回译验证术语一致性，提出多路径验证流程，并将回译视为动态语义嵌入。


<details>
  <summary>Details</summary>
Motivation: 传统专家驱动的标准化方法难以应对快速发展的技术领域（如AI和量子计算）中的术语增长，手动方法在多语言一致性上存在挑战。

Method: 提出LLM-BT框架，通过回译（如英语→中间语言→英语）验证术语一致性，采用“检索-生成-验证-优化”流程，支持串行和并行回译路径。

Result: 实验显示术语一致性超过90%，BLEU分数>0.45，葡萄牙语准确率达100%，验证了跨语言鲁棒性。

Conclusion: LLM-BT将回译转化为多语言术语标准化的主动引擎，实现人机协作，支持全球科技领域的术语治理。
...
### [165] [Unable to forget: Proactive lnterference Reveals Working Memory Limits in LLMs Beyond Context Length](https://arxiv.org/abs/2506.08184)
*Chupei Wang,Jiaqiu Vince Sun*

Main category: cs.CL

TL;DR: 研究发现LLMs在信息检索中存在干扰问题，导致准确性下降，提示需要改进模型抑制无关信息的能力。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在长上下文中的信息检索能力，尤其是干扰对检索的影响。

Method: 采用认知科学中的主动干扰范式（PI），设计PI-LLM评估方法，测试LLMs在语义相关更新流中的检索表现。

Result: LLMs检索准确性随干扰累积呈对数线性下降，提示检索能力受干扰限制。

Conclusion: LLMs存在工作记忆瓶颈，需开发新方法增强其抑制无关信息的能力。
...
### [166] ["I Wrote, I Paused, I Rewrote" Teaching LLMs to Read Between the Lines of Student Writing](https://arxiv.org/abs/2506.08221)
*Samra Zafar,Shaheer Minhas,Syed Ali Hassan Zaidi,Arfa Naeem,Zahra Ali*

Main category: cs.CL

TL;DR: 研究探讨了利用写作过程数据（如击键记录和定期快照）改进大型语言模型（LLM）对学生写作反馈的效果，发现学生更偏好基于过程的反馈。


<details>
  <summary>Details</summary>
Motivation: 当前LLM的反馈仅基于最终文本，缺乏写作过程的上下文，限制了反馈的相关性和实用性。

Method: 开发了一个数字写作工具，记录学生的写作过程，并比较了基于最终文本和完整写作轨迹的LLM反馈。

Result: 学生更偏好过程感知的反馈，且某些编辑行为（如添加内容或重组段落）与更高的写作分数相关。
...
### [167] [Compound AI Systems Optimization: A Survey of Methods, Challenges, and Future Directions](https://arxiv.org/abs/2506.08234)
*Yu-Ang Lee,Guan-Ting Yi,Mei-Yi Liu,Jui-Chao Lu,Guan-Bo Yang,Yun-Nung Chen*

Main category: cs.CL

TL;DR: 本文综述了复合AI系统优化的最新进展，涵盖数值和基于语言的技术，并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着复合AI系统复杂度的增加，优化组件及其交互面临新挑战，需要系统性的方法。

Method: 通过形式化复合AI系统优化的概念，分类现有方法，并分析自然语言反馈等新技术的潜力。

Result: 总结了数值和语言技术在优化复合AI系统中的应用，并指出开放研究问题。

Conclusion: 复合AI系统优化是一个快速发展的领域，未来需进一步探索新方法以应对复杂挑战。
...
### [168] [Can AI Validate Science? Benchmarking LLMs for Accurate Scientific Claim $\rightarrow$ Evidence Reasoning](https://arxiv.org/abs/2506.08235)
*Shashidhar Reddy Javaji,Yupeng Cao,Haohang Li,Yangyang Yu,Nikhil Muralidhar,Zining Zhu*

Main category: cs.CL

TL;DR: CLAIM-BENCH是一个评估大语言模型在科学论文中提取和验证声明与证据能力的基准，揭示了现有模型的局限性，并提出了改进方法。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在理解科学论文中声明与证据复杂关系的能力，填补研究空白。

Method: 通过CLAIM-BENCH基准，系统比较三种分治方法在六种大语言模型上的表现，评估其科学理解能力。

Result: 闭源模型（如GPT-4和Claude）在准确率和召回率上优于开源模型，三遍和逐一提示方法显著提升性能，但计算成本增加。

Conclusion: CLAIM-BENCH为评估大语言模型的科学理解能力设定了新标准，并提供了改进方向。
...
### [169] [Automatic Generation of Inference Making Questions for Reading Comprehension Assessments](https://arxiv.org/abs/2506.08260)
*Wanjing Anya Ma,Michael Flor,Zuowei Wang*

Main category: cs.CL

TL;DR: 论文探讨了阅读理解中推理技能的重要性，提出了一种推理类型分类法，并利用GPT-4o生成诊断性阅读理解题目，实验结果显示生成题目质量高但类型匹配度较低。


<details>
  <summary>Details</summary>
Motivation: 旨在通过自动生成诊断性阅读理解题目，为教育者提供更有效的阅读教学和干预工具。

Method: 提出推理类型分类法，利用GPT-4o通过少量示例生成题目，并比较有无思维链提示的效果。
...
### [170] [Institutional Books 1.0: A 242B token dataset from Harvard Library's collections, refined for accuracy and usability](https://arxiv.org/abs/2506.08300)
...
TL;DR: 报告介绍了Institutional Books 1.0，一个基于哈佛图书馆公共领域书籍的高质量数据集，旨在支持大语言模型的训练和推理。


<details>
  <summary>Details</summary>
Motivation: 解决高质量公开训练数据的稀缺问题，并确保数据管理的可持续性和可追溯性。

Method: 从哈佛图书馆的Google Books项目中提取、分析和处理公共领域书籍，生成包含OCR文本和元数据的数据集。

Result: 发布了983,004卷书籍的文本和元数据，总计242B tokens，覆盖多种语言。
...
### [171] [Wait, We Don't Need to "Wait"! Removing Thinking Tokens Improves Reasoning Efficiency](https://arxiv.org/abs/2506.08343)
*Chenlong Wang,Yuanning Feng,Dongping Chen,Zhaoyang Chu,Ranjay Krishna,Tianyi Zhou*

Main category: cs.CL

TL;DR: NoWait方法通过抑制显式自省标记（如“Wait”和“Hmm”）来减少推理过程中的冗余输出，显著缩短推理链长度，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理中常因过度思考产生冗余输出，影响效率。研究探讨显式自省是否必要。

Method: 提出NoWait方法，在推理过程中抑制显式自省标记，减少冗余输出。

Result: 在10个基准测试中，NoWait将推理链长度缩短27%-51%，且不影响模型性能。

Conclusion: NoWait为多模态推理提供了一种高效且性能无损的即插即用解决方案。
...
### [172] [Evaluating LLMs Across Multi-Cognitive Levels: From Medical Knowledge Mastery to Scenario-Based Problem Solving](https://arxiv.org/abs/2506.08349)
*Yuxuan Zhou,Xien Liu,Chenwei Yan,Chen Ning,Xiao Zhang,Boxun Li,Xiangling Fu,Shijin Wang,Guoping Hu,Yu Wang,Ji Wu*

Main category: cs.CL

TL;DR: 论文提出了一个基于Bloom分类法的多认知层次评估框架，用于评估大语言模型（LLMs）在医学领域的表现。研究发现，随着认知复杂度增加，模型性能显著下降，模型规模在高认知层次中起关键作用。
...
Method: 提出多认知层次评估框架，整合现有医学数据集，设计针对三个认知层次的任务，并系统评估六类主流LLMs。

Result: 模型性能随认知复杂度增加而显著下降，模型规模在高认知层次中更为重要。

Conclusion: 研究强调需提升LLMs在高认知层次的医学能力，为开发适合实际医学应用的LLMs提供参考。
...
### [173] [Text Embeddings Should Capture Implicit Semantics, Not Just Surface Meaning](https://arxiv.org/abs/2506.08354)
...
<details>
  <summary>Details</summary>
Motivation: 当前文本嵌入模型过于关注表层语义，忽视了语言中的隐式意义（如语用、意图和社会文化背景），导致其在需要深层语义理解的任务中表现不佳。

Method: 通过初步研究展示现有模型在隐式语义任务上的局限性，并提出改进建议。

Result: 研究发现，即使是先进模型在隐式语义任务上表现仅略优于简单基线。

Conclusion: 呼吁研究社区转向更丰富的训练数据、设计评估深层语义的基准，并将隐式语义作为核心目标。
...
### [174] [DEAL: Disentangling Transformer Head Activations for LLM Steering](https://arxiv.org/abs/2506.08359)
...
Result: 在7个LLM和5个行为数据集上验证，方法在真实性干预任务中表现优异，且所选注意力头具有跨领域泛化能力。

Conclusion: 提出的框架能更准确地识别行为相关模块，提升推理时干预效果。
...
### [175] [CC-RAG: Structured Multi-Hop Reasoning via Theme-Based Causal Graphs](https://arxiv.org/abs/2506.08364)
*Jash Rajesh Parekh,Pengcheng Jiang,Jiawei Han*

Main category: cs.CL

TL;DR: CC-RAG通过整合零样本三元组提取和主题感知图链，改进了RAG管道，提升了LLMs在专业领域中的因果推理能力。


<details>
  <summary>Details</summary>
Motivation: LLMs在专业领域的因果推理能力不足，标准RAG缺乏对因果依赖的结构化建模。

Method: CC-RAG利用零样本三元组提取构建DAG，并通过前向/后向链式推理生成结构化答案。

Result: 在比特币价格波动和高雪氏病领域，CC-RAG在链相似性、信息密度和词汇多样性上优于标准RAG和零样本LLMs。

Conclusion: 显式建模因果结构能提升LLMs在专业领域中的生成准确性和可解释性。
...
### [176] [Mitigating Posterior Salience Attenuation in Long-Context LLMs with Positional Contrastive Decoding](https://arxiv.org/abs/2506.08371)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在长上下文窗口中存在性能退化问题，现有解决方案训练成本高昂，统计行为和成本效益方法尚未充分探索。

Method: 提出PCD方法，通过对比长文本和局部注意力生成的logits，利用解码空间中的高排名黄金标记，缓解注意力分数退化。

Result: 实验表明，PCD在长上下文基准测试中达到了最先进的性能。

Conclusion: PCD是一种无需训练的有效方法，能显著改善LLMs在长文本中的表现。
...
### [177] [Draft-based Approximate Inference for LLMs](https://arxiv.org/abs/2506.08373)
*Kevin Galim,Ethan Ewer,Wonjun Kang,Minjae Lee,Hyung Il Koo,Kangwook Lee*

Main category: cs.CL

TL;DR: 提出了一种基于小模型预测的近似LLM推理框架，包括SpecKV和SpecPC两种方法，显著提升了长上下文LLM推理的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 由于Transformer的二次计算和线性内存复杂度，优化长上下文LLM推理变得至关重要。现有方法依赖粗略的重要性预测，准确性不足。

Method: 利用小模型预测token和KV对的重要性，提出SpecKV（优化KV缓存丢弃）和SpecPC（丢弃不重要的prompt token）。

Result: 实验表明，该方法在长上下文基准测试中比现有基线更准确，同时保持内存、延迟和吞吐量的改进。

Conclusion: 首次将小模型用于近似LLM推理加速，扩展了其应用范围，显著提升了效率和准确性。
...
### [178] [EIFBENCH: Extremely Complex Instruction Following Benchmark for Large Language Models](https://arxiv.org/abs/2506.08375)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试在单任务环境下缺乏复杂性，无法反映真实场景需求，因此需要更全面的评估工具。

Method: 设计了EIFBENCH基准，包含多任务场景和多种约束条件，并提出SegPO算法优化多任务工作流执行能力。

Result: 评估显示现有大语言模型在复杂指令下表现差异显著，表明仍需优化。

Conclusion: EIFBENCH和SegPO为提升大语言模型在复杂应用中的能力提供了有效工具。
...
### [179] [mSTEB: Massively Multilingual Evaluation of LLMs on Speech and Text Tasks](https://arxiv.org/abs/2506.08400)
...
TL;DR: 论文提出了mSTEB基准，用于评估LLMs在低资源语言上的多任务表现，发现高资源与低资源语言性能差距显著。
...
Method: 引入mSTEB基准，涵盖语言识别、文本分类、问答和翻译任务，评估了Gemini 2.0 Flash、GPT-4o等模型。

Result: 高资源与低资源语言性能差距显著，非洲及美洲/大洋洲语言表现尤其不足。

Conclusion: 需更多投入以解决低资源语言在LLMs中的代表性不足问题。
...
### [180] [TACTIC: Translation Agents with Cognitive-Theoretic Interactive Collaboration](https://arxiv.org/abs/2506.08403)
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）在机器翻译中取得了进展，但其潜力尚未完全发挥。现有方法忽视了认知翻译研究的见解，如人类翻译的认知策略。

Method: TACTIC框架包含六个智能体，分别负责起草、精炼、评估、评分、上下文推理和外部知识收集，模拟人类翻译的认知过程。

Result: 在FLORES-200和WMT24基准测试中，TACTIC表现优于GPT-4.1和DeepSeek-R1，XCOMET和COMETKIWI-23分数显著提升。

Conclusion: TACTIC通过认知理论指导的多智能体协作，有效提升了LLMs的翻译能力，实现了最先进的性能。
...
### [181] [Large Language Models Have Intrinsic Meta-Cognition, but Need a Good Lens](https://arxiv.org/abs/2506.08410)
*Ziyang Ma,Qingyue Yuan,Zhenglin Wang,Deyu Zhou*

Main category: cs.CL

TL;DR: 本文研究了LLM的元认知能力评估，提出AutoMeco框架和MIRA策略，以改进现有评估方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注LLM的认知错误检测，但缺乏对其元认知能力（如自我意识）的系统研究，这对LLM的可靠性至关重要。

Method: 提出AutoMeco框架评估现有元认知指标，并设计无训练的MIRA策略优化这些指标。

Result: 在三个数学推理数据集和三种LLM上的实验表明，AutoMeco优于Best-of-N验证，且MIRA能更准确评估LLM的元认知能力。

Conclusion: AutoMeco和MIRA为LLM元认知能力评估提供了有效工具，未来可进一步优化。
...
### [182] [Know-MRI: A Knowledge Mechanisms Revealer&Interpreter for Large Language Models](https://arxiv.org/abs/2506.08427)
...
TL;DR: 论文介绍了一个开源工具Know-MRI，用于系统分析大语言模型（LLMs）的知识机制，解决现有解释方法输入输出格式不统一的问题。


<details>
  <summary>Details</summary>
Motivation: 当前解释LLMs知识机制的方法输入输出格式不一致，工具支持有限，限制了实际应用。
...
### [183] [CAF-I: A Collaborative Multi-Agent Framework for Enhanced Irony Detection with Large Language Models](https://arxiv.org/abs/2506.08430)
*Ziqi. Liu,Ziyang. Zhou,Mingxuan. Hu*

Main category: cs.CL

TL;DR: CAF-I是一种基于LLM的多代理系统，通过多维分析和协作优化解决讽刺检测中的单视角限制、理解不足和可解释性差问题，实验表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法在讽刺检测中存在单视角限制、理解不足和可解释性差的问题，CAF-I旨在解决这些挑战。

Method: CAF-I采用多代理系统，包括Context、Semantics和Rhetoric代理进行多维分析，Decision Agent整合结果，Refinement Evaluator提供反馈优化。
...
### [184] [Low-resource domain adaptation while minimizing energy and hardware resource consumption](https://arxiv.org/abs/2506.08433)
*Hernán Maina,Nicolás Wolovick,Luciana Benotti*

Main category: cs.CL

TL;DR: 论文探讨了如何通过调整数值精度和数据并行化策略，降低大语言模型（LLM）领域适应的计算成本，以支持资源有限的研究环境。


<details>
  <summary>Details</summary>
Motivation: 大语言模型训练成本高昂，且往往偏向主流文化和价值观。领域适应虽能改善模型与多样文化背景的契合度，但其计算成本仍是障碍，尤其是对资源有限的研究团队。

Method: 评估不同数值精度和数据并行化策略对训练速度和模型准确性的影响。

Result: 研究结果为关注能源效率、可访问性或硬件限制的环境提供了实用指导。

Conclusion: 通过优化数值精度和并行化策略，可以有效降低领域适应的计算成本，促进资源有限环境下的模型适配。
...
### [185] [Olica: Efficient Structured Pruning of Large Language Models without Retraining](https://arxiv.org/abs/2506.08436)
*Jiujun He,Huazhen Lin*

Main category: cs.CL

TL;DR: Olica是一种无需重新训练的大型语言模型剪枝框架，通过正交分解和线性校准压缩模型，保持精度和结构。
...
### [186] [Detecting Harmful Memes with Decoupled Understanding and Guided CoT Reasoning](https://arxiv.org/abs/2506.08477)
*Fengjun Pan,Anh Tuan Luu,Xiaobao Wu*

Main category: cs.CL

TL;DR: U-CoT+ 是一种新型框架，通过将视觉模因转换为文本描述，结合人类指导的零样本链式思维提示，实现高效、灵活且可解释的有害模因检测。


<details>
  <summary>Details</summary>
Motivation: 当前有害模因检测方法在资源效率、灵活性和可解释性方面存在不足，限制了实际应用。

Method: 开发高保真模因转文本管道，结合人类指导的零样本链式思维提示，利用小型语言模型进行分类。

Result: 在七个基准数据集上的实验验证了框架的有效性，支持低资源、可解释的有害模因检测。

Conclusion: U-CoT+ 提供了一种灵活、可解释且资源高效的有害模因检测解决方案，适用于不同平台和需求。
...
### [187] [Efficient Context Selection for Long-Context QA: No Tuning, No Iteration, Just Adaptive-$k$](https://arxiv.org/abs/2506.08479)
*Chihiro Taguchi,Seiji Maekawa,Nikita Bhutani*

Main category: cs.CL

TL;DR: 论文提出了Adaptive-$k$检索方法，通过动态调整检索段落数量，优化开放域问答中的上下文限制问题，无需额外训练或修改现有流程。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在开放域问答中固定检索规模导致的效率低下或关键信息遗漏问题。

Method: 基于查询与候选段落相似度分数的分布，动态选择段落数量，无需额外推理或流程修改。

Result: 在事实性和聚合性问答基准测试中，Adaptive-$k$表现优于固定规模检索，同时减少90%的token使用，保留70%相关段落。

Conclusion: 动态调整上下文规模能显著提升问答系统的效率和准确性。
...
### [188] [Re-Thinking the Automatic Evaluation of Image-Text Alignment in Text-to-Image Models](https://arxiv.org/abs/2506.08480)
...
### [189] [Werewolf: A Straightforward Game Framework with TTS for Improved User Engagement](https://arxiv.org/abs/2506.00160)
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM推理和说服能力的提升，尤其是在DeepSeek R1和V3模型的推动下，基于LLM代理的社交推理游戏（如狼人杀）有望提供更吸引人的体验。

Method: 提出了一种新颖且直接的基于LLM的狼人杀系统，结合优化的TTS模型，以增强与不同LLM模型的兼容性和用户参与度。

Result: 系统通过优化的TTS模型提升了兼容性和用户体验，同时减少了对外部组件的依赖。

Conclusion: 随着LLM推理能力的持续增强，未来在狼人杀等游戏中，额外的组件可能变得多余。
...
### [190] [Fairness is Not Silence: Unmasking Vacuous Neutrality in Small Language Models](https://arxiv.org/abs/2506.08487)
*Sumanth Manduru,Carlotta Domeniconi*

Main category: cs.CL

TL;DR: 论文首次大规模审核了0.5至50亿参数的指令调优小型语言模型（SLMs），揭示了其公平性与性能的关系，并提供了部署建议。


<details>
  <summary>Details</summary>
Motivation: 小型语言模型（SLMs）在资源受限环境中的快速应用，但其伦理风险尚未被充分理解。

Method: 使用BBQ基准在零样本提示下评估了9个开源模型（Qwen 2.5、LLaMA 3.2、Gemma 3和Phi系列），分析其性能和公平性。

Result: 1. Phi模型在F1分数超过90%的同时表现出最小偏见；2. Qwen 2.5模型表面公平但实际为随机猜测或逃避行为；3. 4位AWQ量化对LLaMA 3.2-3B性能有提升，但增加了Phi-4-Mini的偏见。

Conclusion: SLMs的高效与伦理可以共存，但需注意架构和量化对公平性的影响，为资源受限环境提供实用指导。
...
### [191] [EtiCor++: Towards Understanding Etiquettical Bias in LLMs](https://arxiv.org/abs/2506.08488)
*Ashutosh Dwivedi,Siddhant Shivdutt Singh,Ashutosh Modi*

Main category: cs.CL

TL;DR: 论文介绍了EtiCor++语料库，用于评估LLMs对不同地区礼仪的理解和偏见，并提出了相关任务和指标。实验显示LLMs存在对某些地区的固有偏见。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs的文化敏感性，尤其是礼仪方面的理解和偏见，目前缺乏相关资源。

Method: 引入EtiCor++语料库，设计评估任务和偏见度量指标，并进行广泛实验。
...
<details>
  <summary>Details</summary>
Motivation: 现有KD方法在训练学生模型时容易导致分布偏移，引发灾难性遗忘、模式崩溃等问题，需一种更稳定的方法。

Method: POCL框架包含难度测量器和训练调度器，分阶段引入从易到难的样本，并逐步提高损失函数温度。

Result: 实验表明POCL能显著提升学生模型性能，适用于多种KD方法和模型家族。

Conclusion: POCL通过结构化训练数据，有效提升了KD的稳定性和性能，为LLM的压缩提供了新思路。
...
### [193] [Integration of Old and New Knowledge for Generalized Intent Discovery: A Consistency-driven Prototype-Prompting Framework](https://arxiv.org/abs/2506.08490)
*Xiao Wei,Xiaobao Wang,Ning Zhuang,Chenyang Wang,Longbiao Wang,Jianwu dang*

Main category: cs.CL

TL;DR: 论文提出了一种一致性驱动的原型提示框架（GID），用于整合新旧知识以解决广义意图发现问题，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅关注无监督数据聚类而忽视领域适应，限制了广义意图发现的实际应用。

Method: 提出原型提示框架和分层一致性约束，结合外部知识的旧知识和目标领域的新知识。
...
### [194] [DRAGged into Conflicts: Detecting and Addressing Conflicting Sources in Search-Augmented LLMs](https://arxiv.org/abs/2506.08500)
*Arie Cattan,Alon Jacovi,Ori Ram,Jonathan Herzig,Roee Aharoni,Sasha Goldshtein,Eran Ofek,Idan Szpektor,Avi Caciularu*

Main category: cs.CL

TL;DR: 论文提出了一种新的知识冲突分类法，并创建了CONFLICTS基准，用于评估大语言模型在RAG中处理冲突的能力。实验表明，模型在解决冲突时表现不佳，但通过提示显式推理可改善结果。


<details>
  <summary>Details</summary>
Motivation: 研究RAG中检索到的信息冲突问题，明确模型应如何处理这些冲突。

Method: 提出知识冲突分类法，构建CONFLICTS基准，并进行实验评估。

Result: 大语言模型在解决冲突时表现不佳，但显式推理提示能显著改善结果。
...
### [195] [CoMuMDR: Code-mixed Multi-modal Multi-domain corpus for Discourse paRsing in conversations](https://arxiv.org/abs/2506.08504)
*Divyaksh Shukla,Ritesh Baviskar,Dwijesh Gohil,Aniket Tiwari,Atul Shree,Ashutosh Modi*

Main category: cs.CL

TL;DR: 论文介绍了CoMuMDR语料库，用于多模态多领域代码混合对话的篇章解析，并测试了现有模型的表现。


<details>
  <summary>Details</summary>
Motivation: 现有篇章解析数据集局限于单一领域的英语对话，缺乏多模态和代码混合的真实场景数据。

Method: 构建了包含印地语和英语代码混合的音频和文本语料库，标注了九种篇章关系，并测试了多种先进模型。

Result: 现有模型在代码混合多领域语料上表现不佳，凸显了开发更强大模型的必要性。
...
### [196] [Efficient Post-Training Refinement of Latent Reasoning in Large Language Models](https://arxiv.org/abs/2506.08552)
...
<details>
  <summary>Details</summary>
Motivation: 现有显式推理方法（如Chain-of-Thought）存在固定推理轨迹和额外标记开销的问题，而潜在推理方法虽能避免这些问题，但缺乏有效的后训练更新机制。

Method: 提出两种策略：1) 对比推理反馈，通过比较强/弱基线推理嵌入优化更新方向；2) 残差嵌入细化，结合当前和历史梯度稳定更新。

Result: 在五个推理基准测试中验证有效性，MathQA上实现5%的准确率提升（无需额外训练）。

Conclusion: 该框架显著优化潜在推理过程，为后训练推理嵌入更新提供了高效解决方案。
...
### [197] [Neighbors and relatives: How do speech embeddings reflect linguistic connections across the world?](https://arxiv.org/abs/2506.08564)
*Tuukka Törö,Antti Suni,Juraj Šimko*

Main category: cs.CL

TL;DR: 该研究利用机器学习方法分析语音嵌入，探索106种世界语言之间的关系，发现其与传统语言距离测量方法一致，为大规模语言分析提供了新视角。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过机器学习方法补充传统语言关系分析方法，利用语音嵌入探索语言间的关联，以应对语言多样性和变化的挑战。

Method: 使用微调的XLS-R自监督语言识别模型生成语音嵌入，并通过线性判别分析（LDA）对语言进行聚类，与传统语言距离（如谱系、词汇和地理距离）进行比较。

Result: 语音嵌入的距离与传统测量方法高度一致，能够捕捉全局和局部语言类型模式，但也揭示了语言变化的动态性。

Conclusion: 该方法为基于语音嵌入的可扩展语言分析提供了潜力，未来可扩展至低资源语言，并整合社会语言学因素以更全面理解语言多样性。
...
### [198] [CounselBench: A Large-Scale Expert Evaluation and Adversarial Benchmark of Large Language Models in Mental Health Counseling](https://arxiv.org/abs/2506.08584)
*Yahan Li,Jifan Yao,John Bosco S. Bunyi,Adam C. Frank,Angel Hwang,Ruishan Liu*

Main category: cs.CL

TL;DR: CounselBench是一个用于评估大型语言模型（LLM）在心理健康咨询场景中的表现的基准测试，包含专家评估和对抗性测试，发现LLM在质量上优于人类治疗师，但存在安全隐患。
...
Method: 开发CounselBench基准测试，包括专家评估（CounselBench-EVAL）和对抗性测试（CounselBench-Adv），评估LLM在六个临床维度上的表现。

Result: LLM在感知质量上优于人类治疗师，但常被专家标记为存在安全隐患；LLM评委高估模型表现并忽视安全问题。

Conclusion: CounselBench为高风险的LLM心理健康应用提供了临床基准框架，揭示了模型的具体失败模式。
...
### [199] [Dense Retrievers Can Fail on Simple Queries: Revealing The Granularity Dilemma of Embeddings](https://arxiv.org/abs/2506.08592)
*Liyan Xu,Zhenlin Su,Mo Yu,Jiangnan Li,Fandong Meng,Jie Zhou*

Main category: cs.CL

TL;DR: 该论文研究了文本编码器在细粒度实体或事件识别上的局限性，提出了中文评估数据集CapRetrieval，并通过数据生成策略优化编码器性能。
...
Conclusion: 公开数据集和模型，为细粒度语义匹配研究提供资源。
...
### [200] [Hateful Person or Hateful Model? Investigating the Role of Personas in Hate Speech Detection by Large Language Models](https://arxiv.org/abs/2506.08593)
*Shuzhou Yuan,Ercong Nie,Mario Tawfelis,Helmut Schmid,Hinrich Schütze,Michael Färber*

Main category: cs.CL

TL;DR: 本文研究了MBTI人格特质对大型语言模型（LLMs）在仇恨言论检测中的影响，发现人格提示会导致显著的结果差异和不一致性。
...
Result: 研究发现人格提示导致显著的输出差异，包括与真实标签的不一致、人格间的分歧以及逻辑层面的偏见。

Conclusion: 研究强调在LLM标注流程中需谨慎定义人格提示，以确保公平性和与人类价值观的一致性。
...
### [201] [RAISE: Enhancing Scientific Reasoning in LLMs via Step-by-Step Retrieval](https://arxiv.org/abs/2506.08625)
...
Conclusion: RAISE通过逻辑相关文档检索，显著提升了科学推理的效果。
...
### [202] [MEMETRON: Metaheuristic Mechanisms for Test-time Response Optimization of Large Language Models](https://arxiv.org/abs/2506.08643)
*Son The Nguyen,Theja Tulabandhula*

Main category: cs.CL

TL;DR: MEMETRON框架通过黑盒优化方法改进LLM的解码策略，无需重新训练模型即可高效生成高奖励响应。


<details>
  <summary>Details</summary>
Motivation: 现有LLM解码策略（如贪心搜索或采样）缺乏对任务目标的明确优化，限制了控制能力。

Method: MEMETRON将解码问题转化为离散黑盒优化，结合元启发式算法（GENETRON和ANNETRON）和奖励模型指导搜索。

Result: 在人类偏好对齐任务中，MEMETRON显著优于标准解码和重排序方法。

Conclusion: MEMETRON为LLM解码提供了灵活且高效的优化框架，无需模型重新训练即可提升任务表现。
...
### [203] [TableDreamer: Progressive and Weakness-guided Data Synthesis from Scratch for Table Instruction Tuning](https://arxiv.org/abs/2506.08646)
*Mingyu Zheng,Zhifan Feng,Jia Wang,Lanrui Wang,Zheng Lin,Yang Hao,Weiping Wang*

Main category: cs.CL

TL;DR: 论文提出了一种名为TableDreamer的框架，通过渐进式和弱点引导的数据合成方法，解决了现有LLM在表格指令调优数据生成中的多样性和效率问题。
...
Method: TableDreamer首先生成多样化表格和指令作为种子数据，然后基于新发现的弱点数据迭代探索输入空间，最终生成训练数据。

Result: 在10个表格基准测试中，TableDreamer显著提升了目标LLM的性能，平均准确率提高了11.62%，优于现有方法。

Conclusion: TableDreamer通过弱点引导的数据合成，有效提升了表格指令调优的数据多样性和效率，为LLM的表格理解能力提供了显著改进。
...
### [204] [Summarization for Generative Relation Extraction in the Microbiome Domain](https://arxiv.org/abs/2506.08647)
...
Result: 摘要生成能提升生成式关系提取性能，但BERT方法仍表现更好。
...
### [205] [RuleReasoner: Reinforced Rule-based Reasoning via Domain-aware Dynamic Sampling](https://arxiv.org/abs/2506.08672)
...
Result: 在ID和OOD任务中显著优于前沿大型推理模型，并具有更高的计算效率。

Conclusion: RuleReasoner是一种简单有效的方法，能够提升SRMs的推理能力和泛化性能。
...
### [206] [Brevity is the soul of sustainability: Characterizing LLM response lengths](https://arxiv.org/abs/2506.08686)
*Soham Poddar,Paramita Koley,Janardan Misra,Sanjay Podder,Navveen Balani,Niloy Ganguly,Saptarshi Ghosh*

Main category: cs.CL

TL;DR: 论文研究了大型语言模型（LLM）推理过程中的能源消耗问题，提出通过输出压缩和提示工程策略显著减少响应长度，实现25-60%的能源优化。


<details>
  <summary>Details</summary>
Motivation: LLM推理过程消耗大量能源，而输出压缩领域研究较少，因此探索高效节能的推理方法至关重要。

Method: 对12个仅解码器LLM在5个数据集上进行基准测试，定义LLM响应中的六类信息，并通过提示工程策略减少冗余信息。

Result: 适当的提示策略可减少响应长度25-60%，同时保持响应质量。

Conclusion: 通过简单的提示工程策略，可以有效优化LLM的能源消耗，减少冗余输出。
...
### [207] [ClimateViz: A Benchmark for Statistical Reasoning and Fact Verification on Scientific Charts](https://arxiv.org/abs/2506.08700)
*Ruiran Su,Jiasheng Si,Zhijiang Guo,Janet B. Pierrehumbert*

Main category: cs.CL

TL;DR: ClimateViz是首个基于科学图表的大规模事实核查基准，包含49,862个与2,896个可视化图表关联的声明。评估显示当前多模态模型在图表推理上表现不佳，最佳模型准确率仅76.2-77.8%，远低于人类水平。


<details>
  <summary>Details</summary>
Motivation: 科学事实核查主要关注文本和表格，忽略了科学图表的重要性。ClimateViz旨在填补这一空白，提供图表驱动的科学事实核查基准。

Method: 构建ClimateViz数据集，包含专家标注的图表和声明，评估多模态语言模型在零样本和少样本设置下的表现。

Result: 当前模型在图表推理上表现较差，最佳模型准确率低于人类水平（89.3-92.7%）。解释增强输出对部分模型有提升。

Conclusion: ClimateViz为科学图表事实核查提供了新基准，揭示了当前模型的局限性，并推动了多模态推理的研究。
...
### [208] [ConfPO: Exploiting Policy Model Confidence for Critical Token Selection in Large Language Model Preference Optimization](https://arxiv.org/abs/2506.08712)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如DPO）对所有token进行均匀调整，而ConfPO旨在通过优化关键token提高对齐质量并避免过优化。

Method: ConfPO仅依赖训练策略的置信度，识别并优化偏好关键token，无需辅助模型或额外计算。

Result: 实验表明，ConfPO在多个基准测试中优于均匀调整方法，且无额外计算开销。

Conclusion: ConfPO是一种简单、轻量且高效的对齐方法，优于现有技术。
...
### [209] [Explainable Compliance Detection with Multi-Hop Natural Language Inference on Assurance Case Structure](https://arxiv.org/abs/2506.08713)
*Fariz Ikhwantri,Dusica Marijan*

Main category: cs.CL

TL;DR: 论文提出了一种基于自然语言推理（NLI）的合规检测方法EXCLAIM，通过多跳推理实现可解释的合规检测，并利用大语言模型生成保证案例。


<details>
  <summary>Details</summary>
Motivation: 解决复杂系统中合规检测的挑战，如法律和技术文本的复杂性、模型解释需求以及保证案例数据有限的问题。

Method: 将保证案例的声明-论据-证据结构建模为多跳推理，利用大语言模型生成保证案例，并引入覆盖率和结构一致性的度量指标。

Result: 通过GDPR要求的案例研究，展示了生成保证案例在多跳推理任务中的有效性。

Conclusion: 基于NLI的方法在自动化合规检测过程中具有潜力。
...
### [210] [Multi-Teacher Language-Aware Knowledge Distillation for Multilingual Speech Emotion Recognition](https://arxiv.org/abs/2506.08717)
...
Method: 利用Wav2Vec2.0构建单语言教师模型，通过语言感知的多教师知识蒸馏方法将知识迁移到多语言学生模型。

Result: 学生模型表现优异，英语加权召回率达72.9，芬兰语未加权召回率达63.4，优于微调和知识蒸馏基线。

Conclusion: 方法在提升悲伤和中性情绪识别上效果显著，但愤怒和快乐情绪识别仍有挑战。
...
### [211] [Improved LLM Agents for Financial Document Question Answering](https://arxiv.org/abs/2506.08726)
*Nelvin Tan,Zian Seng,Liang Zhang,Yu-Ching Shih,Dong Yang,Amol Salunkhe*

Main category: cs.CL

TL;DR: 本文研究了传统批评代理在无标签情况下的性能下降问题，并提出改进的批评代理和计算器代理，性能优于现有方法且更安全。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在金融文档数值问答任务中表现不佳，传统批评代理依赖标签，但现实中标签常不可用。

Method: 提出改进的批评代理和计算器代理，并研究其交互对性能的影响。

Result: 改进代理优于现有方法（程序思维），且更安全。

Conclusion: 新代理在无标签情况下表现更优，交互研究为其性能提升提供见解。
...
### [212] [Societal AI Research Has Become Less Interdisciplinary](https://arxiv.org/abs/2506.08738)
*Dror Kris Markus,Fabrizio Gilardi,Daria Stetsenko*

Main category: cs.CL

TL;DR: 研究分析了2014-2024年间10万篇AI论文，发现跨学科团队虽更关注社会伦理，但纯计算机科学团队的社会导向研究占比增长显著。


<details>
  <summary>Details</summary>
Motivation: 探讨AI研究如何融入伦理和社会价值，以及跨学科合作的实际作用。

Method: 开发分类器识别论文中的社会内容，分析团队构成与社会导向研究的关联。

Result: 计算机科学团队在社会导向研究中的占比增加，覆盖公平、安全、医疗等多个领域。

Conclusion: 挑战了跨学科团队主导社会导向研究的假设，引发对AI安全和治理的思考。
...
### [213] [Towards Secure and Private Language Models for Nuclear Power Plants](https://arxiv.org/abs/2506.08746)
*Muhammad Anwar,Mishca de Costa,Issam Hammad,Daniel Lau*

Main category: cs.CL

TL;DR: 本文介绍了一种针对核应用领域的特定大型语言模型，基于公开的Essential CANDU教材构建，采用紧凑的Transformer架构，单GPU训练以保护敏感数据。模型在专业词汇捕捉方面表现良好，但生成文本有时缺乏句法连贯性。


<details>
  <summary>Details</summary>
Motivation: 解决核领域数据敏感性问题，同时探索在有限数据集上构建专业领域语言模型的可行性。

Method: 基于Transformer架构，使用单GPU训练，数据来源于Essential CANDU教材。

Result: 模型能够捕捉专业核词汇，但生成文本的句法连贯性有待提升。

Conclusion: 证明了在核领域构建内部LLM的可行性，未来需扩展数据集、优化预处理和指令微调以提升准确性。
...
### [214] [Unlocking the Potential of Large Language Models in the Nuclear Industry with Synthetic Data](https://arxiv.org/abs/2506.08750)
*Muhammad Anwar,Daniel Lau,Mishca de Costa,Issam Hammad*

Main category: cs.CL

TL;DR: 论文探讨如何通过合成数据生成解决核工业中非结构化文本数据的可用性问题，以支持大型语言模型的开发和应用。
...
Conclusion: 合成数据为核工业中的信息检索、知识共享和决策支持提供了新的可能性。
...
### [215] [Factors affecting the in-context learning abilities of LLMs for dialogue state tracking](https://arxiv.org/abs/2506.08753)
...
Method: 使用基于句子嵌入的k近邻方法检索演示，结合模板输入到LLM中，系统分析影响因素。

Result: 在MultiWoZ2.4数据集上实验，发现演示选择和提示上下文对DST性能有显著影响。
...
### [216] [Enhancing Accuracy and Maintainability in Nuclear Plant Data Retrieval: A Function-Calling LLM Approach Over NL-to-SQL](https://arxiv.org/abs/2506.08757)
...
Method: 通过预定义一组经过审核的专用函数，封装已验证的SQL逻辑，避免直接生成SQL查询，确保安全性和准确性。

Result: 研究表明，函数调用方法在准确性和可维护性上优于直接NL-to-SQL生成。

Conclusion: 该框架平衡了用户易用性与操作安全性，为关键系统提供了可靠的数据检索解决方案。
...
### [217] [AraReasoner: Evaluating Reasoning-Based LLMs for Arabic NLP](https://arxiv.org/abs/2506.08768)
*Ahmed Hasanaath,Aisha Alansari,Ahmed Ashraf,Chafik Salmane,Hamzah Luqman,Saad Ezzini*

Main category: cs.CL

TL;DR: 论文研究了大型语言模型（LLMs）在阿拉伯语数据上的表现，重点评估了DeepSeek模型在15个阿拉伯语NLP任务中的性能，发现少量示例选择和微调策略显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语因其丰富的形态、多样的方言和复杂文字，LLMs的表现尚未充分探索，本文旨在填补这一空白。

Method: 采用零样本、少量样本和微调策略，系统评估LLMs在阿拉伯语任务中的表现，特别关注DeepSeek模型。

Result: 少量示例选择显著提升分类任务性能；DeepSeek在零样本设置下优于GPT；LoRA微调进一步提升了模型表现。

Conclusion: 研究证实了LLMs在阿拉伯语任务中的潜力，并提出了有效的性能优化策略。
...
### [218] [The impact of fine tuning in LLaMA on hallucinations for named entity extraction in legal documentation](https://arxiv.org/abs/2506.08827)
*Francisco Vargas,Alejandro González Coene,Gaston Escalante,Exequiel Lobón,Manuel Pulido*

Main category: cs.CL

TL;DR: 论文提出了一种从法律文档中提取交通事故信息的两步法，通过文本分割和实体提取，比较了不同方法的效果，发现基于向量化和LLMs的方法优于传统方法，其中GPT-4 Turbo表现最佳。


<details>
  <summary>Details</summary>
Motivation: 从法律文档中提取交通事故信息（如伤残百分比和赔偿金额）对保险公司成本量化至关重要，但由于文档复杂性，传统方法效果有限。

Method: 采用两步法：1) 文本分割（正则表达式或n-token块向量化）；2) 对选定段落使用LLMs（LLaMA-2/3、GPT-4 Turbo）进行实体提取，部分模型通过LoRA微调。

Result: 基于向量化和LLMs的方法显著优于传统方法（39.5%准确率）。微调后的LLaMA-2 70B达到79.4%，LLaMA-3 8B基础模型为76.6%，GPT-4 Turbo表现最佳（86.1%）。

Conclusion: LLMs结合向量化方法在实体提取任务中表现优异，微调和模型进步显著提升性能，GPT-4 Turbo为当前最优解。
...
### [219] [Advancing STT for Low-Resource Real-World Speech](https://arxiv.org/abs/2506.08836)
*Flavio D'Intino,Hans-Peter Hutter*

Main category: cs.CL

TL;DR: 论文介绍了SRB-300数据集，用于提升瑞士德语（低资源语言）的语音识别性能，通过微调Whisper模型显著降低了错误率并提高了翻译质量。


<details>
  <summary>Details</summary>
Motivation: 瑞士德语缺乏标准化书写形式且方言多样，现有语音识别模型在自然对话中表现不佳，需要更真实的数据集来改进。

Method: 使用SRB-300数据集（300小时真实广播录音）微调OpenAI Whisper模型，覆盖多种方言和自然场景。

Result: 微调后模型显著提升性能，WER降低19%-33%，BLEU分数提高8%-40%，最佳模型WER为17.1%，BLEU为74.8%。

Conclusion: SRB-300数据集和微调方法有效提升了瑞士德语语音识别的鲁棒性，为其他低资源语言提供了参考。
...
### [220] [AdversariaL attacK sAfety aLIgnment(ALKALI): Safeguarding LLMs through GRACE: Geometric Representation-Aware Contrastive Enhancement- Introducing Adversarial Vulnerability Quality Index (AVQI)](https://arxiv.org/abs/2506.08885)
...
<details>
  <summary>Details</summary>
Motivation: 对抗性威胁对LLMs的防御提出了更高要求，现有防御方法存在几何盲点。

Method: 提出ALKALI基准和GRACE框架，通过几何表示增强和对比学习优化防御。

Result: GRACE显著降低攻击成功率（ASR），AVQI指标量化潜在对齐失败。

Conclusion: 几何感知方法能有效提升LLMs的安全性，为未来防御研究提供新方向。
...
### [221] [PlantBert: An Open Source Language Model for Plant Science](https://arxiv.org/abs/2506.08897)
*Hiba Khey,Amine Lakhder,Salma Rouichi,Imane El Ghabi,Kamal Hejjaoui,Younes En-nahli,Fahd Kalloubi,Moez Amri*

Main category: cs.CL

TL;DR: PlantBert是一个基于DeBERTa架构的植物科学领域专用语言模型，专注于从植物应激响应文献中提取结构化知识。
...
Method: 基于DeBERTa架构，结合规则增强的语言后处理和本体实体归一化，使用专家标注的文献进行微调。
...
### [222] [From Legal Texts to Defeasible Deontic Logic via LLMs: A Study in Automated Semantic Analysis](https://arxiv.org/abs/2506.08899)
*Elias Horner,Cristinel Mateis,Guido Governatori,Agata Ciabattoni*

Main category: cs.CL

TL;DR: 论文提出了一种利用大语言模型（LLMs）自动分析法律文本语义的新方法，将其转化为可废止道义逻辑（DDL）的形式化表示。
...
Method: 采用结构化流程，将复杂法律语言分解为原子片段，提取道义规则，并评估其语法和语义一致性。评估了多种LLM配置，包括提示工程、微调模型和多阶段流程。

Result: 实验结果表明，机器生成的形式化表示与专家手工制作的表示具有良好一致性，特别是通过有效提示的LLMs表现突出。

Conclusion: LLMs在有效提示下，能够为可扩展的法律信息学做出重要贡献。
...
### [223] [Dialect Normalization using Large Language Models and Morphological Rules](https://arxiv.org/abs/2506.08907)
*Antonios Dimakis,John Pavlopoulos,Antonios Anastasopoulos*

Main category: cs.CL

TL;DR: 论文提出了一种结合规则和LLM的方言标准化方法，无需平行数据，并在希腊方言上验证了其效果。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言（包括高资源语言的方言）在自然语言处理中的挑战。

Method: 结合基于规则的语言学转换和LLM的少样本提示，无需平行数据。

Result: 在希腊方言数据集上验证了方法，发现先前研究依赖表面语言信息，而新方法能保留更多语义。

Conclusion: 新方法有效且能揭示更深层次的语义信息。
...
### [224] [PropMEND: Hypernetworks for Knowledge Propagation in LLMs](https://arxiv.org/abs/2506.08920)
*Zeyu Leo Liu,Greg Durrett,Eunsol Choi*

Main category: cs.CL

TL;DR: PropMEND是一种基于超网络的知识传播方法，通过元学习修改语言建模损失的梯度，以促进注入知识的传播，解决了现有知识编辑技术在推理任务中的不足。


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑技术无法支持基于注入知识的推理任务，需要一种方法使模型能够回答涉及多跳推理的问题。

Method: 提出PropMEND，扩展MEND的元目标，通过超网络修改梯度更新，使模型能够处理涉及注入知识的多跳问题。

Result: 在RippleEdit数据集上表现优异，多跳问题准确率提升近2倍；在Controlled RippleEdit数据集上，对未见实体-关系对的泛化能力优于现有方法。

Conclusion: PropMEND在知识传播方面表现突出，但对未见关系的泛化能力仍有提升空间，未来需进一步研究。
...
### [225] [Can A Gamer Train A Mathematical Reasoning Model?](https://arxiv.org/abs/2506.08935)
*Andrew Shin*
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在数学推理等任务上表现优异，但其训练通常需要高昂的计算资源。本文旨在证明，通过优化技术，可以在资源有限的环境中实现高性能。

Method: 结合强化学习和内存优化技术，在单张RTX 3080 Ti（16GB内存）上训练1.5B参数的数学推理模型。

Result: 该模型在数学推理基准测试中表现优于或接近更大规模的模型，挑战了高性能数学推理需要大规模基础设施的范式。

Conclusion: 研究表明，通过技术创新，可以在资源受限的环境中实现高性能AI研究，降低了AI研究的门槛。
...
### [226] [FaithfulRAG: Fact-Level Conflict Modeling for Context-Faithful Retrieval-Augmented Generation](https://arxiv.org/abs/2506.08938)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在知识冲突时强制抑制LLM的参数知识，导致模型内部知识结构受损和上下文误解风险增加。

Method: FaithfulRAG在事实层面识别冲突知识，设计自思考过程，让LLM推理并整合冲突事实后再生成响应。

Result: 实验表明，FaithfulRAG优于现有方法。

Conclusion: FaithfulRAG通过显式处理知识冲突，显著提升了生成结果的忠实性和准确性。
...
### [227] [Can LLMs Ground when they (Don't) Know: A Study on Direct and Loaded Political Questions](https://arxiv.org/abs/2506.08952)
*Clara Lachenmaier,Judith Sieker,Sina Zarrieß*

Main category: cs.CL

TL;DR: 论文研究了大型语言模型（LLMs）在政治领域中处理共同基础的能力，发现其在纠正错误用户信念方面存在挑战。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在政治领域中如何管理共同基础，尤其是在存在错误信息和高风险的情况下。

Method: 通过直接知识问题和预设错误信息的引导性问题，评估LLMs的回应能力及其政治偏见。

Result: LLMs在纠正错误用户信念和主动建立共同基础方面表现不佳。

Conclusion: 研究揭示了LLMs在政治话语中减少错误信息的能力有限，引发对其实际应用的担忧。
...
### [228] [Pre-trained Language Models Learn Remarkably Accurate Representations of Numbers](https://arxiv.org/abs/2506.08966)
*Marek Kadlčík,Michal Štefánik,Timothee Mickus,Michal Spiegel,Josef Kuchař*

Main category: cs.CL

TL;DR: 预训练语言模型（LMs）在算术运算中容易出错。现有方法在探测数值表示时效果有限，表明这些错误可能源于分布学习的嵌入在表示精确数量时的固有不可靠性。然而，研究发现之前的探测方法未能捕捉到学习到的数值嵌入的正弦模式结构。为此，提出了一种新的探测技术，能够从输入嵌入中近乎完美地解码数值，证明了LMs在预训练后能高精度表示数字。此外，发现嵌入的精确性与算术错误高度相关，并通过对齐嵌入模式显著减少了这些错误。


<details>
  <summary>Details</summary>
Motivation: 预训练语言模型在算术任务中表现不佳，现有方法未能有效探测其数值表示能力，因此需要更精确的探测技术。

Method: 提出了一种新的探测技术，能够解码输入嵌入中的数值，并验证其准确性。

Result: 新探测技术能近乎完美地解码数值，证明LMs能高精度表示数字；嵌入的精确性与算术错误高度相关。

Conclusion: 通过改进的探测技术和对齐嵌入模式，可以显著减少LMs在算术任务中的错误。
...
### [229] [Atomic-to-Compositional Generalization for Mobile Agents with A New Benchmark and Scheduling System](https://arxiv.org/abs/2506.08972)
*Yuan Guo,Tingjia Miao,Zheng Wu,Pengzhou Cheng,Ming Zhou,Zhuosheng Zhang*

Main category: cs.CL

TL;DR: UI-NEXUS是一个评估移动代理在组合任务上表现的基准，现有代理在性能和效率上表现不佳，AGENT-NEXUS通过动态分解任务提升了成功率。
...
Method: UI-NEXUS包含三类组合操作，支持本地和在线应用环境，AGENT-NEXUS通过动态分解任务优化性能。

Result: 现有代理在UI-NEXUS上表现不佳，AGENT-NEXUS提升了24%至40%的任务成功率。
...
### [230] [FROST-EMA: Finnish and Russian Oral Speech Dataset of Electromagnetic Articulography Measurements with L1, L2 and Imitated L2 Accents](https://arxiv.org/abs/2506.08981)
*Satu Hopponen,Tomi Kinnunen,Alexandre Nikolaev,Rosa González Hautamäki,Lauri Tavi,Einar Meister*

Main category: cs.CL

TL;DR: 介绍了FROST-EMA语料库，包含18名双语者的L1、L2及模仿L2的语音数据，支持语音和技术研究。
...
Result: 案例1：L2和模仿L2对说话人验证系统的影响；案例2：一名说话者在L1、L2及模仿口音中的发音模式。
...
### [231] [Naturalistic Language-related Movie-Watching fMRI Task for Detecting Neurocognitive Decline and Disorder](https://arxiv.org/abs/2506.08986)
*Yuejiao Wang,Xianmin Gong,Xixin Wu,Patrick Wong,Hoi-lam Helene Fung,Man Wai Mak,Helen Meng*

Main category: cs.CL

TL;DR: 论文提出了一种基于语言相关fMRI任务的新方法，用于早期检测老年认知衰退和神经认知障碍（NCD），并在97名非痴呆中国老年人中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 早期检测对预防和减缓老年人群中常见的神经认知障碍（NCD）至关重要，语言相关fMRI可能是一种有前景的方法。

Method: 研究设计了一种自然语言相关的fMRI任务，结合机器学习和人口统计学特征（年龄、性别、教育年限），对参与者认知状态进行分类。

Result: 模型分类性能良好（AUC=0.86），特征定位显示关键脑区与语言处理相关（如颞上回、颞中回、右小脑）。

Conclusion: 自然语言相关fMRI任务在早期检测老年认知衰退和NCD方面具有潜力。
...
### [232] [Employing self-supervised learning models for cross-linguistic child speech maturity classification](https://arxiv.org/abs/2506.08999)
*Theo Zhang,Madurya Suresh,Anne S. Warlaumont,Kasia Hitczenko,Alejandrina Cristia,Margaret Cychosz*

Main category: cs.CL

TL;DR: 论文提出新数据集SpeechMaturity，用于改进儿童语音分类任务，效果优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 儿童语音数据稀缺且复杂，现有系统表现不佳，需更有效的数据集和方法。

Method: 使用新数据集SpeechMaturity训练Transformer模型，分类儿童发声类型（如哭、笑、成熟/不成熟语音）。

Result: 模型表现优于现有方法，分类准确率接近人类水平，且在不同环境下表现稳健。

Conclusion: 新数据集和模型显著提升了儿童语音分类任务的性能。
...
### [233] [SWE-Flow: Synthesizing Software Engineering Data in a Test-Driven Manner](https://arxiv.org/abs/2506.09003)
...
<details>
  <summary>Details</summary>
Motivation: 现有软件工程数据依赖人工提交的问题，SWE-Flow旨在自动生成可验证的TDD任务。

Method: 构建运行时依赖图（RDG）以捕捉函数交互，生成逐步的开发计划，包括部分代码库、单元测试和代码修改。

Result: 从GitHub项目中生成了16,061个训练实例和2,020个测试实例，实验显示模型性能显著提升。

Conclusion: SWE-Flow为TDD任务提供了高效的数据合成方法，并公开了代码、数据集和模型。
...
### [234] [UD-KSL Treebank v1.3: A semi-automated framework for aligning XPOS-extracted units with UPOS tags](https://arxiv.org/abs/2506.09009)
*Hakyung Sung,Gyu-Ho Shin,Chanyoung Lee,You Kyung Sung,Boo Kyung Jung*

Main category: cs.CL

TL;DR: 本文提出了一种半自动化框架，用于将XPOS序列与UPOS类别对齐，扩展了L2韩语语料库，并验证了对齐数据对形态句法分析的提升效果。


<details>
  <summary>Details</summary>
Motivation: 扩展L2韩语语料库并改进形态句法标注的准确性和一致性。

Method: 引入半自动化框架对齐XPOS序列与UPOS类别，并标注2,998个新句子；使用两种NLP工具包对对齐与非对齐数据进行微调。

Result: 对齐数据提高了标注一致性，并显著提升了形态句法标注和依存解析的准确性，尤其在标注数据有限的情况下。

Conclusion: XPOS-UPOS对齐框架有效提升了L2韩语形态句法分析的性能，尤其在数据稀缺时表现更优。
...
### [235] [Learning to Reason Across Parallel Samples for LLM Reasoning](https://arxiv.org/abs/2506.09014)
*Jianing Qi,Xi Ye,Hao Tang,Zhigang Zhu,Eunsol Choi*

Main category: cs.CL

TL;DR: 论文提出了一种名为样本集聚合器（SSA）的紧凑型LLM，通过强化学习优化，用于聚合多个样本生成最终答案，在数学领域表现优于其他测试时扩展方法。


<details>
  <summary>Details</summary>
Motivation: 利用多样本集提升大型语言模型（LLM）在测试时的性能，通过聚合多个样本来提高答案准确性。

Method: 训练一个紧凑型LLM（SSA），输入为多个样本的拼接序列，通过强化学习优化其输出最终答案的准确性。

Result: 在多个推理数据集上，SSA表现优于基于奖励模型的重新排序等方法，并展现出良好的泛化能力。

Conclusion: 通过分离生成答案和分析聚合样本的LLM，SSA能够高效利用黑盒模型的输出，提升性能。
...
### [236] [Comparing human and LLM proofreading in L2 writing: Impact on lexical and syntactic features](https://arxiv.org/abs/2506.09021)
*Hakyung Sung,Karla Csuros,Min-Chang Sung*

Main category: cs.CL

TL;DR: 研究比较了人类和LLM校对在二语写作中的词汇和句法干预效果，发现两者均能提升词汇连贯性，但LLM更倾向于生成性修改，且三种LLM模型结果高度一致。


<details>
  <summary>Details</summary>
Motivation: 探讨人类和LLM校对在提升二语写作可理解性方面的差异及LLM模型间的一致性。

Method: 比较人类和三种LLM（ChatGPT-4o、Llama3.1-8b、Deepseek-r1-8b）对相同二语写作的校对效果，分析词汇和句法变化。

Result: 人类和LLM校对均提升词汇连贯性；LLM更倾向于生成性修改，如使用更多样词汇和形容词修饰。三种LLM结果高度一致。

Conclusion: LLM校对在词汇和句法修改上表现一致且更具生成性，可能为二语写作提供新支持。
...
### [237] [Router-R1: Teaching LLMs Multi-Round Routing and Aggregation via Reinforcement Learning](https://arxiv.org/abs/2506.09033)
*Haozhen Zhang,Tao Feng,Jiaxuan You*

Main category: cs.CL

TL;DR: Router-R1是一个基于强化学习的多LLM路由框架，通过动态调用和整合多个模型的能力，优化性能和成本的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有LLM路由器通常只能单轮一对一映射，无法充分利用多个LLM的互补优势，限制了处理复杂任务的能力。

Method: Router-R1将路由和聚合建模为序列决策过程，利用LLM的推理能力交替执行内部思考和动态模型调用，并通过轻量级规则奖励指导学习。

Result: 在七个通用和多跳QA基准测试中，Router-R1优于多个基线，表现出卓越的性能、泛化能力和成本管理。

Conclusion: Router-R1通过强化学习优化多LLM路由，显著提升了复杂任务的处理能力，同时兼顾性能和成本。
...
### [238] [Same Task, Different Circuits: Disentangling Modality-Specific Mechanisms in VLMs](https://arxiv.org/abs/2506.09047)
*Yaniv Nikankin,Dana Arad,Yossi Gandelsman,Yonatan Belinkov*

Main category: cs.CL

TL;DR: 研究发现视觉-语言模型（VLMs）在视觉和文本任务中存在性能差距，通过分析计算子图（circuits）并提出一种无需训练的干预方法，缩小了部分差距。


<details>
  <summary>Details</summary>
Motivation: 探索视觉-语言模型在视觉和文本任务中性能差距的原因，并提出改进方法。

Method: 比较不同模态的计算子图，分析数据表示差异，并提出通过将视觉数据表示从后期层修补到早期层的方法。

Result: 实验表明，该方法平均缩小了模态间性能差距的三分之一。

Conclusion: 揭示了多模态性能差距的原因，并提出了一种无需训练的干预方法。
...
### [239] [DCD: A Semantic Segmentation Model for Fetal Ultrasound Four-Chamber View](https://arxiv.org/abs/2506.08534)
*Donglian Li,Hui Guo,Minglang Chen,Huizhen Chen,Jialing Chen,Bocheng Liang,Pengchen Liang,Ying Tan*

Main category: eess.IV

TL;DR: 提出了一种基于深度学习的模型DCD，用于胎儿心脏四腔心切面的自动分割，结合Dense ASPP和CBAM模块，提升分割精度。


<details>
  <summary>Details</summary>
Motivation: 胎儿心脏四腔心切面的精确分割对先天性心脏病的早期诊断至关重要，但超声伪影和噪声等问题使其具有挑战性。

Method: 采用Dense ASPP模块进行多尺度特征提取，结合CBAM模块增强自适应特征表示。

Result: DCD模型实现了精确且鲁棒的分割，有助于提升产前心脏评估的准确性。

Conclusion: DCD模型通过结合局部和全局上下文信息，显著改善了胎儿心脏四腔心切面的分割效果。
...
### [240] [Structured Variational $D$-Decomposition for Accurate and Stable Low-Rank Approximation](https://arxiv.org/abs/2506.08535)
*Ronald Katende*

Main category: math.NA

TL;DR: 论文提出了一种非正交矩阵分解方法$D$-decomposition，通过最小化正则化Frobenius损失实现，优于传统方法如SVD和CUR。


<details>
  <summary>Details</summary>
Motivation: 传统矩阵分解方法（如LU或SVD）无法灵活控制秩、稀疏性和条件数，因此需要一种更灵活的分解方法。

Method: 采用交替最小化算法计算$D$-decomposition，形式为$A \approx P D Q$，并通过正则化Frobenius损失优化。

Result: 实验表明，该方法在MovieLens、MNIST等数据集上重建精度优于SVD、CUR和非负矩阵分解，尤其在稀疏和噪声条件下表现更好。

Conclusion: $D$-decomposition是一种灵活且高效的矩阵分解方法，适用于多种实际场景。
...
### [241] [sparseGeoHOPCA: A Geometric Solution to Sparse Higher-Order PCA Without Covariance Estimation](https://arxiv.org/abs/2506.08670)
*Renjie Xu,Chong Wu,Maolin Che,Zhuoheng Ran,Yimin Wei,Hong Yan*

Main category: math.NA

TL;DR: 提出了一种名为sparseGeoHOPCA的新框架，通过几何视角解决高维张量分解问题，提升了计算效率和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统稀疏高阶主成分分析（SHOPCA）方法在高维和不平衡数据场景中效率低且难以解释，需要改进。

Method: 将输入张量沿各模式展开，转化为结构化二元线性优化问题，避免显式协方差估计和迭代缩减。

Result: 理论证明了几何子问题与原始SHOPCA的等价性，计算复杂度线性增长，实验验证了其高效性和鲁棒性。

Conclusion: sparseGeoHOPCA在稀疏支持恢复、分类性能和图像重建方面表现优异，具有广泛适用性。
...
### [242] [A Privacy-Preserving Federated Learning Framework for Generalizable CBCT to Synthetic CT Translation in Head and Neck](https://arxiv.org/abs/2506.08654)
*Ciro Benito Raggio,Paolo Zaffino,Maria Francesca Spadea*

Main category: physics.med-ph

TL;DR: 该论文提出了一种基于联邦学习的CBCT-to-sCT合成方法，解决了多中心数据共享的隐私问题，并在头部和颈部区域验证了其泛化能力。
...
Method: 采用跨机构水平联邦学习框架（FedSynthCT），结合条件生成对抗网络，利用三个欧洲医疗中心的数据进行协作训练。

Result: 联邦模型在多中心数据上表现出色（MAE: 64.38-85.90 HU, SSIM: 0.882-0.922, PSNR: 32.86-34.91 dB），并在外部验证集上保持性能。

Conclusion: 联邦学习为CBCT-to-sCT合成提供了隐私保护的协作解决方案，无需集中数据共享或特定站点微调。
...
### [243] [midr: Learning from Black-Box Models by Maximum Interpretation Decomposition](https://arxiv.org/abs/2506.08338)
*Ryoichi Asashiba,Reiji Kozuma,Hirokazu Iwasawa*

Main category: stat.ME

TL;DR: 论文介绍了R包midr，用于实现最大解释分解（MID），以解释黑盒模型，提供全局替代模型。


<details>
  <summary>Details</summary>
Motivation: 在需要模型和预测可解释性的领域，采用可解释的机器学习（IML）和可解释人工智能（XAI）方法至关重要。

Method: MID是一种功能分解方法，通过最小化黑盒模型预测函数与低阶加法表示之间的平方误差，构建全局替代模型。

Result: midr包能够通过学习黑盒模型，提供具有高级分析能力的全局替代模型。

Conclusion: 论文展示了midr包的用法及其关键特性，为黑盒模型解释提供了新工具。
...
### [244] [Exp4Fuse: A Rank Fusion Framework for Enhanced Sparse Retrieval using Large Language Model-based Query Expansion](https://arxiv.org/abs/2506.04760)
*Lingyuan Liu,Mengxiang Zhang*
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM生成文档质量依赖复杂提示策略和高计算成本的问题，改进稀疏检索。

Method: 引入Exp4Fuse框架，结合原始查询和LLM扩展查询的双重检索路径，使用改进的互惠排名融合方法。
...
### [245] [Hierarchical Lexical Graph for Enhanced Multi-Hop Retrieval](https://arxiv.org/abs/2506.08074)
...
TL;DR: 论文提出了一种名为HLG的三层索引结构，通过StatementGraphRAG和TopicGraphRAG两种检索器，解决了RAG在跨语义远距离文档检索中的不足，并引入合成数据集生成流程以评估多跳检索系统。实验显示，该方法在检索召回率和正确性上平均提升23.1%。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法在需要跨语义远距离文档拼接答案时表现不佳，缺乏对多跳检索系统的严格评估。

Method: 提出HLG三层索引结构，包括原子命题溯源、潜在主题聚类和实体关系链接，并开发两种互补的检索器StatementGraphRAG和TopicGraphRAG。

Result: 在五个数据集上的实验表明，该方法比基于分块的RAG平均提升23.1%的检索召回率和正确性。

Conclusion: HLG和配套检索器有效解决了跨文档检索问题，并通过合成数据集实现了对多跳检索系统的严格评估。
...
### [246] [Bridging RDF Knowledge Graphs with Graph Neural Networks for Semantically-Rich Recommender Systems](https://arxiv.org/abs/2506.08743)
*Michael Färber,David Lamprecht,Yuni Susanti*

Main category: cs.IR

TL;DR: 论文提出了一种将RDF知识图谱与图神经网络（GNN）结合的方法，以提升推荐系统的性能，通过实验验证了其有效性。
...
Method: 提出了RDF KGs与GNN的综合集成方法，利用RDF对象属性的拓扑信息和数据类型属性的内容信息。

Result: 实验表明，利用RDF KGs的语义丰富性显著提升了推荐系统性能。
...
### [247] [Multimodal Representation Alignment for Cross-modal Information Retrieval](https://arxiv.org/abs/2506.08774)
*Fan Xu,Luis A. Leiva*

Main category: cs.IR

TL;DR: 该论文研究了多模态检索中视觉和文本嵌入的几何关系，并比较了不同相似性度量方法的效果，发现Wasserstein距离和余弦相似性表现最佳。


<details>
  <summary>Details</summary>
Motivation: 多模态检索中，不同模型对同一概念的表示方式不同，如何有效对齐这些表示是一个关键问题。

Method: 研究了视觉和文本嵌入的几何关系，并使用四种标准相似性度量和两种基于神经网络的学习度量进行对齐。

Result: Wasserstein距离能有效衡量模态差距，余弦相似性在特征对齐任务中表现最佳，传统架构如多层感知机不足以捕捉复杂交互。
...
### [248] [MOSAIC-F: A Framework for Enhancing Students' Oral Presentation Skills through Personalized Feedback](https://arxiv.org/abs/2506.08634)
...
Result: 在提高口头演讲技能的测试中表现良好。

Conclusion: MOSAIC-F通过结合人类与数据评估，显著提升了反馈的准确性和个性化。
...
TL;DR: 论文提出了一种在NXP MCXN947微控制器上实现的关键词检测系统，结合MFCC特征提取和CNN分类器，通过量化感知训练优化模型，在NPU上实现实时语音交互。
...
Method: 系统采用MFCC特征提取和CNN分类器，通过量化感知训练优化模型，并在集成NPU的微控制器上部署。

Result: 实验结果显示，使用NPU比仅用CPU快59倍，模型大小为30.58 KB，准确率达97.06%。

Conclusion: 研究表明，在嵌入式平台上实现高效低功耗的语音接口是可行的。
...
### [251] [Dynamic Diffusion Schrödinger Bridge in Astrophysical Observational Inversions](https://arxiv.org/abs/2506.08065)
*Ye Zhu,Duo Xu,Zhiwei Deng,Jonathon C. Tan,Olga Russakovsky*

Main category: astro-ph.IM

TL;DR: 论文研究了扩散Schrödinger桥（DSB）模型在天体物理系统中的应用，提出Astro-DSB模型，提升了预测性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决巨分子云（GMCs）中恒星形成的观测逆预测问题，探索扩散模型在天体物理中的应用。

Method: 提出Astro-DSB模型，基于成对域假设，通过物理模拟和真实观测数据（Taurus B213）验证。

Result: 在物理模拟和真实数据中，Astro-DSB在可解释性、学习效率和预测性能上优于传统方法，且在OOD测试中表现更优。

Conclusion: 研究扩展了扩散模型的应用范围，展示了其学习物理动态的能力，为未来物理感知生成模型提供了方向。
...
### [252] [Neural-Augmented Kelvinlet: Real-Time Soft Tissue Deformation with Multiple Graspers](https://arxiv.org/abs/2506.08043)
...
TL;DR: 提出了一种基于物理信息的神经模拟器，结合Kelvinlet先验，实现了实时、高精度的软组织变形模拟。
...
Result: 实现了高保真的手术操作模拟，验证了方法的有效性。

Conclusion: Kelvinlet增强学习是一种高效、物理感知的软组织模拟策略。
...
Result: NQES算法成功计算了多个激发态及其可观测量，揭示了长程相互作用系统的能隙标度和关联特征，与实验观测一致。

Conclusion: NQES算法为量子多体系统激发态计算提供了可扩展且高效的方法，有望应用于量子设备基准测试等领域。
...
TL;DR: 论文提出了一种基于量子退火（QA）和二次化方法的新黑盒优化方案，适用于涉及机器学习的复杂问题。


<details>
  <summary>Details</summary>
Motivation: 传统二次化方法难以处理机器学习中的强非线性和密集交互问题，因此需要一种新的方法。

Method: 使用修正线性单元基的和建模目标函数，并通过数值和解析验证其可行性。

Result: 验证了方法的有效性，并设计了结合QA和二次化的新黑盒优化方案。

Conclusion: 该方法为复杂问题的量子退火应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 研究量子模型中对抗鲁棒性与泛化性的相互作用，填补现有文献空白。

Method: 利用Lipschitz界量化鲁棒性和泛化性，提出正则化训练方法。

Result: 理论结果在时间序列分析中得到验证。

Conclusion: 可训练数据编码策略对量子模型的鲁棒性和泛化性至关重要。
...
TL;DR: 研究探讨了量子计算在生成类似人类行为的密码方面的潜力，提出基于QUBO和UD-MIS的新方法，并在256量子位中性原子量子计算机上验证了其可行性。
...
Method: 采用QUBO和UD-MIS问题编码，通过量子态制备和测量生成密码。

Result: 在256量子位量子计算机上生成了128个类似人类的密码（如“Tunas200992”）。
...
TL;DR: 提出了一种基于叠加参数化量子电路的量子机器学习方法，通过非线性激活函数和并行训练提升表达能力和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有量子机器学习方法受限于线性酉操作和共享参数，无法达到经典深度网络的多层非线性表达能力。

Method: 结合翻转量子随机存储和重复直到成功协议，构建叠加参数化量子电路，嵌入指数级子模型并通过振幅变换实现非线性激活。

Result: 在1D回归任务中误差降低三个数量级，2D分类任务中准确率提升至81.4%，运行方差减少三倍。

Conclusion: 叠加参数化量子电路为硬件高效的量子学习提供了更深的表达能力，适用于复杂决策边界。
...
TL;DR: 论文提出了一种多目标核学习工作流程，用于从高分辨率成像数据中推断控制铁电极化切换的微观结构规则，揭示了域壁构型和局部切换动力学的关键关系。
...
Method: 采用多目标核学习工作流程，结合自动压电响应力显微镜（PFM）实验，分析域壁构型与局部切换动力学的关系。
...
### [259] [Mic-hackathon 2024: Hackathon on Machine Learning for Electron and Scanning Probe Microscopy](https://arxiv.org/abs/2506.08423)
...
TL;DR: 论文探讨了显微镜数据管理中的挑战，提出通过黑客马拉松促进ML与显微镜社区合作，开发标准化工具和数据集。


<details>
  <summary>Details</summary>
Motivation: 解决显微镜数据管理不一致、分析效率低及ML与显微镜社区脱节的问题。

Method: 通过黑客马拉松促进跨学科合作，开发标准化代码、数据集和数字孪生显微镜。

Result: 生成了基准数据集和数字孪生显微镜，代码开源。

Conclusion: 黑客马拉松是推动ML与显微镜融合的有效方式，为未来研究和工作力培养奠定基础。
...
### [260] [A Metrics-Oriented Architectural Model to Characterize Complexity on Machine Learning-Enabled Systems](https://arxiv.org/abs/2506.08153)
...
<details>
  <summary>Details</summary>
Motivation: 探讨复杂性对ML-enabled系统（MLES）的影响，并提供管理复杂性的方法。

Method: 扩展参考架构以描述MLES并收集指标，作为构建指标架构模型的第一步。

Result: 展示了扩展参考架构的初步成果，为后续指标模型奠定基础。

Conclusion: 该研究为MLES的复杂性管理提供了初步框架，未来将进一步开发完整模型。
...
### [261] [Worst-Case Symbolic Constraints Analysis and Generalisation with Large Language Models](https://arxiv.org/abs/2506.08171)
*Daniel Koh,Yannic Noller,Corina S. Pasareanu,Adrians Skapars,Youcheng Sun*

Main category: cs.SE

TL;DR: 论文研究了大型语言模型（LLM）在程序最坏情况执行中的符号约束分析能力，提出了一种新任务并通过符号推理指导的微调提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 探索LLM在复杂符号推理任务中的潜力，填补其在程序分析领域的空白。

Method: 通过SMT约束求解和专门设计的符号约束数据集，对LLM进行符号推理指导的微调。

Result: 实验显示，3B规模的WARP-1.0-3B模型在性能上超越了更大规模的基线模型。

Conclusion: LLM具备深度符号推理能力，支持神经网络学习与形式化方法的更紧密集成。
...
### [262] [Repeton: Structured Bug Repair with ReAct-Guided Patch-and-Test Cycles](https://arxiv.org/abs/2506.08173)
*Nguyen Phu Vinh,Anh Chung Hoang,Chris Ngo,Truong-Son Hy*

Main category: cs.SE

TL;DR: Repeton是一个开源框架，利用LLMs进行精确的代码操作，通过分步的补丁和测试流程提升复杂软件工程任务的性能。
...
Method: 采用结构化补丁和测试流程，逐步诊断问题、提出代码更改并通过自动化测试验证。

Result: 在SWE-bench Lite基准测试中表现优于基于RAG的方法，补丁有效性和可解释性均较好。

Conclusion: Repeton通过模块化和可验证的步骤，为可扩展且透明的自主调试提供了实用路径。
...
### [263] [Understanding Software Engineering Agents Through the Lens of Traceability: An Empirical Study](https://arxiv.org/abs/2506.08311)
*Ira Ceka,Saurabh Pujar,Shyam Ramji,Luca Buratti,Gail Kaiser,Baishakhi Ray*

Main category: cs.SE

TL;DR: 本文通过执行轨迹首次系统研究了软件工程代理（SWE代理）的行为，提出了决策路径的分类法，并分析了影响代理成功的核心组件。
...
Method: 通过执行轨迹分析，提出决策路径分类法，并深入研究代理成功的三个核心组件：错误定位、补丁生成和测试生成。

Result: 研究发现测试生成对成功补丁生产的影响，并揭示了代理生成补丁与开发者编写补丁在结构和风格上的差异。

Conclusion: 研究为代理设计提供了新见解，有助于开发更有效且更符合人类开发实践的代理。
...
### [264] [Do Generative AI Tools Ensure Green Code? An Investigative Study](https://arxiv.org/abs/2506.08790)
...
<details>
  <summary>Details</summary>
Motivation: 随着生成式AI工具的普及，其生成的代码对环境的影响尚未充分研究，论文旨在填补这一空白。

Method: 通过分析ChatGPT、BARD和Copilot三种工具生成的代码，评估其是否符合可持续编码实践。

Result: 结果显示这些工具生成的代码普遍缺乏绿色性，未遵循可持续编码规则。

Conclusion: 论文强调需深入研究并制定有效策略，以提高AI生成代码的可持续性。
...
### [265] [On The Impact of Merge Request Deviations on Code Review Practices](https://arxiv.org/abs/2506.08860)
...
Method: 识别了七种偏差类别，并提出了一种少样本学习检测方法（准确率91%）。通过排除偏差，改进了预测审查完成时间的ML模型性能。

Result: 偏差发生在37.02%的MR中，排除偏差后，53.33%的模型性能提升（最高2.25倍），特征重要性显著变化（整体47%，top-k 60%）。

Conclusion: 研究贡献包括偏差分类、AI检测方法及其对ML分析的影响证据，有助于优化审查流程和确保可靠见解。
...
### [266] [UAVs Meet Agentic AI: A Multidomain Survey of Autonomous Aerial Intelligence and Agentic UAVs](https://arxiv.org/abs/2506.08045)
*Ranjan Sapkota,Konstantinos I. Roumeliotis,Manoj Karkee*

Main category: cs.RO

TL;DR: Agentic UAVs integrate AI for adaptive, goal-driven autonomy in complex environments, surpassing traditional UAVs. The paper covers their architecture, applications, challenges, and future directions.
...
Method: Comprehensive analysis of architectural components, enabling technologies, and comparative advancements in autonomy, learning, and mission flexibility.
...
Conclusion: Proposes a roadmap for future development, emphasizing self-evolving ecosystems and equitable deployment of Agentic UAVs.
...
### [267] [Ego-centric Learning of Communicative World Models for Autonomous Driving](https://arxiv.org/abs/2506.08149)
*Hang Wang,Dechen Gao,Junshan Zhang*

Main category: cs.RO

TL;DR: 论文提出CALL方法，通过生成式AI和世界模型的潜在表示解决多智能体强化学习中的部分可观测性和非平稳性问题，减少通信开销并提升性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习（MARL）在复杂高维环境中面临部分可观测性和非平稳性问题，传统信息共享方法存在通信开销大和可扩展性差的挑战。

Method: CALL方法结合生成式AI和世界模型，将智能体的状态和意图编码为低维潜在表示，通过轻量级通信共享信息，并利用其泛化能力改进预测和规划。
...
### [268] [Re4MPC: Reactive Nonlinear MPC for Multi-model Motion Planning via Deep Reinforcement Learning](https://arxiv.org/abs/2506.08344)
...
Conclusion: Re4MPC为高自由度机器人的实时运动规划提供了一种高效解决方案。
...
### [269] [Diffusion Models for Safety Validation of Autonomous Driving Systems](https://arxiv.org/abs/2506.08459)
*Juanran Wang,Marc R. Schlichting,Harrison Delecki,Mykel J. Kochenderfer*

Main category: cs.RO

TL;DR: 使用去噪扩散模型生成自动驾驶系统潜在故障案例，无需外部数据集，适用于交通路口安全验证。


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶系统安全验证的高风险和成本问题，以及潜在故障的罕见性和多样性。

Method: 训练去噪扩散模型，根据初始交通状态生成潜在故障案例。

Result: 在四路交叉口实验中，模型能生成多样化且真实的故障样本。

Conclusion: 该方法无需外部数据集，计算资源需求低，适用于交通路口安全验证。
...
### [270] [Bayesian Inverse Physics for Neuro-Symbolic Robot Learning](https://arxiv.org/abs/2506.08756)
*Octavio Arriaga,Rebecca Adam,Melvin Laux,Lisa Gutzeit,Marco Ragni,Jan Peters,Frank Kirchner*

Main category: cs.RO

TL;DR: 论文提出了一种结合数据驱动学习与结构化推理的混合神经符号架构，以提升机器人在未知动态环境中的适应性和效率。
...
Result: 机器人能够泛化超出训练数据、推理新情境并持续扩展知识。
...
TL;DR: 开发了一种基于摄像头和模仿学习的全自主假手控制系统，无需用户生成肌电信号即可自动抓取和释放物体。


<details>
  <summary>Details</summary>
Motivation: 传统假手控制方法依赖肌电信号，对用户身心负担大。本研究旨在提供一种更轻松的控制方式。

Method: 通过摄像头和模仿学习训练假手控制模型，利用人类示范数据实现自主抓取和释放。

Result: 模型仅需少量数据即可实现高成功率，并能泛化到不同用户和未见物体。

Conclusion: 该系统显著降低了假手使用的心理负担，提供了一种更易用的控制接口。
...
### [272] [FreqPolicy: Efficient Flow-based Visuomotor Policy via Frequency Consistency](https://arxiv.org/abs/2506.08822)
*Yifei Su,Ning Liu,Dong Chen,Zhen Zhao,Kun Wu,Meng Li,Zhiyuan Xu,Zhengping Che,Jian Tang*

Main category: cs.RO

TL;DR: FreqPolicy通过频率一致性约束改进基于生成模型的视觉运动策略，实现高效的一步动作生成，适用于实时机器人系统。


<details>
  <summary>Details</summary>
Motivation: 现有方法在机器人操作中生成动作轨迹时缺乏时间连续性，而图像生成的加速技术不适用于时间依赖的任务。

Method: 提出FreqPolicy，通过频率一致性约束和自适应一致性损失，确保动作特征在时间上的连贯性。

Result: 在53个任务和3个仿真基准测试中表现优异，集成到VLA模型后实现93.5Hz的推理频率。

Conclusion: FreqPolicy在保持性能的同时显著提升效率，适用于实际机器人场景。
...
### [273] [Flow-Lenia: Emergent evolutionary dynamics in mass conservative continuous cellular automata](https://arxiv.org/abs/2506.08569)
...
TL;DR: Flow-Lenia是Lenia的扩展，用于生成复杂生命样模式，支持多物种模拟和进化动态研究。


<details>
  <summary>Details</summary>
Motivation: 探索人工生命系统中自发生成生命特性的方法，如自复制和进化。

Method: 提出Flow-Lenia，一种质量守恒的Lenia扩展，通过优化参数生成复杂行为模式。

Result: 成功生成具有复杂行为的空间局部化模式，并实现多物种模拟。
...
### [274] [TS-PIELM: Time-Stepping Physics-Informed Extreme Learning Machine Facilitates Soil Consolidation Analyses](https://arxiv.org/abs/2506.08381)
*He Yang,Fei Ren,Hai-Sui Yu,Xueyu Geng,Pei-Zhi Zhuang*

Main category: physics.geo-ph

TL;DR: 论文提出了一种高效且高精度的物理信息机器学习方法TS-PIELM，用于改进传统PINN在土壤固结分析中的不足。


<details>
  <summary>Details</summary>
Motivation: 传统PINN在土壤固结分析中的精度和效率不足，需要改进以成为有竞争力的替代方案。

Method: 提出TS-PIELM框架，将固结过程分为多个时间区间，使用单层前馈极限学习机（ELM）近似解，通过线性方程组直接计算输出层权重，显著提升效率。

Result: TS-PIELM在一维案例中的计算效率和精度分别比PINN提高了1000倍和100倍。

Conclusion: TS-PIELM证明了物理信息机器学习在计算岩土工程中的潜力，是一种高效且高精度的工具。
...
### [275] [POLARON: Precision-aware On-device Learning and Adaptive Runtime-cONfigurable AI acceleration](https://arxiv.org/abs/2506.08785)
*Mukul Lokhande,Santosh Kumar Vishvakarma*

Main category: cs.AR

TL;DR: PARV-CE是一种支持多精度格式的SIMD MAC引擎，通过统一数据路径优化性能和能耗，适用于边缘AI加速。
...
Result: 相比现有技术，PDP提升2倍，资源使用减少3倍，精度损失控制在1.8%以内。

Conclusion: PARV-CE是一种可扩展且高能效的边缘AI加速解决方案。
...
### [276] [Towards Reliable AR-Guided Surgical Navigation: Interactive Deformation Modeling with Data-Driven Biomechanics and Prompts](https://arxiv.org/abs/2506.08048)
*Zheng Han,Jun Zhou,Jialun Pei,Jing Qin,Yingfang Fan,Qi Dou*

Main category: cs.CV

TL;DR: 提出一种数据驱动的生物力学算法，结合人机交互机制，提高AR手术导航中变形建模的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 解决传统有限元方法在AR手术导航中计算成本高且难以处理大解剖变化的问题。

Method: 采用数据驱动的生物力学算法，引入人机交互机制，允许外科医生动态修正解剖对齐。
...
### [277] [IGraSS: Learning to Identify Infrastructure Networks from Satellite Imagery by Iterative Graph-constrained Semantic Segmentation](https://arxiv.org/abs/2506.08137)
...
TL;DR: IGraSS是一个结合语义分割和图优化模块的框架，用于改进不完整的地面实况数据并准确绘制运河网络。


<details>
  <summary>Details</summary>
Motivation: 现有的语义分割模型依赖大量标注数据，但不完整的地面实况会限制其性能。基础设施网络（如运河）的图级属性（如可达性）可用于优化地面实况。

Method: IGraSS结合RGB和多模态数据（NDWI、DEM）的语义分割模块与基于图的优化模块，迭代改进地面实况。

Result: IGraSS将不可达运河段从18%降至3%，且使用优化后的地面实况显著提升运河识别效果。

Conclusion: IGraSS是一个通用框架，可用于优化噪声地面实况和从遥感图像中绘制基础设施网络。
...
### [278] [Surgeon Style Fingerprinting and Privacy Risk Quantification via Discrete Diffusion Models in a Vision-Language-Action Framework](https://arxiv.org/abs/2506.08185)
*Huixin Zhan,Jason H. Moore*

Main category: cs.CV

TL;DR: 论文提出了一种基于离散扩散框架和视觉-语言-动作（VLA）管道的个性化外科医生指纹建模方法，用于机器人手术中精细化的手势预测。
...
Method: 通过多模态输入（内窥镜视频、手术意图语言和隐私感知的外科医生身份嵌入），将手势预测建模为结构化序列去噪任务，并利用第三方语言模型编码个性化指纹。

Result: 在JIGSAWS数据集上验证了方法的有效性，能够准确重建手势序列并学习到独特的外科医生运动指纹。同时发现更个性化的嵌入会提高任务性能，但也增加身份泄露风险。
...
### [279] [A Comprehensive Study of Decoder-Only LLMs for Text-to-Image Generation](https://arxiv.org/abs/2506.08210)
*Andrew Z. Wang,Songwei Ge,Tero Karras,Ming-Yu Liu,Yogesh Balaji*

Main category: cs.CV

TL;DR: 研究探讨了使用现代仅解码器LLM作为文本编码器在文本到图像扩散模型中的效果，发现多层平均嵌入优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型仍使用过时的T5和CLIP作为文本编码器，研究旨在评估现代LLM的效果。

Method: 构建标准化训练和评估流程，训练27个模型，分析12种文本编码器的嵌入提取方式、LLM变体和模型大小。

Result: 多层平均嵌入显著提升复杂提示的对齐效果，多数LLM优于基线T5模型。
...
### [280] [Highly Compressed Tokenizer Can Generate Without Training](https://arxiv.org/abs/2506.08257)
*L. Lao Beyer,T. Li,X. Chen,S. Karaman,K. He*

Main category: cs.CV

TL;DR: 1D图像标记器通过高度压缩的一维序列实现图像编辑和生成能力，无需训练生成模型。


<details>
  <summary>Details</summary>
Motivation: 探索1D图像标记器在高度压缩下的潜在空间表达能力，实现图像编辑和生成。

Method: 使用基于梯度的测试时优化和即插即用损失函数（如重建或CLIP相似性）构建图像生成流程。

Result: 展示了在修复和文本引导图像编辑中的多样性和真实性样本生成能力。

Conclusion: 1D图像标记器通过简单操作即可实现精细图像编辑和生成，无需复杂模型训练。
...
### [281] [Seeing Voices: Generating A-Roll Video from Audio with Mirage](https://arxiv.org/abs/2506.08279)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法要么忽略声音，要么局限于特定领域（如重新配音）。Mirage旨在解决这一问题，实现通用的音频到视频生成。

Method: 提出了一种统一的训练方法，基于自注意力机制，支持从头训练或利用现有权重，生成高质量视频。

Result: Mirage生成的视频在主观质量上优于其他方法，尤其在表现力和真实性方面。

Conclusion: Mirage为音频到视频生成提供了通用且高质量的解决方案，推动了多模态内容生成的发展。
...
### [282] [SEMA: a Scalable and Efficient Mamba like Attention via Token Localization and Averaging](https://arxiv.org/abs/2506.08297)
...
<details>
  <summary>Details</summary>
Motivation: 传统注意力机制（如vanilla softmax attention和linear attention）在计算复杂度和聚焦能力上存在不足，限制了其在计算机视觉任务中的应用。

Method: 通过数学定义广义注意力，并基于Mamba形式的注意力设计了SEMA，利用token定位避免分散，同时通过算术平均捕捉全局注意力。

Result: 在Imagenet-1k分类任务中，SEMA在更大规模的图像上表现优于线性注意力和近期视觉Mamba模型。
...
### [283] [How Much To Guide: Revisiting Adaptive Guidance in Classifier-Free Guidance Text-to-Vision Diffusion Models](https://arxiv.org/abs/2506.08351)
*Huixuan Zhang,Junzhe Zhang,Xiaojun Wan*

Main category: cs.CV

TL;DR: 提出了一种名为Step AG的通用自适应引导策略，通过限制分类器自由引导在前几步去噪中，实现了20%至30%的速度提升，同时保持图像质量和文本对齐。


<details>
  <summary>Details</summary>
Motivation: 当前分类器自由引导方法需要两倍的计算步骤，成本较高，且现有自适应引导方法缺乏分析和实证支持，无法通用。

Method: 提出Step AG策略，限制分类器自由引导在前几步去噪中，减少计算步骤。

Result: 实验表明，该方法在图像质量与文本对齐上表现良好，速度提升20%至30%，适用于多种模型和设置。

Conclusion: Step AG是一种简单通用的自适应引导策略，显著提升了效率且不影响生成质量。
...
### [284] [CulturalFrames: Assessing Cultural Expectation Alignment in Text-to-Image Models and Evaluation Metrics](https://arxiv.org/abs/2506.08835)
...
TL;DR: 研究量化了文本到图像（T2I）模型在文化表达上的偏差，发现模型在显性和隐性文化期望上的表现均不佳，现有评估指标与人类判断相关性低。


<details>
  <summary>Details</summary>
Motivation: 随着T2I模型在视觉内容生成中的普及，其能否准确表达多样文化背景引发担忧。

Method: 引入CulturalFrames基准，覆盖10个国家、5个文化领域，包含983个提示、3637张图像和10k+人工标注，系统评估T2I模型的文化对齐能力。

Result: T2I模型在显性期望（68%）和隐性期望（49%）上均表现不佳，现有评估指标与人类判断相关性差。

Conclusion: 研究揭示了T2I模型在文化表达上的重大缺陷，为开发更具文化意识的模型和评估方法提供了方向。
...
### [285] [Socratic-MCTS: Test-Time Visual Reasoning by Asking the Right Questions](https://arxiv.org/abs/2506.08927)
...
Method: 采用蒙特卡洛树搜索（MCTS）算法，通过插入子问题-子答案对，将推理过程转化为搜索问题。

Result: 在三个基准测试中表现一致提升，MMMU-PRO上总体提升2%，其中文科领域显著提升9%。

Conclusion: 研究表明，通过搜索机制可以激发非推理模型的隐藏知识，生成长推理链，无需额外训练。
...
### [286] [Autoregressive Semantic Visual Reconstruction Helps VLMs Understand Better](https://arxiv.org/abs/2506.09040)
...
<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型（LVLMs）仅对文本序列进行自回归监督，未充分利用视觉模态，导致无法处理无标注图像、遗漏视觉细节及无法表达视觉中心内容等问题。
...
Conclusion: ASVR通过语义重建有效提升多模态理解，为视觉语言模型提供了一种更高效的训练方法。
...
### [287] [MLVTG: Mamba-Based Feature Alignment and LLM-Driven Purification for Multi-Modal Video Temporal Grounding](https://arxiv.org/abs/2506.08512)
*Zhiyi Zhu,Xiaoyu Wu,Zihao Liu,Linlin Yang*

Main category: cs.CV

TL;DR: MLVTG框架通过MambaAligner和LLMRefiner模块解决视频时间定位任务中的冗余注意力和多模态对齐问题，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的方法存在冗余注意力和多模态对齐不佳的问题，需改进。

Method: 提出MLVTG框架，结合MambaAligner（基于Vision Mamba建模时间依赖）和LLMRefiner（利用预训练LLM的语义先验增强对齐）。

Result: 在QVHighlights、Charades-STA和TVSum数据集上取得最优性能。

Conclusion: MLVTG通过双对齐策略显著提升了视频时间定位的准确性。
...
Conclusion: TrajFlow在计算效率和预测性能上均优于现有方法，适用于自动驾驶的实时需求。
...
### [289] [Transformers Meet Hyperspectral Imaging: A Comprehensive Study of Models, Challenges and Open Problems](https://arxiv.org/abs/2506.08596)
...
<details>
  <summary>Details</summary>
Motivation: Transformer在长距离依赖学习中表现出色，但在高光谱成像（HSI）中的应用仍处于起步阶段，需要系统性的综述和指导。

Method: 通过分析300多篇论文，对HSI分类流程的每个阶段（如预处理、特征提取、注意力机制等）进行对比和总结。

Result: 总结了当前Transformer在HSI中的应用进展，并指出了数据稀缺、计算开销等挑战。

Conclusion: 提出了未来研究重点，包括公开数据集、轻量化模型和可解释性机制，以指导下一代HSI应用的发展。
...
### [290] [ECMNet:Lightweight Semantic Segmentation with Efficient CNN-Mamba Network](https://arxiv.org/abs/2506.08629)
*Feixiang Du,Shengkun Wu*

Main category: cs.CV

TL;DR: 提出了一种轻量级ECMNet网络，结合CNN与Mamba，通过胶囊框架解决其互补弱点，显著提升语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 尽管CNN与Transformer在语义分割中表现优异，但全局上下文建模仍不足。Mamba在视觉任务中展现出长程依赖建模优势，因此结合CNN与Mamba以弥补各自不足。

Method: 设计了轻量级瓶颈增强双注意力块（EDAB）、多尺度注意力单元（MSAU）以及Mamba增强的特征融合模块（FFM），用于多尺度特征聚合与特征融合。

Result: 在Cityscapes和CamVid数据集上分别达到70.6%和73.6%的mIoU，参数仅0.87M，计算量为8.27G FLOPs。

Conclusion: ECMNet在准确性与效率间取得平衡，为语义分割任务提供了一种高效解决方案。
...
### [291] [Geometric deep learning for local growth prediction on abdominal aortic aneurysm surfaces](https://arxiv.org/abs/2506.08729)
...
<details>
  <summary>Details</summary>
Motivation: 当前临床指南仅基于AAA最大直径制定监测策略，忽略了3D形状与生长之间的复杂关系，可能导致标准化监测间隔不适用。

Method: 使用SE(3)-对称变换器模型，直接在血管模型表面结合局部多物理特征预测AAA生长。训练数据包括24名AAA患者的113次CTA扫描。

Result: 模型预测AAA生长的中位直径误差为1.18毫米，并能以93%的准确率识别患者是否在两年内需手术修复。外部验证集也证实了模型的泛化能力。

Conclusion: 通过血管表面局部方向性生长预测AAA生长是可行的，可为个性化监测策略提供支持。
...
### [292] [Spatial Transcriptomics Expression Prediction from Histopathology Based on Cross-Modal Mask Reconstruction and Contrastive Learning](https://arxiv.org/abs/2506.08854)
*Junzhuo Liu,Markus Eckstein,Zhixiang Wang,Friedrich Feuerhake,Dorit Merhof*

Main category: cs.CV

TL;DR: 本文提出了一种基于对比学习的深度学习方法，用于从全切片图像预测空间分辨的基因表达，显著提升了预测性能，并在小样本数据中表现出潜力。
...
Conclusion: 该方法不仅提升了预测性能，还能在小样本数据中保持基因相关性，并具有在癌症组织定位中的应用潜力。
...
### [293] [Product of Experts for Visual Generation](https://arxiv.org/abs/2506.08894)
*Yunzhi Zhang,Carson Murtuza-Lanier,Zizhang Li,Yilun Du,Jiajun Wu*

Main category: cs.CV

TL;DR: 提出了一种基于专家乘积（PoE）的框架，通过异构模型的推理时知识组合，提升图像和视频合成的可控性和灵活性。


<details>
  <summary>Details</summary>
Motivation: 整合来自不同来源（如生成模型、视觉语言模型、图形引擎等）的多样化知识，以增强视觉生成任务的表现。

Method: 采用训练无关的方法，通过退火重要性采样（AIS）从专家乘积分布中采样。

Result: 在图像和视频合成任务中表现出更好的可控性，并提供灵活的生成目标指定接口。

Conclusion: PoE框架为异构知识组合提供了一种有效且灵活的解决方案。
...
### [294] [Inherently Faithful Attention Maps for Vision Transformers](https://arxiv.org/abs/2506.08915)
*Ananthu Aniraj,Cassio F. Dantas,Dino Ienco,Diego Marcos*

Main category: cs.CV

TL;DR: 提出了一种基于注意力的两阶段方法，通过二进制注意力掩码限制预测仅受相关图像区域影响，提高对虚假相关性和分布外背景的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决上下文对物体感知的干扰，特别是在分布外背景下可能导致的偏差表示，同时满足需要上下文的任务需求。

Method: 两阶段框架：第一阶段处理全图发现物体部分和任务相关区域；第二阶段利用注意力掩码限制接收域，进行聚焦分析。

Result: 在多样化基准测试中显著提升对虚假相关性和分布外背景的鲁棒性。

Conclusion: 该方法通过联合训练两阶段，有效过滤干扰信息，提升任务表现。
...
### [295] [Segment Concealed Objects with Incomplete Supervision](https://arxiv.org/abs/2506.08955)
...
TL;DR: 论文提出了一种名为SEE的统一方法，用于解决不完全监督的隐蔽目标分割（ISCOS）问题。通过结合视觉基础模型SAM和均值教师框架，生成高质量伪标签，并设计特征分组模块以提升分割效果。


<details>
  <summary>Details</summary>
Motivation: 隐蔽目标分割任务因不完全标注数据和目标与背景的相似性而极具挑战性。现有方法难以应对这些问题，因此需要一种统一且高效的解决方案。

Method: 提出SEE框架，利用SAM生成伪标签，并通过均值教师框架优化训练。设计了伪标签生成、存储和监督策略，以及混合粒度特征分组模块以提升分割效果。

Result: 实验表明，SEE在多个ISCOS任务中达到最先进性能，并能作为即插即用方案提升现有模型表现。

Conclusion: SEE通过结合SAM和均值教师框架，有效解决了ISCOS中的不完全监督和内在相似性问题，为隐蔽目标分割提供了高效解决方案。
...
### [296] [Diversity-Guided MLP Reduction for Efficient Large Vision Transformers](https://arxiv.org/abs/2506.08591)
...
<details>
  <summary>Details</summary>
Motivation: 大型Transformer模型参数过多导致计算和内存成本高昂，MLP模块占据大部分参数。

Method: 采用Gram-Schmidt权重剪枝策略消除MLP冗余神经元，保留权重多样性以提升性能恢复能力。

Result: 在多个大型视觉Transformer上实验，参数和FLOPs减少超过57%，性能几乎无损。EVA-CLIP-E模型参数和FLOPs减少71.5%。

Conclusion: DGMR方法高效压缩模型参数，显著降低成本，性能接近无损。
...
### [297] [Efficient Medical Vision-Language Alignment Through Adapting Masked Vision Models](https://arxiv.org/abs/2506.08990)
*Chenyu Lian,Hong-Yu Zhou,Dongyun Liang,Jing Qin,Liansheng Wang*

Main category: cs.CV

TL;DR: ALTA是一种高效的医学视觉-语言对齐方法，通过适应预训练的视觉模型，显著提升了检索和零样本分类任务的表现。
...
Method: ALTA通过适应预训练的视觉模型（来自掩码记录建模），仅需约8%的可训练参数和不到1/5的计算消耗。还整合了时间多视角放射影像输入以增强一致性。

Result: ALTA在文本到图像准确率上超过最佳对比方法4%以上，图像到文本检索准确率提升约6%。

Conclusion: ALTA通过高效对齐促进了视觉和语言理解的提升，代码已开源。
...
### [298] [Diffuse and Disperse: Image Generation with Representation Regularization](https://arxiv.org/abs/2506.09027)
...
<details>
  <summary>Details</summary>
Motivation: 扩散生成模型通常缺乏显式正则化，且与表示学习进展独立发展。本文旨在弥合生成建模与表示学习之间的差距。

Method: 提出Dispersive Loss，鼓励隐藏空间中的内部表示分散，类似于对比自监督学习，但无需正样本对。

Result: 在ImageNet数据集上评估，Dispersive Loss在多种模型中均表现优于基线方法。

Conclusion: Dispersive Loss是一种简单有效的正则化方法，有望推动生成建模与表示学习的结合。
...
### [299] [Landsat-Bench: Datasets and Benchmarks for Landsat Foundation Models](https://arxiv.org/abs/2506.08780)
*Isaac Corley,Lakshay Sharma,Ruth Crasto*

Main category: cs.CV

TL;DR: Landsat-Bench是一套基于Landsat影像的基准测试，包含EuroSAT-L、BigEarthNet-L和LC100-L，用于评估地理空间基础模型（GFM）的性能。实验表明，基于SSL4EO-L预训练的GFM在下游任务中表现优于ImageNet。
...
Method: 引入Landsat-Bench，包含三个基准测试，并采用标准化的评估方法比较不同架构和预训练模型（如SSL4EO-L和ImageNet）。

Result: SSL4EO-L预训练的GFM在EuroSAT-L和BigEarthNet-L上分别取得了+4% OA和+5.1% mAP的性能提升。

Conclusion: Landsat-Bench为Landsat数据提供了标准化评估工具，且SSL4EO-L预训练的GFM表现更优。
...
### [300] [StreamSplat: Towards Online Dynamic 3D Reconstruction from Uncalibrated Video Streams](https://arxiv.org/abs/2506.08862)
*Zike Wu,Qi Yan,Xuanyu Yi,Lele Wang,Renjie Liao*

Main category: cs.CV

TL;DR: StreamSplat是一个实时动态3D场景重建框架，解决了未校准视频流处理、动态场景建模和长期稳定性的挑战。
...
Conclusion: StreamSplat支持在线重建任意长度视频流，性能优越。
...
### [301] [Data Augmentation For Small Object using Fast AutoAugment](https://arxiv.org/abs/2506.08956)
...
Method: 使用Fast AutoAugment快速找到最优数据增强策略，以克服小目标检测中的性能退化问题。

Result: 在DOTA数据集上实现了20%的性能提升。

Conclusion: 该方法有效提升了小目标检测性能，具有实际应用价值。
...
### [302] [DIsoN: Decentralized Isolation Networks for Out-of-Distribution Detection in Medical Imaging](https://arxiv.org/abs/2506.09024)
*Felix Wagner,Pramit Saha,Harry Anthony,J. Alison Noble,Konstantinos Kamnitsas*

Main category: cs.CV

TL;DR: 论文提出了一种名为Decentralized Isolation Networks (DIsoN)的框架，用于在无法共享数据的情况下检测分布外（OOD）样本，适用于医疗影像等安全关键领域。


<details>
  <summary>Details</summary>
Motivation: 在医疗影像等安全关键领域部署机器学习模型时，需要检测训练数据中未见的输入（OOD样本），以防止不可靠的预测。现有方法通常无法在部署后访问训练数据，或假设测试数据与训练数据集中存储，这在现实中难以实现。

Method: 提出了Isolation Network框架，通过解决二分类任务量化测试样本与训练数据的分离难度。进一步提出DIsoN，允许在无法共享数据的情况下通过交换模型参数进行比较，并扩展了类条件化的版本。
...
### [303] [Learning-Based Multiuser Scheduling in MIMO-OFDM Systems with Hybrid Beamforming](https://arxiv.org/abs/2506.08263)
*Pouya Agheli,Tugce Kobal,François Durand,Matthew Andrews*

Main category: cs.IT

TL;DR: 研究了多用户调度问题，采用混合波束成形和OFDM技术，通过两时间尺度协议和组合算法优化比例公平性。


<details>
  <summary>Details</summary>
Motivation: 提升混合波束成形系统的频谱效率和长期性能，尤其是在毫米波信道下。

Method: 两时间尺度协议：长时尺度分配模拟波束，短时尺度设计数字预编码和用户调度；采用贪婪、排序算法和机器学习方法。

Result: 数值结果表明了性能与复杂度的权衡，不同方法适用于不同场景。

Conclusion: 选择合适的调度方法需根据具体场景的需求和限制。
...
### [304] [The World of AI: A Novel Approach to AI Literacy for First-year Engineering Students](https://arxiv.org/abs/2506.08041)
*Siddharth Siddharth,Brainerd Prince,Amol Harsh,Shreyas Ramachandran*

Main category: cs.CY

TL;DR: 介绍了一门名为《AI世界》的新课程，面向无AI背景的工科大一学生，通过跨学科教学提升学生对AI基础及社会影响的理解。
...
Method: 课程分为三个模块（行星、社会影响、职场），由工程和人文学科教师联合授课，结合技术与社会讨论。

Result: 学生AI挑战认知提升，包括环境意识增强和公平问题解决能力提高。

Conclusion: 课程通过跨学科设计和教学方式，成功提升学生对AI的理解及其社会影响的认知。
...
### [305] [Evaluation of Machine Learning Models in Student Academic Performance Prediction](https://arxiv.org/abs/2506.08047)
*A. G. R. Sandeepa,Sanka Mohottala*

Main category: cs.CY

TL;DR: 该研究探讨了机器学习方法在预测学生学业表现中的应用，使用多层感知器分类器（MLPC）取得了最高86.46%的测试集准确率。


<details>
  <summary>Details</summary>
Motivation: 研究动机是利用机器学习方法预测学生的学业表现，以帮助教育工作者更好地理解和支持学生。

Method: 采用了多层感知器分类器（MLPC）等经典机器学习模型，结合行为、学术和人口统计数据，并通过特征选择和多评估方法优化性能。

Result: MLPC在测试集上最高准确率为86.46%，10折交叉验证平均准确率为79.58%，训练集准确率达99.65%。

Conclusion: 研究表明神经网络（如MLPC）是数据高效模型，特征选择和可解释性方法对模型性能提升至关重要。
...
### [306] [WIP: Large Language Model-Enhanced Smart Tutor for Undergraduate Circuit Analysis](https://arxiv.org/abs/2506.08962)
*Liangliang Chen,Huiru Xie,Jacqueline Rohde,Ying Zhang*

Main category: cs.CY

TL;DR: 本文介绍了一款AI智能导师，用于本科生电路分析课程的作业评估与反馈，展示了其设计、部署及初步效果。


<details>
  <summary>Details</summary>
Motivation: 解决传统作业反馈效率低、个性化不足的问题，同时为教师提供实时学生数据支持。

Method: 设计基于AI的智能导师，包括开放式问题解答和作业反馈生成，部署于Microsoft Azure平台，并收集学生互动数据。

Result: 90.9%的学生对导师表示满意，初步数据分析帮助教师识别学生难点。
...
### [307] [Inverse Design in Distributed Circuits Using Single-Step Reinforcement Learning](https://arxiv.org/abs/2506.08029)
*Jiayu Li,Masood Mortazavi,Ning Yan,Yihong Ma,Reza Zafarani*

Main category: eess.SY

TL;DR: DCIDA是一个分布式电路逆设计框架，通过学习采样策略生成接近最优的设计，显著降低了设计误差。
...
Method: DCIDA通过联合训练的联合条件分布采样设计决策，利用映射将原始设计动作转换为物理表示。

Result: 实验显示DCIDA的Transformer策略网络显著降低了设计误差，尤其在复杂传递函数场景中表现更优。
...
### [308] [Feasibility Study of CNNs and MLPs for Radiation Heat Transfer in 2-D Furnaces with Spectrally Participative Gases](https://arxiv.org/abs/2506.08033)
*Axel TahmasebiMoradi,Vincent Ren,Benjamin Le-Creurer,Chetra Mang*

Main category: eess.SY

TL;DR: 论文提出用CNN和MLP构建替代模型以减少计算成本，CNN表现优于MLP。


<details>
  <summary>Details</summary>
Motivation: 降低数值模拟的计算成本，适应CNN架构处理非图像输入。

Method: 使用ICARUS2D生成数据集，比较CNN和MLP的速度与精度，优化超参数。

Result: CNN和MLP均显著提速且误差可接受，CNN精度更高且对超参数变化更稳健。

Conclusion: CNN在替代模型中表现更优，适合工业应用。
...
### [309] [MasHost Builds It All: Autonomous Multi-Agent System Directed by Reinforcement Learning](https://arxiv.org/abs/2506.08507)
*Kuo Yang,Xingjie Yang,Linhui Yu,Qing Xu,Yan Fang,Xu Wang,Zhengyang Zhou,Yang Wang*

Main category: cs.MA

TL;DR: MasHost是一个基于强化学习的框架，用于自主和查询自适应的多智能体系统设计，通过图搜索问题和分层相对策略优化实现高效、合理的系统构建。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统构建方法依赖人工规则或启发式方法，限制了自主能力并引入人为偏见，需要更自主的构建框架。

Method: 将多智能体系统构建建模为图搜索问题，通过统一概率采样机制联合采样智能体角色和交互，并引入分层相对策略优化（HRPO）进行多目标优化。
...
### [310] [SPBA: Utilizing Speech Large Language Model for Backdoor Attacks on Speech Classification Models](https://arxiv.org/abs/2506.08346)
*Wenhan Yao,Fen Xiao,Xiarun Chen,Jia Liu,YongQiang He,Weiping Wen*

Main category: cs.SD

TL;DR: 论文提出了一种基于语音大语言模型（SLLM）的语音后门攻击方法（SPBA），通过聚焦音色和情感等语音元素生成多样化触发器，并引入多梯度下降算法（MGDA）以应对攻击成本问题。实验证明SPBA在攻击效果上表现优异。


<details>
  <summary>Details</summary>
Motivation: 语音分类任务（如关键词检测和说话人验证）在语音人机交互中至关重要，但其安全性易受后门攻击。现有方法因触发器功能限制，只能生成有限数量的后门。

Method: 利用语音大语言模型（SLLM）生成多样化触发器，聚焦音色和情感等语音元素，并引入多梯度下降算法（MGDA）优化攻击策略。

Result: 实验表明，SPBA在两种语音分类任务中表现出显著的触发器有效性，并在攻击指标上取得优异性能。

Conclusion: SPBA通过多样化触发器和MGDA优化，显著提升了后门攻击的效果，为语音分类任务的安全性研究提供了新思路。
...
### [311] [MD-ViSCo: A Unified Model for Multi-Directional Vital Sign Waveform Conversion](https://arxiv.org/abs/2506.08357)
*Franck Meyer,Kyunghoon Hur,Edward Choi*

Main category: cs.SD

TL;DR: MD-ViSCo是一个统一框架，能够从单一输入波形生成多种目标波形（如ECG、PPG、ABP），优于现有方法。
...
Result: 在公开数据集上，MAE降低8.8%，PC提高4.9%，ABP波形符合AAMI和BHS标准。
...
### [312] [Step-Audio-AQAA: a Fully End-to-End Expressive Large Audio Language Model](https://arxiv.org/abs/2506.08967)
...
TL;DR: Step-Audio-AQAA是一种端到端的大型音频语言模型，通过双码本音频分词器和1300亿参数LLM实现音频查询-音频回答任务，显著提升了语音交互的自然性。
...
Method: 模型采用双码本音频分词器提取特征，结合1300亿参数LLM和神经声码器进行语音合成，并通过后训练优化语义连贯性。

Result: 在StepEval-Audio-360基准测试中，Step-Audio-AQAA在语音控制方面表现优异，超越现有技术。

Conclusion: 该研究为端到端音频语言模型提供了有效解决方案，并验证了基于分词的声码器对提升AQAA任务性能的关键作用。
...
### [313] [Teaching Physical Awareness to LLMs through Sounds](https://arxiv.org/abs/2506.08524)
*Weiguo Wang,Andy Nie,Wenrui Zhou,Yi Kai,Chengchen Hu*

Main category: cs.SD

TL;DR: ACORN框架通过声音教授LLMs物理感知能力，利用物理模拟器生成训练数据，构建AQA-PHY数据集，并连接音频编码器与LLMs，在模拟和现实任务中取得合理结果。


<details>
  <summary>Details</summary>
Motivation: LLMs在文本和多模态处理中表现出色，但缺乏对现实物理现象的理解。ACORN旨在填补这一空白，通过声音教授LLMs物理感知能力。

Method: ACORN引入基于物理的模拟器，结合真实声音源和可控物理通道生成多样化训练数据，构建AQA-PHY数据集，并设计音频编码器处理幅度和相位信息。

Result: 在模拟和现实任务（如视线检测、多普勒效应估计和到达方向估计）中取得合理结果。

Conclusion: ACORN为LLMs理解物理世界铺平了道路，展示了其在物理感知任务中的潜力。
...
### [314] [Auto-Regressive vs Flow-Matching: a Comparative Study of Modeling Paradigms for Text-to-Music Generation](https://arxiv.org/abs/2506.08570)
*Or Tal,Felix Kreuk,Yossi Adi*

Main category: cs.SD

TL;DR: 论文比较了文本到音乐生成中两种常见建模范式（自回归解码和条件流匹配）的性能差异，通过控制变量实验揭示了各自的优缺点。
...
Method: 使用相同数据集、训练配置和架构，对比自回归解码和条件流匹配范式，评估生成质量、鲁棒性、可扩展性等。

Result: 研究揭示了两种范式的独特优势和局限性，为未来模型设计提供实用见解。

Conclusion: 比较分析为文本到音乐生成系统的未来架构和训练决策提供了指导。
...
### [315] [ChemGraph: An Agentic Framework for Computational Chemistry Workflows](https://arxiv.org/abs/2506.06363)
*Thang D. Pham,Aditya Tanikanti,Murat Keçeli*

Main category: physics.chem-ph

TL;DR: ChemGraph是一个基于AI的框架，用于自动化计算化学和材料科学工作流，结合图神经网络和大型语言模型，简化复杂任务。
...
Method: 利用图神经网络和大型语言模型，提供直观界面，支持多种计算任务和方法。

Result: 在13个基准任务中，小模型适用于简单任务，复杂任务需大模型，多代理框架可提升小模型性能。

Conclusion: ChemGraph通过多代理框架和AI技术，显著提升了原子模拟的效率和可访问性。
...
### [316] [Aligning Proteins and Language: A Foundation Model for Protein Retrieval](https://arxiv.org/abs/2506.08023)
...
<details>
  <summary>Details</summary>
Motivation: 利用视觉-语言模型（VLMs）的最新进展，促进蛋白质结构的功能解释，特别是在冷冻电镜（cryo-EM）等结构测定方法中。

Method: 构建了一个包含约20万蛋白质-描述对的大规模数据集，采用对比学习训练模型，并在PDB和EMDB数据集上评估其检索性能。

Result: 模型在域内和跨数据库检索中均表现出良好的零样本检索性能。

Conclusion: 多模态基础模型在蛋白质生物学中的结构与功能理解方面具有潜力。
...
### [317] [EDINET-Bench: Evaluating LLMs on Complex Financial Tasks using Japanese Financial Statements](https://arxiv.org/abs/2506.08762)
*Issa Sugiura,Takashi Ishida,Taro Makino,Chieko Tazuke,Takanori Nakagawa,Kosuke Nakago,David Ha*

Main category: q-fin.ST

TL;DR: 论文介绍了EDINET-Bench，一个开源日本金融基准数据集，用于评估大语言模型在金融任务中的表现，结果显示当前模型表现不佳。


<details>
  <summary>Details</summary>
Motivation: 解决日本金融数据稀缺问题，推动金融分析领域的学术创新。

Method: 通过下载EDINET过去10年的年报数据，自动标注任务标签，构建EDINET-Bench数据集。

Result: 即使最先进的大语言模型在欺诈检测和盈利预测等任务中表现仅略优于逻辑回归。

Conclusion: 大语言模型在金融领域的应用仍面临挑战，需领域特定优化，数据集公开以促进未来研究。
...
### [318] [Generalizing while preserving monotonicity in comparison-based preference learning models](https://arxiv.org/abs/2506.08616)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于比较的偏好学习模型（如大型语言模型）缺乏单调性保证，且广义Bradley-Terry模型无法泛化到未比较数据。本文旨在填补这一空白。

Method: 提出线性广义Bradley-Terry模型，结合扩散先验，并确定保证单调性的嵌入条件。

Result: 实验表明，新模型在有限数据集上提高了准确性，且单调性并非普遍保证。

Conclusion: 新模型在保证单调性的同时具备泛化能力，尤其适用于数据有限的情况。
...
### [319] [PerfTracker: Online Performance Troubleshooting for Large-scale Model Training in Production](https://arxiv.org/abs/2506.08528)
...
Method: PerfTracker利用细粒度分析和在线性能分析，结合差分可观测性，定位硬件和软件层面的性能问题。

Result: PerfTracker已部署在包含约10,000个GPU的生产集群中，成功诊断多种复杂性能问题。
...
### [320] [KP-PINNs: Kernel Packet Accelerated Physics Informed Neural Networks](https://arxiv.org/abs/2506.08563)
*Siyuan Yang,Cheng Song,Zhilu Lai,Wenjia Wang*

Main category: cs.CE

TL;DR: KP-PINNs提出了一种基于RKHS范数的新损失函数，并通过KP方法加速计算，提高了PINNs的稳定性和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统PINNs使用L2损失函数在某些复杂微分方程中表现不稳定且不准确，因此需要改进。

Method: 提出KP-PINNs框架，利用RKHS范数重新定义损失函数，并采用KP方法加速计算。
...
### [321] [Real-Time Cascade Mitigation in Power Systems Using Influence Graph Improved by Reinforcement Learning](https://arxiv.org/abs/2506.08893)
...
<details>
  <summary>Details</summary>
Motivation: 现代电力系统因可再生能源渗透增加而面临级联停电风险上升的问题，需快速且复杂的实时决策。

Method: 将影响图扩展为MDP模型，结合强化学习（策略梯度算法）求解，并设计奖励机制以保守决策。

Result: 在IEEE 14-bus和118-bus系统验证中，主动断开线路可有效降低级联风险，并识别出关键线路。

Conclusion: 该方法通过保守策略和快速收敛的算法，显著提升了级联停电的实时缓解能力。
...
### [322] [How Good LLM-Generated Password Policies Are?](https://arxiv.org/abs/2506.08320)
*Vivek Vaidya,Aditya Patwardhan,Ashish Kundu*

Main category: cs.CR

TL;DR: 论文研究了生成式AI（特别是大语言模型LLMs）在网络安全访问控制系统中的应用，重点关注LLM生成密码策略的一致性和准确性。


<details>
  <summary>Details</summary>
Motivation: LLMs在自然语言处理方面表现出色，但其输出的不一致性和不可预测性在安全关键领域（如访问控制）带来挑战，尤其是LLM生成响应的稳定性问题。

Method: 采用两种方法：1）仅通过自然语言提示生成pwquality.conf配置文件；2）提供官方文档作为参考基线，评估生成配置的合理性、准确性和一致性。

Result: 研究发现当前LLMs在生成配置时存在显著挑战，突显了其在访问控制系统中部署的局限性。
...
TL;DR: GradEscape是一种基于梯度的攻击方法，专门针对AI生成文本检测器，通过加权嵌入和模型参数更新实现高效攻击，并在实验中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决文本离散性导致的不可微分问题，以及检测器与攻击模型之间的分词器不匹配问题。

Method: 引入加权嵌入和暖启动攻击模型，结合分词器推断和模型提取技术。

Result: 在多个数据集和语言模型上表现优异，成功应用于商业检测器。
...
### [324] [Your Agent Can Defend Itself against Backdoor Attacks](https://arxiv.org/abs/2506.08336)
*Li Changjiang,Liang Jiacheng,Cao Bochuan,Chen Jinghui,Wang Ting*

Main category: cs.CR

TL;DR: ReAgent是一种针对LLM代理后门攻击的新型防御方法，通过检测用户指令、代理规划和执行间的不一致性来识别攻击，显著降低攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型代理在训练和微调中面临后门攻击的安全风险，可能导致恶意操作，亟需有效防御手段。

Method: ReAgent采用两级检测方法：执行层面验证代理思维与行动的一致性，规划层面通过重构指令检查与用户指令的一致性。

Result: ReAgent在多种任务中有效防御后门攻击，如数据库操作任务中攻击成功率降低90%，优于现有方法。

Conclusion: 研究表明，利用被攻击代理自身能力可有效缓解后门风险，ReAgent为LLM代理安全提供了新思路。
...
### [325] [WGLE:Backdoor-free and Multi-bit Black-box Watermarking for Graph Neural Networks](https://arxiv.org/abs/2506.08602)
...
Method: WGLE基于LDDE（层间边距离差）量化节点特征与预测距离的差异，通过预定义边上的LDDE值嵌入水印，避免影响主任务。

Result: 在六个数据集和GNN架构上，WGLE实现了100%验证准确率，0.85%的平均保真度下降，且嵌入开销低。
...
### [326] [Interpreting Agent Behaviors in Reinforcement-Learning-Based Cyber-Battle Simulation Platforms](https://arxiv.org/abs/2506.08192)
*Jared Claypoole,Steven Cheung,Ashish Gehani,Vinod Yegneswaran,Ahmad Ridley*

Main category: cs.CR

TL;DR: 论文分析了两个开源深度强化学习代理在CAGE Challenge 2中的表现，通过简化状态和动作空间以及跟踪关键事件，揭示了防御和攻击代理的细粒度行为。


<details>
  <summary>Details</summary>
Motivation: 研究目的是通过分析代理在模拟网络防御中的表现，提高其行为的可解释性，并识别成功与失败的模式。

Method: 通过简化复杂的状态和动作空间，并跟踪关键事件（如渗透和清除事件），分析代理的行为。

Result: 防御代理通常在主机被攻击后1-2个时间步内清除渗透；某些关键动作的无效性高达40%-99%；诱饵服务可阻止高达94%的直接特权访问攻击。

Conclusion: 研究提供了对代理行为的深入理解，并指出了CAGE Challenge 4中改进的方向。
...
### [327] [Approaching Dialogue State Tracking via Aligning Speech Encoders and LLMs](https://arxiv.org/abs/2506.08633)
*Šimon Sedláček,Bolaji Yusuf,Ján Švec,Pradyoth Hegde,Santosh Kesiraju,Oldřich Plchot,Jan Černocký*

Main category: eess.AS

TL;DR: 通过连接语音编码器和LLM的表示空间，结合开源组件（WavLM-large, OLMo），在SpokenWOZ数据集上实现了对话状态跟踪（DST）的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过开源组件和模块化设计提升语音对话状态跟踪的性能。

Method: 使用WavLM-large和OLMo，通过小连接模块桥接表示空间，并研究适配器微调、对话历史代理轮次及模糊匹配后处理的影响。

Result: 最佳模型（WavLM + connector + OLMo-1B）在SpokenWOZ测试集上达到34.66% JGA，Gemma-2-9B-instruct进一步提升至42.17% JGA。

Conclusion: 模块化设计和开源组件能有效提升语音DST性能，模糊匹配后处理显著改善命名实体识别。
...
### [328] [CaliciBoost: Performance-Driven Evaluation of Molecular Representations for Caco-2 Permeability Prediction](https://arxiv.org/abs/2506.08059)
...
Method: 系统评估八种分子特征表示类型（2D/3D描述符、结构指纹、深度学习嵌入）结合AutoML技术，使用TDC基准和OCHEM数据集验证。

Result: PaDEL、Mordred和RDKit描述符表现最佳；AutoML模型CaliciBoost的MAE最优；3D描述符比2D降低15.73% MAE。

Conclusion: AutoML在ADMET建模中有效，为数据有限的任务提供了特征选择指导。
...
### [329] [Protriever: End-to-End Differentiable Protein Homology Search for Fitness Prediction](https://arxiv.org/abs/2506.08954)
*Ruben Weitzman,Peter Mørch Groth,Lood Van Niekerk,Aoi Otani,Yarin Gal,Debora Marks,Pascal Notin*

Main category: q-bio.QM

TL;DR: Protriever是一个端到端的可微分框架，通过学习检索相关同源序列并同时训练目标任务，解决了传统MSA方法的高计算成本和局限性。


<details>
  <summary>Details</summary>
Motivation: 传统基于MSA的同源序列检索方法计算成本高，对高度分歧序列或复杂插入删除模式效果不佳，且与下游建模目标无关。

Method: Protriever框架结合检索和目标任务训练，利用高效向量搜索实现快速检索。
...
### [330] [Physics-Informed Teleconnection-Aware Transformer for Global Subseasonal-to-Seasonal Forecasting](https://arxiv.org/abs/2506.08049)
...
<details>
  <summary>Details</summary>
Motivation: S2S预报因大气系统的混沌性和多尺度交互复杂性而面临挑战，现有方法未能明确建模关键物理过程和遥相关。

Method: TelePiT包含三个关键组件：球谐嵌入、多尺度物理神经ODE和遥相关感知Transformer。

Result: TelePiT在实验中显著优于现有数据驱动和数值预报系统，如2米温度RMSE降低57.7%。

Conclusion: TelePiT通过整合物理和遥相关建模，为S2S预报提供了高效解决方案。
...
### [331] [WWAggr: A Window Wasserstein-based Aggregation for Ensemble Change Point Detection](https://arxiv.org/abs/2506.08066)
*Alexander Stepikin,Evgenia Romanenkova,Alexey Zaytsev*

Main category: stat.ML

TL;DR: 本文提出了一种基于Wasserstein距离的新型集成聚合方法WWAggr，用于解决高维数据流中的变化点检测问题，提升了现有方法的性能并解决了决策阈值选择的难题。


<details>
  <summary>Details</summary>
Motivation: 高维数据流中的变化点检测（CPD）因数据模式复杂和常见假设不成立而具有挑战性，现有深度神经网络检测器性能不足，集成方法虽能提升鲁棒性，但标准聚合技术（如平均）效果不佳。

Method: 提出WWAggr方法，基于Wasserstein距离进行任务特定的集成聚合，适用于多种深度CPD模型集成。

Result: WWAggr方法有效提升了变化点检测的性能，并解决了决策阈值选择的长期问题。

Conclusion: WWAggr是一种高效且通用的集成聚合方法，显著提升了高维数据流中变化点检测的准确性和鲁棒性。
...
### [332] [Constrained Pareto Set Identification with Bandit Feedback](https://arxiv.org/abs/2506.08127)
*Cyrille Kone,Emilie Kaufmann,Laura Richert*

Main category: stat.ML

TL;DR: 论文提出了一种在多臂老虎机设置下识别满足可行性约束的Pareto集的方法，其算法在固定置信度下显著优于传统方法，并证明了其样本复杂度的接近最优性。


<details>
  <summary>Details</summary>
Motivation: 解决在多目标优化中识别Pareto集时面临的可行性约束问题，特别是在未知均值的情况下。

Method: 提出了一种新算法，用于在固定置信度下识别满足线性约束的Pareto集，并通过信息理论下界证明其样本复杂度的优越性。

Result: 算法在实验中显著优于竞速类算法和两阶段方法，且样本复杂度接近理论最优。

Conclusion: 该算法在多目标优化中识别Pareto集时具有高效性和理论保障，适用于实际应用。
...
### [333] [Model-Free Kernel Conformal Depth Measures Algorithm for Uncertainty Quantification in Regression Models in Separable Hilbert Spaces](https://arxiv.org/abs/2506.08325)
*Marcos Matabuena,Rahul Ghosal,Pavlo Mozharovskyi,Oscar Hernan Madrid Padilla,Jukka-Pekka Onnela*

Main category: stat.ML

TL;DR: 提出了一种基于条件深度度量的新型无模型不确定性量化算法，用于定义预测区域，适用于可分离希尔伯特空间中的预测变量和响应变量。


<details>
  <summary>Details</summary>
Motivation: 深度度量在复杂随机对象中有广泛应用，但其在回归建模中提供预测区域的研究较少。

Method: 结合条件核均值嵌入和集成深度度量，提出算法，并引入保形预测变体以增强实用性。

Result: 算法在模拟研究和数字健康应用中表现良好，具有快速收敛和一致性保证。

Conclusion: 新方法在理论和实践中均表现出色，适用于多种数据类型和场景。
...
### [334] [Asymptotic Normality of Infinite Centered Random Forests -Application to Imbalanced Classification](https://arxiv.org/abs/2506.08548)
*Moria Mayala,Erwan Scornet,Charles Tillier,Olivier Wintenberger*

Main category: stat.ML

TL;DR: 本文研究了在类别不平衡数据上训练中心化随机森林（CRF）的理论，提出了一种去偏估计器IS-ICRF，并证明了其在高度不平衡设置下的方差降低优势。


<details>
  <summary>Details</summary>
Motivation: 类别不平衡数据在分类任务中常见，但现有方法缺乏理论支持，本文旨在填补这一空白。

Method: 通过重要性采样（IS）方法提出去偏估计器IS-ICRF，并分析其在CRF上的表现。

Result: 证明了IS-ICRF满足中心极限定理（CLT），且在高度不平衡设置下具有方差降低的优势。

Conclusion: 理论分析和实验表明，在再平衡数据集上训练随机森林（并去偏）优于直接使用原始数据。
...
### [335] [Flexible and Efficient Drift Detection without Labels](https://arxiv.org/abs/2506.08734)
*Nelvin Tan,Yu-Ching Shih,Dong Yang,Amol Salunkhe*

Main category: stat.ML

TL;DR: 提出了一种无标签环境下高效检测概念漂移的算法，基于统计过程控制，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型广泛用于自动化决策，需确保性能，但传统方法依赖即时标签，难以应对大规模数据。

Method: 使用统计过程控制在无标签设置下检测概念漂移，并引入新框架整合先验检测信息。

Result: 在计算限制下，算法统计功效优于现有方法，数值模拟显示良好性能。

Conclusion: 新算法和框架在无标签环境下高效检测概念漂移，适用于大规模数据场景。
...
### [336] [syren-baryon: Analytic emulators for the impact of baryons on the matter power spectrum](https://arxiv.org/abs/2506.08783)
*Lukas Kammerer,Deaglan J. Bartlett,Gabriel Kronberger,Harry Desmond,Pedro G. Ferreira*

Main category: astro-ph.CO

TL;DR: 论文提出了一种符号回归方法，用于构建重子物理对物质功率谱影响的解析近似，并提供了不同重子物理模型的函数表达及其不确定性分析。


<details>
  <summary>Details</summary>
Motivation: 重子物理对宇宙物质分布的显著影响是当前和未来宇宙学调查的关键系统误差，需要简单且物理合理的参数化方法。

Method: 使用符号回归从CAMELS流体动力学模拟中构建重子物理对物质功率谱影响的解析近似，并分析其不确定性和物理行为。

Result: 得到的解析近似误差与样本方差相当，且在小尺度上误差增加，但仍优于1%。这些函数可用于区分不同重子物理模型。

Conclusion: 解析形式的函数可直接解释宇宙学和反馈参数的影响，并公开了相关代码，为实际数据分析提供了工具。
...
### [337] [MOSS: Multi-Objective Optimization for Stable Rule Sets](https://arxiv.org/abs/2506.08030)
...
<details>
  <summary>Details</summary>
Motivation: 解决决策规则集中稀疏性、准确性和稳定性之间的权衡问题，提供快速评估和选择模型的能力。
...
### [338] [Continuous Policy and Value Iteration for Stochastic Control Problems and Its Convergence](https://arxiv.org/abs/2506.08121)
*Qi Feng,Gu Wang*

Main category: math.OC

TL;DR: 提出了一种连续策略-值迭代算法，通过Langevin型动力学同时更新随机控制问题的值函数和最优控制的近似解。


<details>
  <summary>Details</summary>
Motivation: 解决熵正则化松弛控制问题和经典控制问题中的值函数和最优控制同时优化问题。

Method: 利用Langevin型随机微分方程进行连续更新，结合分布采样和非凸学习技术。

Result: 在哈密顿单调性条件下，证明了策略改进和收敛到最优控制。

Conclusion: 该方法为机器学习中的分布采样和非凸学习技术提供了新的应用场景。
...
### [339] [Solving Convex-Concave Problems with $\tilde{\mathcal{O}}(ε^{-4/7})$ Second-Order Oracle Complexity](https://arxiv.org/abs/2506.08362)
...
### [340] [Sharper Convergence Rates for Nonconvex Optimisation via Reduction Mappings](https://arxiv.org/abs/2506.08428)
*Evan Markou,Thalaiyasingam Ajanthan,Stephen Gould*

Main category: math.OC

TL;DR: 论文提出了一种利用高维优化问题中最小化器的几何结构（如平滑流形）的框架，通过降维映射改善优化问题的曲率特性，从而加速收敛。


<details>
  <summary>Details</summary>
Motivation: 高维优化问题的最小化器常具有几何结构（如平滑流形），若能利用这些结构，可通过降维映射去除冗余方向，优化问题条件。

Method: 引入降维映射框架，将参数空间重新参数化到解流形上，去除冗余方向，改善目标函数的曲率特性。

Result: 理论证明，设计良好的降维映射能改善优化问题的曲率，实现更快收敛。

Conclusion: 该框架为利用结构信息加速优化提供了统一的理论解释，支持了相关算法的经验效果。
...
### [341] [Optimization over Sparse Support-Preserving Sets: Two-Step Projection with Global Optimality Guarantees](https://arxiv.org/abs/2506.08558)
*William de Vazelhes,Xiao-Tong Yuan,Bin Gu*

Main category: math.OC

TL;DR: 本文提出了一种新的迭代硬阈值算法，用于解决稀疏优化中带有额外支持保留约束的问题，并提供了全局收敛保证。


<details>
  <summary>Details</summary>
Motivation: 稀疏优化中，硬约束（如ℓ0伪范数）能更好地控制稀疏性，但实际应用中常需额外约束。现有方法通常需要闭式投影或仅提供局部收敛保证，本文旨在填补这一空白。

Method: 提出了一种带有两步连续投影算子的迭代硬阈值算法，适用于混合约束，并引入了稀疏性与次优性的新权衡。

Result: 在确定性、随机和零阶设置下，算法在常规限制性强凸/光滑假设下提供了全局目标值保证。

Conclusion: 本文通过扩展经典三点引理，为混合约束提供了新的收敛分析技术，并在零阶情况下改进了现有结果。
...
TL;DR: 研究发现，指令调优的多模态大语言模型（MLLMs）在视频和音频任务中显著优于非指令调优模型，且其层次结构与大脑活动高度对齐。


<details>
  <summary>Details</summary>
Motivation: 填补现有研究在评估MLLMs与大脑对齐性时对指令调优模型和多模态刺激的不足。
...
### [343] [QUITE: A Query Rewrite System Beyond Rules with LLM Agents](https://arxiv.org/abs/2506.07675)
*Yuyang Song,Hanxu Yan,Jiale Lao,Yibo Wang,Yufei Li,Yuanchun Zhou,Jianguo Wang,Mingjie Tang*

Main category: cs.DB

TL;DR: 论文提出了一种基于LLM的SQL查询重写方法QUITE，通过多智能体框架和实时反馈，解决了规则方法的局限性，显著提升了查询性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于规则的查询重写方法覆盖范围有限且可能引发性能退化，而人类专家和LLM在语义和推理能力上表现优异，但直接应用LLM会导致查询不等价或次优。

Method: 设计了基于有限状态机的多智能体框架，结合外部工具和实时数据库反馈；开发了重写中间件和提示注入技术以优化查询。

Result: QUITE将查询执行时间最多减少35.8%，重写数量增加24.1%，覆盖了以往方法无法处理的查询案例。

Conclusion: QUITE通过LLM智能体和反馈机制，显著提升了查询重写的性能和覆盖范围，超越了传统规则方法。
...
### [344] [RADAR: Benchmarking Language Models on Imperfect Tabular Data](https://arxiv.org/abs/2506.08249)
...
Result: 前沿模型在无数据异常时表现尚可，但在数据异常存在时性能显著下降，显示出其在数据感知分析中的不足。

Conclusion: RADAR为提升表格推理能力提供了灵活且可扩展的资源，揭示了语言模型在数据感知分析中的关键短板。
...
### [345] [LEANN: A Low-Storage Vector Index](https://arxiv.org/abs/2506.08276)
...
TL;DR: LEANN是一种高效的近似最近邻搜索索引，专为资源受限的个人设备设计，显著减少存储开销。


<details>
  <summary>Details</summary>
Motivation: 本地设备上存储个人数据时，传统嵌入搜索的高存储开销难以实现，需要一种更高效的解决方案。

Method: LEANN结合紧凑的图结构和动态重计算策略，以最小化存储需求。

Result: LEANN将索引大小降至原始数据的5%，存储减少50倍，同时保持90%的top-3召回率和2秒内的响应时间。

Conclusion: LEANN为本地设备上的嵌入搜索提供了一种高效且实用的解决方案。
